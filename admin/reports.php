...
if ($report) {
    ...
    $isPrintable = $reportdata["isPrintable"] ?? true;
    $canCsvExport = $reportdata["canCsvExport"] ?? true;
    ...
    if (!$print) {
        if ($isPrintable || $canCsvExport) {
            $link = App::getPhpSelf();
            $menu = "";
            $menuItems = [];
            $menuItemTemplate = "<li>\n    <a href=\"%s\">\n        <i class=\"fas %s\"></i> %s\n    </a>\n</li>";
            if ($canCsvExport) {
                $menuItems[] = ["csvdownload.php?" . $requestString, "fa-download", AdminLang::trans("reports.exportcsv")];
            }
            if ($isPrintable) {
                $menuItems[] = [$link . "?" . $requestString . "&print=true", "fa-print", AdminLang::trans("reports.printableversion")];
            }
            foreach ($menuItems as $menuItem) {
                $menu .= vsprintf($menuItemTemplate, $menuItem);
            }
    ...
    echo "<p>" . AdminLang::trans("reports.description") . "</p>";
    $reports = ["reports.group.general" => ["daily_performance", "disk_usage_summary", "monthly_orders", "product_suspensions", "promotions_usage", "ssl_certificate_monitoring", ""], "reports.group.billing" => ["aging_invoices", "credits_reviewer", "direct_debit_processing", "sales_tax_liability", "vat_moss", ""], "reports.group.income" => ["annual_income_report", "income_forecast", "income_by_product", "monthly_transactions", "sales_tax_liability", "server_revenue_forecasts", ""], "reports.group.clients" => ["new_customers", "client_sources", "client_statement", "clients_by_country", "top_10_clients_by_income", "affiliates_overview", "domain_renewal_emails", "customer_retention_time", ""], "reports.group.support" => ["support_ticket_replies", "ticket_feedback_scores", "ticket_feedback_comments", "ticket_ratings_reviewer", "ticket_tags", ""], "reports.group.exports" => ["client", "clients", "domains", "invoices", "services", "transactions", "pdf_batch", "", "", ""], "reports.group.system" => ["smarty_compatibility", ""]];
    echo "<div class=\"reports-index\">";
    foreach ($reports as $type => $reports_array) {
        echo "<h2>" . AdminLang::trans($type) . "</h2>";
        $reps = [];
        switch ($type) {
            case "reports.group.general":
                $btnclass = "btn-info";
                break;
            case "reports.group.exports":
