...
$aInt->title = AdminLang::trans("configOptions.pageTitle");
$aInt->sidebar = "config";
...
$manageoptions = App::getFromRequest("manageoptions");
$action = App::getFromRequest("action");
$save = App::getFromRequest("save");
$deleted = App::getFromRequest("deleted");
$duplicated = App::getFromRequest("duplicated");
$id = App::getFromRequest("id");
$savegroup = App::getFromRequest("savegroup");
$name = App::getFromRequest("name");
$description = App::getFromRequest("description");
$order = App::getFromRequest("order");
$deleteconfigoption = App::getFromRequest("deleteconfigoption");
$cid = App::getFromRequest("cid");
$group = App::getFromRequest("group");
$optionname = App::getFromRequest("optionname");
$price = App::getFromRequest("price");
$gid = App::getFromRequest("gid");
$hidden = $requestOptionalArray("hidden");
$sortorder = $requestOptionalArray("sortorder");
$productlinks = $requestOptionalArray("productlinks");
$jquerycode = App::getFromRequest("jquerycode");
$addhidden = App::getFromRequest("addhidden");
$addsortorder = App::getFromRequest("addsortorder");
$addoptionname = App::getFromRequest("addoptionname");
if ($manageoptions) {
    ...
    if ($save) {
        ...
        if (!$cid) {
            ...
            $group = Illuminate\Database\Capsule\Manager::table("tblproductconfiggroups")->find($gid);
    ...
    if ($optiontype == "4") {
        echo "<div>Minimum Quantity Required: <input type=\"text\" name=\"qtyminimum\" size=\"6\" value=\"" . $qtyminimum . "\" /> Maximum Allowed: <input type=\"text\" name=\"qtymaximum\" size=\"6\" value=\"" . $qtymaximum . "\" /> (Set to 0 for Unlimited)</div>";
    }
    ...
    if ($action == "duplicate") {
        ...
        $existinggroupid = App::getFromRequest("existinggroupid");
        $newgroupname = App::getFromRequest("newgroupname");
    ...
    if ($action == "") {
        ...
        echo "\n<p>";
        echo AdminLang::trans("configOptions.pageDescription");
        echo "</p>\n\n<p>\n    <div class=\"btn-group\" role=\"group\">\n        <a href=\"";
        echo $_SERVER["PHP_SELF"];
        echo "?action=managegroup\" class=\"btn btn-default\">\n            <i class=\"fas fa-plus\"></i>\n            ";
        echo AdminLang::trans("products.createnewgroup");
        echo "        </a>\n        <a href=\"";
        echo $_SERVER["PHP_SELF"];
        echo "?action=duplicategroup\" class=\"btn btn-default ";
        if (!WHMCS\Product\ConfigOptionGroup::query()->exists()) {
            echo "disabled";
        }
        echo "\">\n            <i class=\"fas fa-plus-square\"></i>\n            ";
        echo AdminLang::trans("configOptions.optionGroupDuplicate");
        echo "        </a>\n    </div>\n</p>\n\n";
