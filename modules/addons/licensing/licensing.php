...
{
    return ["addonLicenseRequired" => true, "addonLicenseName" => "Licensing Addon"];
}

function licensing_config()
{
    $configarray = ["name" => "Licensing Manager", "description" => "License, restrict & distribute your own coding and applications using the same system we use for WHMCS.<br />Find out more & purchase @ <a href=\"http://go.whmcs.com/94/licensing-addon\" target=\"_blank\">www.whmcs.com/addons/licensing-addon</a>", "premium" => true, "version" => "3.1", "author" => "WHMCS", "language" => "english", "fields" => []];
    if (!LICENSINGADDONLICENSE) {
        $configarray["fields"]["license"] = ["FriendlyName" => "License Check Failed", "Type" => "", "Description" => "You need to purchase the licensing addon from <a href=\"http://go.whmcs.com/94/licensing-addon\" target=\"_blank\">www.whmcs.com/addons/licensing-addon</a> before you can use this functionality. If you just purchased it recently, please <a href=\"configaddonmods.php?larefresh=1#licensing\">click here</a> to refresh this message"];
    } else {
        $configarray["fields"] = ["clientverifytool" => ["FriendlyName" => "Public License Verification Tool", "Type" => "yesno", "Description" => "Check to enable the Client Area License Verification Tool (accessed via /index.php?m=licensing)"], "maxreissues" => ["FriendlyName" => "Maximum Allowed Reissues", "Type" => "text", "Size" => "4", "Default" => "10", "Description" => "Enter the maximum number of reissues you want to allow (abuse protection)"], "logprune" => ["FriendlyName" => "Auto Logs Prune", "Type" => "text", "Size" => "4", "Default" => "90", "Description" => "Enter the number of days to keep license access log history for"]];
    }
    return $configarray;
}
...
function licensing_output($vars)
{
    ...
    if (!$action) {
        ...
        if ($action == "list") {
            ...
            $result = select_query("tblproducts", "id,name", ["servertype" => "licensing"], "name", "ASC");
            ...
            if ($_REQUEST["search_licensekey"]) {
                $where["licensekey"] = ["sqltype" => "LIKE", "value" => trim($_REQUEST["search_licensekey"])];
            }
            if ($_REQUEST["search_domain"]) {
                $where["validdomain"] = ["sqltype" => "LIKE", "value" => trim($_REQUEST["search_domain"])];
            }
            if ($_REQUEST["search_ip"]) {
                $where["validip"] = ["sqltype" => "LIKE", "value" => trim($_REQUEST["search_ip"])];
            }
            if ($_REQUEST["search_dir"]) {
                $where["validdirectory"] = ["sqltype" => "LIKE", "value" => trim($_REQUEST["search_dir"])];
            }
            if ($_REQUEST["search_status"]) {
                $where["status"] = $_REQUEST["search_status"];
            }
            $aInt->sortableTableInit("id", "ASC");
            if (!in_array($orderby, ["id", "licensekey", "validdomain", "validip", "lastaccess", "status"])) {
                $orderby = "id";
            }
            ...
            while ($data = mysql_fetch_array($result)) {
                ...
                $tabledata[] = ["<a href=\"" . $uri . "\" target=\"_blank\">" . $licensekey . "</a>", $validdomain, $validip, $lastaccess, $status, "<a href=\"" . $modulelink . "&action=manage&id=" . $id . "\"><img src=\"images/edit.gif\" border=\"0\"></a>"];
            }
            echo $aInt->sortableTable([["licensekey", "License Key"], ["validdomain", "Valid Domains"], ["validip", "Valid IPs"], ["lastaccess", "Last Access"], ["status", "Status"], ""], $tabledata);
        } else {
            if ($action == "manage") {
                if ($_REQUEST["save"]) {
                    update_query("mod_licensing", ["validdomain" => licensing_addon_valid_input_clean($_REQUEST["validdomain"]), "validip" => licensing_addon_valid_input_clean($_REQUEST["validip"]), "validdirectory" => licensing_addon_valid_input_clean($_REQUEST["validdirectory"]), "reissues" => $_REQUEST["reissues"], "status" => $_REQUEST["status"]], ["id" => $id]);
                    redir("module=licensing&action=manage&id=" . $id);
                }
                $result = select_query("mod_licensing", "", ["id" => $id]);
                $data = mysql_fetch_array($result);
                $id = $data["id"];
                if (!$id) {
                ...
                $result = select_query("mod_licensinglog", "", ["licenseid" => $id], "id", "DESC", "0,10");
                while ($data = mysql_fetch_array($result)) {
                    $domain = $data["domain"];
                    $ip = $data["ip"];
                    $path = $data["path"];
                    ...
                    $tabledata[] = [$datetime, $domain, $ip, $path, $message];
                }
                echo $aInt->sortableTable(["Date", "Domain", "IP", "Path", "Result"], $tabledata);
            } else {
                if ($action == "bans") {
                    if ($_REQUEST["save"]) {
                        check_token();
                        if (trim($_REQUEST["banvalue"])) {
                            insert_query("mod_licensingbans", ["value" => trim($_REQUEST["banvalue"]), "notes" => trim($_REQUEST["bannote"])]);
                        }
                        redir("module=licensing&action=bans");
                    }
                    if ($_REQUEST["delete"]) {
                        check_token();
                        delete_query("mod_licensingbans", ["id" => $_REQUEST["delete"]]);
                        redir("module=licensing&action=bans");
                    }
                    ...
                    while ($data = mysql_fetch_array($result)) {
                        ...
                        $value = $data["value"];
                        $notes = $data["notes"];
                        $tabledata[] = [$value, $notes, "<a href=\"#\" onClick=\"doDelete('" . $id . "');return false\"><img src=\"images/delete.gif\" width=\"16\" height=\"16\" border=\"0\" alt=\"" . $aInt->lang("global", "delete") . "\"></a>"];
                    }
                    echo $aInt->sortableTable(["Domain/IP", "Ban Reason/Notes", ""], $tabledata);
                } else {
                    if ($action == "log") {
                        ...
                        if ($_REQUEST["search_domainlog"]) {
                            $where["domain"] = ["sqltype" => "LIKE", "value" => trim($_REQUEST["search_domainlog"])];
                        }
                        if ($_REQUEST["search_iplog"]) {
                            $where["ip"] = ["sqltype" => "LIKE", "value" => trim($_REQUEST["search_iplog"])];
                        }
                        if ($_REQUEST["search_dirlog"]) {
                            $where["path"] = ["sqltype" => "LIKE", "value" => trim($_REQUEST["search_dirlog"])];
                        }
                        if ($_REQUEST["search_message"]) {
                            $where["message"] = ["sqltype" => "LIKE", "value" => trim($_REQUEST["search_message"])];
                        }
                        ...
                        while ($data = mysql_fetch_array($result)) {
                            ...
                            $datetime = $data["datetime"];
                            $datetime = fromMySQLDate($datetime, true);
                            $tabledata2[] = [$datetime, $domain, $ip, $path, $message];
                        }
                        echo $aInt->sortableTable(["Date", "Domain", "IP", "Path", "Status Message"], $tabledata2);
                    }
                }
            }
        }
    ...
    if ($domain) {
        ...
        while ($data = mysql_fetch_array($result)) {
            ...
            if (in_array($domain, $validdomains) || in_array($domain, $validips)) {
                $results[] = ["productname" => $productname, "domain" => $validdomains[0], "ip" => $validips[0], "status" => $status];
            }
        }
    }
    return ["pagetitle" => "License Verification Tool", "breadcrumb" => ["index.php?m=licensing" => "License Verification Tool"], "templatefile" => "licenseverify", "vars" => ["domain" => $domain, "check" => $check, "results" => $results, "ADDONLANG" => $vars["_lang"]]];
}
