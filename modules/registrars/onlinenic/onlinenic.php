...
function onlinenic_getConfigArray()
{
    ...
    $configarray = ["FriendlyName" => ["Type" => "System", "Value" => "OnlineNIC"], "Username" => ["Type" => "text", "Size" => "20", "Description" => "Onlinenic ID"], "Password" => ["Type" => "password", "Size" => "20", "Description" => "Password"], "TestMode" => ["Type" => "yesno"], "SyncNextDueDate" => ["Type" => "yesno", "0" => "Description", "1" => "Check if you want the expiry date sync script to update the expiry and next due dates (cron must be configured)"]];
    return $configarray;
}

function onlinenic_config_validate($params)
{
...
function onlinenic_GetNameservers($params)
{
    ...
    $clTrid = substr(md5($domain), 0, 10) . mt_rand(1000000000, 0);
...
function onlinenic_SaveRegistrarLock($params)
{
    ...
    onlinenic_Logout($fp, $username, $password);
    $resultcode = onlinenic_getResultCode($result);
    $validResultCodes = [1000, 2302];
    if (!in_array($resultcode, $validResultCodes)) {
        $errormsg = onlinenic_getResultText($resultcode);
        $msg = onlinenic_GetValue($result, "<msg>", "</msg>");
        $error = onlinenic_GetValue($result, "<value>", "</value>");
        $error = $msg . " - " . $error;
        ...
        if ($check != "0") {
            $result = update_query("mod_onlinenic", ["lockstatus" => $locked], ["domain" => $domain]);
        } else {
            $result = insert_query("mod_onlinenic", ["lockstatus" => $locked, "domain" => $domain]);
        }
    }
    return $values;
}
...
function onlinenicX_GetDNS($params)
{
    ...
    $hostrecords = [];
    $hostrecords[] = ["hostname" => "ns1", "type" => "A", "address" => "192.168.0.1"];
    $hostrecords[] = ["hostname" => "ns2", "type" => "A", "address" => "192.168.0.2"];
    return $hostrecords;
}
...
function onlinenic_sendCommand($fp, $command, $username = "", $password = "")
{
    ...
    $xmlinput = $xmlinput[2];
    logModuleCall("onlinenic", $xmlinput, $command, $result, "", [$username, $password]);
    return $result;
}
