...
{
    $configarray = ["FriendlyName" => ["Type" => "System", "Value" => "Heart Internet"], "Username" => ["Type" => "text", "Size" => "25", "Description" => "Your Domain Reseller API Username as given at https://customer.heartinternet.co.uk/manage/api"], "Password" => ["Type" => "text", "Size" => "25", "Description" => "Your Domain Reseller API Password as given at https://customer.heartinternet.co.uk/manage/api"], "TestMode" => ["Type" => "yesno", "Description" => "Check to enable test mode"]];
    return $configarray;
}
...
function heartinternet_RegisterDomain($params)
{
    ...
    if (is_array($registrantid)) {
        $values["error"] = "Failed to register domain. Failed to create contact. " . $registrantid["error"];
        return $values;
    }
...
function heartinternet_RenewDomain($params)
{
    ...
    $cltrid = md5(date("YmdHis"));
    $values = [];
    $current_expiry_date = mysql_fetch_assoc(select_query("tbldomains", "expirydate", ["domain" => $params["sld"] . "." . $params["tld"]]));
...
function heartinternet_SaveNameservers($params)
{
    ...
    if ($addnsxml) {
        ...
        $params["Action"] = "savenameserversadd";
    ...
    if ($removensxml) {
        $updatexml = "<?xml version=\"1.0\"?>\n<epp xmlns=\"urn:ietf:params:xml:ns:epp-1.0\" xmlns:ext-domain=\"http://www.heartinternet.co.uk/whapi/ext-domain-1.3\" xmlns:domain=\"urn:ietf:params:xml:ns:domain-1.0\">\n<command>\n<update>\n<domain:update>\n<domain:name>" . $params["sld"] . "." . $params["tld"] . "</domain:name>\n" . $removensxml . "\n</domain:update>\n</update>\n<clTRID>" . $cltrid . "</clTRID>\n</command>\n</epp>";
        $params["Action"] = "savenameserversremove";
        $xmldata = heartinternetreg_curlcall($updatexml, "on", $params);
        if (trim($xmldata["epp"]["response"]["result"]["attr"]["code"]) != "1000") {
            $values["error"] .= $xmldata["epp"]["response"]["result"]["msg"]["value"];
        }
...
function heartinternetreg_curlcall($xml, $verbose = "off", $params)
{
    ...
    try {
        $hi_api->connect(isset($params["TestMode"]) && $params["TestMode"] == "on");
    } catch (Exception $e) {
        $error = $e->getMessage();
        logModuleCall("heartinternet", "connect", "", $error, "", []);
        return ["error" => $error];
    }
    $objects = ["urn:ietf:params:xml:ns:contact-1.0", "urn:ietf:params:xml:ns:domain-1.0", "http://www.heartinternet.co.uk/whapi/null-1.1"];
    $extensions = ["http://www.heartinternet.co.uk/whapi/ext-domain-1.2", "http://www.heartinternet.co.uk/whapi/ext-contact-1.0", "http://www.heartinternet.co.uk/whapi/ext-host-1.0", "http://www.heartinternet.co.uk/whapi/ext-null-1.0", "http://www.heartinternet.co.uk/whapi/ext-whapi-1.0"];
    $mask = [$params["Username"], $params["Password"]];
    $loginXml = "";
    try {
        $loginXml = $hi_api->getLogInRequest($params["Username"], $params["Password"], $objects, $extensions);
        $hi_api->logIn($loginXml);
    } catch (Exception $e) {
        $error = "Failed to log in. " . $e->getMessage();
        logModuleCall("heartinternet", "login", $loginXml, $error, "", $mask);
        return ["error" => $error];
    }
    $data = $hi_api->sendMessage($xml, true);
    $action = $params["Action"] ?? "";
    logModuleCall("heartinternet", $action, $xml, $data, "", []);
    if ($verbose == "on") {
        $result = heartinternetreg_xml2array($data);
    } else {
...
function heartinternetreg_createContact($params)
{
    ...
    if (trim($xmldata["EPP"]["RESPONSE"]["RESULT"]["MSG"]) == "Command completed successfully") {
        return trim($xmldata["EPP"]["RESPONSE"]["RESDATA"]["CONTACT:CREDATA"]["CONTACT:ID"]);
    }
    if (isset($xmldata["EPP"]["RESPONSE"]["RESULT"]["MSG"])) {
        return ["error" => $xmldata["EPP"]["RESPONSE"]["RESULT"]["MSG"]];
    }
