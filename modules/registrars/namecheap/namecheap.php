...
function namecheap_config_validate($params)
{
...
function namecheap_GetNameservers($params)
{
    ...
    $tld = $params["tld"];
    $sld = $params["sld"];
    if (!in_array(get_query_val("tbldomains", "status", ["id" => $params["domainid"]]), ["Active", "Expired"])) {
        return ["error" => "Unable to obtain Nameservers for an unregistered domain"];
    }
    $response = "";
    $result = $request_params = $values = [];
    $oIDNA = new NamecheapRegistrarIDNA($sld, $tld);
    $sld = $oIDNA->getEncodedSld();
    try {
        $request_params = ["SLD" => $sld, "TLD" => $tld];
        $api = new NamecheapRegistrarApi($username, $password, $testmode);
        ...
        logModuleCall("namecheap", "GetNameservers", ["command" => "namecheap.domains.dns.getList"] + $request_params, $response, $result, [$password]);
    }
    return $values;
}
...
function namecheap_SaveNameservers($params)
{
    ...
    $result = $request_params = $values = [];
    $defaultNs = true;
    $defaultNsServers = ["dns1.registrar-servers.com", "dns2.registrar-servers.com", "dns3.registrar-servers.com", "dns4.registrar-servers.com", "dns5.registrar-servers.com"];
    ...
    try {
        ...
        $values["error"] = "An error occurred: " . $e->getMessage();
        logModuleCall("namecheap", "SetNameservers", ["command" => "namecheap.domains.dns.setCustom"] + $request_params, $response, $result, [$password]);
    }
    return $values;
}
...
function namecheap_GetRegistrarLock($params)
{
    ...
    $result = $request_params = $values = [];
    try {
        $request_params = ["DomainName" => $sld . "." . $tld];
        $api = new NamecheapRegistrarApi($username, $password, $testmode);
        ...
        $values["error"] = "An error occurred: " . $e->getMessage();
        logModuleCall("namecheap", "GetRegistrarLock", ["command" => "namecheap.domains.getRegistrarLock"] + $request_params, $response, $result, [$password]);
    }
    return $values;
}
...
function namecheap_SaveRegistrarLock($params)
{
    ...
    $result = $request_params = $values = [];
    try {
        $request_params = ["DomainName" => $sld . "." . $tld, "LockAction" => "locked" == $params["lockenabled"] ? "lock" : "unlock"];
        $api = new NamecheapRegistrarApi($username, $password, $testmode);
        $response = $api->request("namecheap.domains.setRegistrarLock", $request_params);
        $result = $api->parseResponse($response);
    } catch (Exception $e) {
        $rl_unable_domains = ["ca", "cm", "co.uk", "org.uk", "me.uk", "de", "eu", "ws", "uk"];
        $msg = $e->getMessage();
        ...
        logModuleCall("namecheap", "SaveRegistrarLock", ["command" => "namecheap.domains.setRegistrarLock"] + $request_params, $response, $result, [$password]);
    }
    return $values;
}
...
function namecheap_GetEmailForwarding($params)
{
    ...
    try {
        ...
        foreach ($forward as $v) {
            $values[] = ["prefix" => $v["@attributes"]["mailbox"], "forwardto" => $v["@value"]];
        }
    } catch (Exception $e) {
        $values["error"] = "An error occurred: " . $e->getMessage();
        logModuleCall("namecheap", "GetEmailForwarding", ["command" => "namecheap.domains.dns.getEmailForwarding"] + $request_params, $response, $result, [$password]);
    }
    return $values;
}
...
function namecheap_SaveEmailForwarding($params)
{
    ...
    try {
        ...
        $values["error"] = "An error occurred: " . $e->getMessage();
        logModuleCall("namecheap", "SaveEmailForwarding", ["command" => "namecheap.domains.dns.setEmailForwarding"] + $request_params, $response, $result, [$password]);
    }
    return $values;
}
...
function namecheap_GetDNS($params)
{
    ...
    try {
        ...
        foreach ($host as $v) {
            $values[] = ["hostname" => $v["@attributes"]["Name"], "type" => $v["@attributes"]["Type"], "address" => $v["@attributes"]["Address"]];
        }
    } catch (Exception $e) {
        $values["error"] = "An error occurred: " . $e->getMessage();
        logModuleCall("namecheap", "GetDNS", ["command" => "namecheap.domains.dns.getHosts"] + $request_params, $response, $result, [$password]);
    }
    return $values;
}
...
function namecheap_SaveDNS($params)
{
    ...
    try {
        ...
        $values["error"] = "An error occurred: " . $e->getMessage();
        logModuleCall("namecheap", "SaveDNS", ["command" => "namecheap.domains.dns.setHosts"] + $request_params, $response, $result, [$password]);
    }
    return $values;
}
...
function namecheap_RegisterDomain($params)
{
    ...
    try {
        ...
        $request_params = ["DomainName" => $sld . "." . $tld, "Years" => $params["regperiod"], "Nameservers" => implode(",", $nameservers)];
        ...
        $wg_ex = ["bz", "ca", "cn", "co.uk", "de", "eu", "in", "me.uk", "mobi", "nu", "org.uk", "us", "ws", "uk"];
        ...
        if ("eu" == strtolower($tld)) {
            ...
            $langs = ["BG" => "Bulgaria", "CS" => "Czech", "DS" => "Danish", "NL" => "Dutch", "EN" => "English", "ET" => "Estonian", "FI" => "Finnish", "FR" => "French", "DE" => "German", "EL" => "Greek", "HL" => "Hungarian", "IT" => "Italian", "LV" => "Latvian", "LI" => "Lithuanian", "MT" => "Maltese", "PL" => "Polish", "PT" => "Portuguese", "RO" => "Romania", "SK" => "Slovak", "SL" => "Slovenian", "ES" => "Spanish", "SV" => "Swedish"];
        ...
        $api = new self($username, $password, $testmode);
        ...
        $values["error"] = "An error occurred: " . $e->getMessage();
        logModuleCall("namecheap", "RegisterDomain", ["command" => "namecheap.domains.create"] + $request_params, $response, $result, [$password]);
    }
    return $values;
}
...
function namecheap_TransferDomain($params)
{
    ...
    $result = $request_params = $values = [];
    try {
        $request_params = ["DomainName" => $sld . "." . $tld, "Years" => $params["regperiod"], "EPPCode" => $params["transfersecret"]];
        if (!empty($params["PromotionCode"])) {
            $request_params["PromotionCode"] = $params["PromotionCode"];
        }
        ...
        $values["error"] = "An error occurred: " . $e->getMessage();
        logModuleCall("namecheap", "TransferDomain", ["command" => "namecheap.domains.transfer.create"] + $request_params, $response, $result, [$password]);
    }
    return $values;
}
...
function namecheap_RenewDomain($params)
{
    ...
    $exCode = 0;
    try {
        $request_params = ["DomainName" => $sld . "." . $tld, "Years" => $params["regperiod"]];
        if (!empty($params["PromotionCode"])) {
            $request_params["PromotionCode"] = $params["PromotionCode"];
        }
        $api = new NamecheapRegistrarApi($username, $password, $testmode);
        ...
        $exCode = $e->getCode();
        $values["error"] = "An error occurred: " . $e->getMessage();
        logModuleCall("namecheap", "RenewDomain", ["command" => "namecheap.domains.renew"] + $request_params, $response, $result, [$password]);
    }
    if ($exCode != 2020166) {
        return $values;
    }
    try {
        ...
        $values["error"] = "An error occurred: " . $e->getMessage();
        logModuleCall("namecheap", "ReactivateDomain", ["command" => "namecheap.domains.reactivate"] + $request_params, $response, $result, [$password]);
    }
    return $values;
}
...
function namecheap_GetContactDetails($params)
{
    ...
    try {
        ...
        foreach ($result["DomainContactsResult"] as $k => $v) {
            if (in_array($k, ["Registrant", "Admin", "Tech", "AuxBilling"])) {
        ...
        $values["error"] = "An error occurred: " . $e->getMessage();
        logModuleCall("namecheap", "GetContactDetails", ["command" => "namecheap.domains.getContacts"] + $request_params, $response, $result, [$password]);
    }
    return $values;
}
...
function namecheap_SaveContactDetails($params)
{
    ...
    try {
        ...
        $values["error"] = "An error occurred: " . $e->getMessage();
        logModuleCall("namecheap", "SaveContactDetails", ["command" => "namecheap.domains.setContacts"] + $request_params, $response, $result, [$password]);
    }
    return $values;
}
...
function namecheap_RegisterNameserver($params)
{
    ...
    $result = $request_params = $values = [];
    try {
        $request_params = ["SLD" => $sld, "TLD" => $tld, "Nameserver" => $params["nameserver"], "IP" => $params["ipaddress"]];
        $api = new NamecheapRegistrarApi($username, $password, $testmode);
        $response = $api->request("namecheap.domains.ns.create", $request_params);
        $result = $api->parseResponse($response);
    } catch (Exception $e) {
        $values["error"] = "An error occurred: " . $e->getMessage();
        logModuleCall("namecheap", "RegisterNameserver", ["command" => "namecheap.domains.ns.create"] + $request_params, $response, $result, [$password]);
    }
    return $values;
}
...
function namecheap_ModifyNameserver($params)
{
    ...
    $result = $request_params = $values = [];
    try {
        $request_params = ["SLD" => $sld, "TLD" => $tld, "Nameserver" => $params["nameserver"], "IP" => $params["newipaddress"], "OldIP" => $params["currentipaddress"]];
        $api = new NamecheapRegistrarApi($username, $password, $testmode);
        $response = $api->request("namecheap.domains.ns.update", $request_params);
        $result = $api->parseResponse($response);
    } catch (Exception $e) {
        $values["error"] = "An error occurred: " . $e->getMessage();
        logModuleCall("namecheap", "ModifyNameserver", ["command" => "namecheap.domains.ns.update"] + $request_params, $response, $result, [$password]);
    }
    return $values;
}
...
function namecheap_DeleteNameserver($params)
{
    ...
    $result = $request_params = $values = [];
    try {
        $request_params = ["SLD" => $sld, "TLD" => $tld, "Nameserver" => $params["nameserver"]];
        $api = new NamecheapRegistrarApi($username, $password, $testmode);
        $response = $api->request("namecheap.domains.ns.delete", $request_params);
        $result = $api->parseResponse($response);
    } catch (Exception $e) {
        $values["error"] = "An error occurred: " . $e->getMessage();
        logModuleCall("namecheap", "DeleteNameserver", ["command" => "namecheap.domains.ns.delete"] + $request_params, $response, $result, [$password]);
    }
    return $values;
}
...
function namecheap_Sync($params)
{
    ...
    $values = [];
    try {
        $request_params = ["ListType" => "ALL", "Page" => 1, "PageSize" => 10, "SortBy" => "NAME", "SearchTerm" => $sld . "." . $tld];
        if (!empty($params["PromotionCode"])) {
            $request_params["PromotionCode"] = $params["PromotionCode"];
        }
        $api = new NamecheapRegistrarApi($username, $password, $testmode);
...
function namecheap_TransferSync($params)
{
    ...
    $sld = $oIDNA->getEncodedSld();
    try {
        $request_params = ["ListType" => "ALL", "Page" => 1, "PageSize" => 10, "SortBy" => "DOMAINNAME", "SearchTerm" => $sld . "." . $tld];
        if (!empty($params["PromotionCode"])) {
            $request_params["PromotionCode"] = $params["PromotionCode"];
        }
        $api = new NamecheapRegistrarApi($username, $password, $testmode);
