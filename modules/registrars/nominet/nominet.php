...
{
    $configarray = ["Description" => ["Type" => "System", "Value" => "The Official UK Domain Registry Module"], "Username" => ["Type" => "text", "Size" => "25", "Description" => ""], "Password" => ["Type" => "password", "Size" => "25", "Description" => ""], "TestMode" => ["Type" => "yesno"], "AllowClientTAGChange" => ["Type" => "yesno", "Description" => "Check to allow clients to change TAGs on domains"], "DeleteOnTransfer" => ["Type" => "yesno", "Description" => "Check if you want the domain to be deleted entirely on RELEASE. Not Recommended - This option may be removed in a future release."]];
    return $configarray;
}

function nominet_config_validate($params)
{
    ...
    if ($nominet->connectAndLogin()) {
        ...
        if ($success) {
            if ($nominet->isErrorCode()) {
                return ["error" => $nominet->getErrorDesc()];
            }
            $x = 1;
            $values = [];
            ...
            return ["error" => $nominet->getLastError()];
        }
    } else {
        return ["error" => $nominet->getLastError()];
    }
...
function nominet_SaveNameservers($params)
{
    ...
    if ($nominet->connectAndLogin()) {
        ...
        $domainHostFormat = "<domain:hostObj>%s</domain:hostObj>\n";
        if (0 < strlen($params["ns2"])) {
            $ns[2] = $params["ns2"];
            $xml .= sprintf($domainHostFormat, $nominet->escapeParam($params["ns2"]));
        }
        if ($params["ns3"]) {
            $ns[3] = $params["ns3"];
                                            ...
                                            if ($LegalType == "UK Corporation by Royal Charter") {
                                                ...
                                                if ($LegalType == "UK Statutory Body") {
                                                    ...
                                                    if ($LegalType == "UK Entity (other)") {
                                                        ...
                                                        if (in_array($LegalType, ["Non-UK Individual (representing self)", "Non-UK Individual"])) {
                                                            $LegalTypeID = "FIND";
                                                        } else {
...
function nominet_RegisterDomain($params)
{
    ...
    if ($nominet->connectAndLogin()) {
        ...
        if (!trim($RegistrantName)) {
            return ["error" => "Registrant Name is missing. Please check the contact fields on the domains tab."];
        }
        ...
        if (!$LegalTypeID) {
            return ["error" => "Legal Type is missing. Please check field on domains tab"];
        }
        if ($LegalTypeID != "IND") {
            $WhoisOptOut = "";
        }
...
function nominet_TransferDomain($params)
{
    ...
    if ($nominet->connectAndLogin()) {
        ...
        if ($success) {
            ...
            return ["error" => "Domain already exists at domain registrar"];
        }
        return ["error" => $nominet->getLastError()];
    }
    return ["error" => $nominet->getLastError()];
...
function nominet_RenewDomain($params)
{
    $nominet = WHMCS_Nominet::init($params);
    if ($nominet->connectAndLogin()) {
        $expiry = get_query_val("tbldomains", "expirydate", ["id" => $params["domainid"]]);
    ...
    if ($nominet->connectAndLogin()) {
        ...
        if ($success) {
            ...
            if ($nominet->getResultCode() == 1000 && $params["DeleteOnTransfer"]) {
                delete_query("tbldomains", ["id" => $params["domainid"]]);
                return ["deleted" => true];
            }
        } else {
            return ["error" => $nominet->getLastError()];
        ...
        if ($nominet->isErrorCode()) {
            if ($nominet->getResultCode() == 2302) {
                ...
                if (10 < $params["contactCreateCount"]) {
                    return ["error" => "Failed to create contact. Please contact support."];
                }
...
function nominet_createHost($nominet, $ns = [])
{
