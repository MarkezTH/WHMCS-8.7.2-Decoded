...
{
    $configarray = ["Username" => ["Type" => "text", "Size" => "20", "Description" => "Enter your username here"], "Password" => ["Type" => "password", "Size" => "20", "Description" => "Enter your password here"], "TestMode" => ["Type" => "yesno"]];
    return $configarray;
}
...
function bizcn_GetNameservers($params)
{
    ...
    $tld = $params["tld"];
    $sld = $params["sld"];
    $_params = ["username" => $username, "password" => md5($password), "testmode" => $testmode, "module" => "getdomaindns", "domainname" => $sld . "." . $tld];
...
function bizcn_SaveNameservers($params)
{
    ...
    $_params = ["username" => $username, "password" => md5($password), "testmode" => $testmode, "module" => "moddomaindns", "domainname" => $sld . "." . $tld, "dns_host" => $dnsHost];
    $result = com_71_call($_params, $testmode);
    if (empty($result)) {
        $error = "return is null";
        $values["error"] = $error;
...
function bizcn_GetRegistrarLock($params)
{
    ...
    $tld = $params["tld"];
    $sld = $params["sld"];
    $_params = ["username" => $username, "password" => md5($password), "testmode" => $testmode, "module" => "getdomainlock", "domainname" => $sld . "." . $tld];
    $result = com_71_call($_params, $testmode);
    if (empty($result)) {
        return false;
    }
...
function bizcn_SaveRegistrarLock($params)
{
    ...
    $domainname = $sld . "." . $tld;
    $_params = ["username" => $username, "password" => md5($password), "testmode" => $testmode, "module" => $lockstatus, "domainname" => $domainname];
    $result = com_71_call($_params, $testmode);
    if (empty($result)) {
        $error = "return is null";
    }
...
function bizcn_GetDNS($params)
{
    ...
    $tld = $params["tld"];
    $sld = $params["sld"];
    $_params = ["username" => $username, "password" => md5($password), "testmode" => $testmode, "module" => "getdnsrecord", "domainname" => $sld . "." . $tld];
    $result = com_71_call($_params, $testmode);
    if (empty($result)) {
        return false;
    }
    if (substr($result, 0, 3) == 200) {
        ...
        foreach ($_result as $_key => $_value) {
            ...
            if (0 < $_key) {
                $hostrecords[] = ["hostname" => $_tmpNS[0], "type" => $_tmpNS[1], "address" => $_tmpNS[2]];
            }
        }
        return $hostrecords;
    } else {
...
function bizcn_SaveDNS($params)
{
    ...
    if (0 < count($trimDnsData)) {
        foreach ($trimDnsData as $_key3 => $_value3) {
            ...
            if ($func != "NULL") {
                if ($func == "ADD") {
                    $_params = ["username" => $username, "password" => md5($password), "module" => "adddnsrecord", "domainname" => $sld . "." . $tld, "resolvetype" => $_value3["type"], "resolvehost" => $_value3["hostname"], "resolvevalue" => $_value3["address"], "mxlevel" => 10];
                    $result = com_71_call($_params, $testmode);
                    if (empty($result)) {
                        $error = "return is null";
                        $values["error"] = $error;
                ...
                if ($func == "MOD") {
                    $_params = ["username" => $username, "password" => md5($password), "module" => "moddnsrecord", "domainname" => $sld . "." . $tld, "resolvetype" => $_value3["type"], "resolvehost" => $_value3["hostname"], "resolveoldvalue" => $_value3["oldAddress"], "resolvevalue" => $_value3["address"], "mxlevel" => 10];
                    $result = com_71_call($_params, $testmode);
                    if (empty($result)) {
                        $error = "return is null";
                        $values["error"] = $error;
                ...
                if ($func == "DEL") {
                    $_params = ["username" => $username, "password" => md5($password), "module" => "deldnsrecord", "domainname" => $sld . "." . $tld, "resolvetype" => $_value3["type"], "resolvehost" => $_value3["hostname"], "resolveoldvalue" => $_value3["address"], "mxlevel" => 10];
                    $result = com_71_call($_params, $testmode);
                    if (empty($result)) {
                        $error = "return is null";
                        $values["error"] = $error;
...
function bizcn_RenewDomain($params)
{
    ...
    $sld = $params["sld"];
    $regperiod = $params["regperiod"];
    $_params = ["username" => $username, "password" => md5($password), "module" => "renewdomain", "domain" => $sld . "." . $tld, "term" => $regperiod];
    $result = com_71_call($_params, $testmode);
    if (substr($result, 0, 3) == 200) {
        return true;
    }
...
function bizcn_GetContactDetails($params)
{
    ...
    $tld = $params["tld"];
    $sld = $params["sld"];
    $_params = ["username" => $username, "password" => md5($password), "testmode" => $testmode, "module" => "getcontactdetails", "domainname" => $sld . "." . $tld];
    $result = com_71_call($_params, $testmode);
    if (empty($result)) {
...
function bizcn_RegisterNameserver($params)
{
    ...
    $_params = ["username" => $username, "password" => md5($password), "testmode" => $testmode, "tld" => $tld, "sld" => $sld, "module" => "createnameserver", "hostname" => $nameserver, "ip" => $ipaddress];
    $result = com_71_call($_params, $testmode);
    if (empty($result)) {
        $error = "return is null";
        $values["error"] = $error;
...
function bizcn_ModifyNameserver($params)
{
    ...
    $_params = ["username" => $username, "password" => md5($password), "testmode" => $testmode, "tld" => $tld, "sld" => $sld, "module" => "modnameserver", "hostname" => $nameserver, "oldip" => $currentipaddress, "newip" => $newipaddress];
    $result = com_71_call($_params, $testmode);
    if (empty($result)) {
        $error = "return is null";
        $values["error"] = $error;
...
function bizcn_DeleteNameserver($params)
{
    ...
    $sld = $params["sld"];
    $nameserver = $params["nameserver"];
    $_params = ["username" => $username, "password" => md5($password), "testmode" => $testmode, "tld" => $tld, "sld" => $sld, "module" => "delnameserver", "hostname" => $nameserver];
    $result = com_71_call($_params, $testmode);
    if (empty($result)) {
        $error = "return is null";
        $values["error"] = $error;
...
function com_71_call($params, $testModel = false)
{
    ...
    $ret = curl_exec($ch);
    $globalMessage = $ret;
    logModuleCall("bizcn", $params["module"], $params, $ret, "", [$params["username"], $params["password"]]);
    return $ret;
}
