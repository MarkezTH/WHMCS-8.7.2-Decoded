...
function m101domain_getConfigArray()
{
    if (!class_exists("\\DOMDocument")) {
        return ["error" => ["Description" => "Error: The DOM PHP Extension needs to be installed to use 101domain"]];
    }
    $configuration = ["FriendlyName" => ["Type" => "System", "Value" => "101 Domain"], "Username" => ["Type" => "text", "Size" => "20", "Description" => "Enter your username here"], "Password" => ["Type" => "password", "Size" => "20", "Description" => "Enter your password here"], "ApiUrl" => ["Type" => "text", "Size" => "50", "Description" => "Customize the API URL here if required", "Default" => "https://api.101domain.com/epp.xml"], "DisableValidation" => ["Type" => "yesno", "Description" => "Check to disable SSL Validation if you are receiving SSL related errors"]];
...
function m101domain_SaveDNS($params)
{
    ...
    foreach ($params["dnsrecords"] as &$values) {
        if ($values["type"] == "MXE") {
            $values["type"] = "MX";
            $aRecord = ["hostname" => "mailserver", "type" => "A", "address" => $values["address"]];
...
function m101domain_RegisterDomain($params)
{
    try {
        ...
        $check_contacts = m101domain_eppCheckContact($params, [$registrant_handle, $admin_handle, $tech_handle, $bill_handle]);
        ...
        if ($create_res->error_code == 1001) {
            return ["error" => "Domain was submitted for registration and is pending approval"];
        }
        return ["error" => ""];
    } catch (M101Domain\Exception\Error $e) {
        $values = ["error" => $e->getMessage()];
...
function m101domain_eppDomainCreateExt(&$params, $registrant)
{
    M101Domain\DomainFields::init();
    $xml = "";
    $tld = strtolower("." . ltrim($params["tld"], "."));
    $fields = !empty(M101Domain\DomainFields::$m101domain_fieldconfig[$tld]) ? M101Domain\DomainFields::$m101domain_fieldconfig[$tld] : [];
    if (!empty($fields)) {
        $first = reset($fields);
        if ($first["_ext_type"] == "simple") {
            ...
            foreach ($fields as &$field) {
                Log::debug("epp reg", [$field, $params]);
            ...
            if ($first["_ext_type"] == "hk") {
                ...
                if ($vars["org_type"] == "org") {
                    ...
                    $org_fields = ["org_doc_type" => "documentationType", "org_chinese" => "chineseOrg", "org_doc_country" => "issuingCountry", "org_doc_number" => "documentNumber", "org_industry_type" => "industryType"];
                } else {
                    $org_node = "individual";
                    $org_fields = ["ind_doc_type" => "documentationType", "ind_doc_country" => "issuingCountry", "ind_doc_number" => "documentNumber", "ind_under18" => "under18"];
                }
                ...
                if ($first["_ext_type"] == "ru") {
                    ...
                    if ($vars["org_type"] == "ORG") {
                        $org_node = "organization";
                        $org_fields = ["taxpayerNumber" => "taxpayerNumber", "taxpayerNumber2" => "taxpayerNumber"];
                    } else {
                        $org_node = "individual";
                        $org_fields = ["birthday" => "birthday", "passportNumber" => "passportNumber", "passportIssuer" => "passportIssuer", "passportIssueDate" => "passportIssueDate", "whoisPP" => "whoisPP"];
                    }
                    $xml .= sprintf("<%s:%s>", $pfx, $org_node);
                    foreach ($org_fields as $var_name => $node_name) {
                        $xml .= sprintf("<%s:%s>%s</%s:%s>", $pfx, $node_name, $vars[$var_name], $pfx, $node_name);
                    ...
                    if ($first["_ext_type"] == "travel") {
                        ...
                        if ($first["_ext_type"] == "cn") {
                            ...
                            if ($vars["org_type"] == "org") {
                                $org_node = "organization";
                                $org_fields = ["industry" => "industry", "documentType" => "documentType", "organizationNumber" => "organizationNumber", "managerIDType" => "managerIDType", "managerIDNumber" => "managerIDNumber"];
                            } else {
                                $org_node = "individual";
                                $org_fields = ["idNumber" => "idNumber", "idType" => "idType"];
                            }
                            $xml .= sprintf("<%s:%s>", $pfx, $org_node);
                            foreach ($org_fields as $var_name => $node_name) {
                                $xml .= sprintf("<%s:%s>%s</%s:%s>", $pfx, $node_name, $vars[$var_name], $pfx, $node_name);
...
function m101domain_addFieldGetVal(&$field, &$params)
{
...
function m101domain_GetContactDetails($params)
{
    ...
    try {
        ...
        $contacts = array_merge(["registrant" => $domain_info->registrant], $domain_info->contacts);
        $values = [];
...
function m101domain_SaveContactDetails($params)
{
    try {
        list($registrant_handle, $admin_handle, $tech_handle, $bill_handle) = m101domain_contacthandles($params);
        $handles = ["registrant" => $registrant_handle, "admin" => $admin_handle, "tech" => $tech_handle, "billing" => $bill_handle];
...
function m101domain_Sync($params)
{
    ...
    try {
        ...
        $values = ["expirydate" => date("Y-m-d", $info->ex_date), "active" => true, "expired" => false];
...
function m101domain_eppCheckContact($params, $handles)
{
...
function m101domain_connect($params)
{
    ...
    throw new M101Domain\Exception\Error(sprintf("%s (%d) %s", $result->error_code, $result->error_message, implode(", ", $result->reason)));
}
...
function m101domain_command($params, $xml, $command = "")
{
    ...
    if ($m101domain_session_id) {
        curl_setopt($ch, CURLOPT_HTTPHEADER, ["Cookie: EPPSESSION=" . $m101domain_session_id, "Expect:"]);
    } else {
        curl_setopt($ch, CURLOPT_HTTPHEADER, ["Expect:"]);
    }
    $result = curl_exec($ch);
    logModuleCall("m101domain", $command, $xml, $result, "", []);
    if ($result === false) {
        $res->error_message = "Failed to connect: " . curl_error($ch);
        $res->error_code = 2400;
        throw new M101Domain\Exception\Error(sprintf("%s (%d) %s", $res->error_message, $res->error_code, implode(", ", $res->reason)), $res->error_code);
    }
                                ...
                                if ($postal instanceof DOMElement && $postal->namespaceURI == "urn:ietf:params:xml:ns:contact-1.0") {
                                    switch ($postal->localName) {
                                        case "name":
                                            ...
                                            $contact->first_name = implode(" ", $spl);
                                            break;
                                        case "org":
                                            $postal->normalize();
...
function m101domain_contacthandles($params)
{
    ...
    return [$registrant_handle, $admin_handle, $tech_handle, $bill_handle];
}
