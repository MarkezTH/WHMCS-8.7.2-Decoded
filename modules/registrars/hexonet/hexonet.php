...
{
    $configarray = ["FriendlyName" => ["Type" => "System", "Value" => "HEXONET"], "Description" => ["Type" => "System", "Value" => "Don't have a HEXONET Account yet? Get one here: <a target=\"_blank\" href=\"//go.whmcs.com/1405/hexonet-signup\">www.hexonet.net/sign-up</a>"], "Username" => ["Type" => "text", "Size" => "20", "Description" => "Enter your HEXONET Login ID"], "Password" => ["Type" => "password", "Size" => "20", "Description" => "Enter your HEXONET Password "], "TestMode" => ["Type" => "yesno", "Description" => "Connect to OT&amp;E (Test Environment)"], "ProxyServer" => ["Type" => "text", "Description" => "Optional (HTTP(S) Proxy Server)"], "ConvertIDNs" => ["Type" => "dropdown", "Options" => "API,PHP", "Default" => "API", "Description" => "Use API or PHP function (idn_to_ascii)"], "DNSSEC" => ["Type" => "yesno", "Description" => "Display the DNSSEC Management functionality in the domain details"], "TRANSFERLOCK" => ["Type" => "yesno", "Description" => "Locks automatically a domain after a new registration"]];
    if (!function_exists("idn_to_ascii")) {
        $configarray["ConvertIDNs"] = ["Type" => "dropdown", "Options" => "API", "Default" => "API", "Description" => "Use API (PHP function idn_to_ascii not available)"];
    }
    return $configarray;
}
...
function hexonet_CheckAvailability($params)
{
    ...
    $commandCall = ["COMMAND" => "CheckDomains", "DOMAIN" => $domains, "PREMIUMCHANNELS" => "*"];
                    ...
                    if ($domainPremiumClass || $domainPremiumChannel) {
                        if ($params["premiumEnabled"]) {
                            if ($isPremium || $isReserved) {
                                ...
                                $premiumPricing = ["register" => $domainPrice, "renew" => $domainRenew, "CurrencyCode" => $priceCurrency];
...
function hexonet_GetDomainSuggestions($params)
{
    ...
    $command = ["COMMAND" => "QueryDomainSuggestionList", "KEYWORD" => $label, "ZONE" => $zones, "SOURCE" => "ISPAPI-SUGGESTIONS"];
...
{
    return ["suggestions" => ["FriendlyName" => AdminLang::trans("general.searchTerm"), "Type" => "yesno", "Description" => AdminLang::trans("global.ticktoenable") . " (" . AdminLang::trans("global.recommended") . ")", "Default" => true]];
}
...
function hexonet_getRenewPrice($params, $class, $cur_id, $tld)
{
    ...
    if (empty($class)) {
        ...
        if (!empty($dataRenewPrice) && !in_array($dataRenewPrice->msetupfee, ["-1", "0"])) {
...
function hexonet_getUserRelations($params)
{
    ...
    if (!isset($_SESSION["HEXONETCACHE"]) || $_SESSION["HEXONETCACHE"]["TIMESTAMP"] + 600 < $date->getTimestamp()) {
        ...
        if ($response["CODE"] == 200) {
            $_SESSION["HEXONETCACHE"] = ["TIMESTAMP" => $date->getTimestamp(), "RELATIONS" => $response["PROPERTY"]];
...
function hexonet_dnssec($params)
{
    ...
    if (App::isInRequest("submit") && App::getFromRequest("submit") == 1) {
        check_token();
        $command = ["COMMAND" => "ModifyDomain", "DOMAIN" => $domain, "SECDNS-MAXSIGLIFE" => $_POST["MAXSIGLIFE"]];
        $command["SECDNS-DS"] = [];
        ...
        if (empty($command["SECDNS-DS"])) {
            ...
            $secdnsds = [];
            $command2 = ["COMMAND" => "StatusDomain", "DOMAIN" => $domain];
    ...
    $secdnskey = [];
    $maxsiglife = "";
    $command = ["COMMAND" => "StatusDomain", "DOMAIN" => $domain];
    $response = hexonet_call($command, hexonet_config($params));
    if ($response["CODE"] == 200) {
        $maxsiglife = isset($response["PROPERTY"]["SECDNS-MAXSIGLIFE"]) ? $response["PROPERTY"]["SECDNS-MAXSIGLIFE"][0] : "";
        $secdnsds = isset($response["PROPERTY"]["SECDNS-DS"]) ? $response["PROPERTY"]["SECDNS-DS"] : [];
        $secdnskey = isset($response["PROPERTY"]["SECDNS-KEY"]) ? $response["PROPERTY"]["SECDNS-KEY"] : [];
        $secdnskeynew = [];
    ...
    foreach ($secdnsds as $ds) {
        ...
        array_push($secdnsds_newformat, ["keytag" => $keytag, "alg" => $alg, "digesttype" => $digesttype, "digest" => $digest]);
    }
    $secdnskey_newformat = [];
    foreach ($secdnskey as $key) {
        list($flags, $protocol, $alg, $pubkey) = preg_split("/\\s+/", $key);
        array_push($secdnskey_newformat, ["flags" => $flags, "protocol" => $protocol, "alg" => $alg, "pubkey" => $pubkey]);
    }
    return ["templatefile" => "dnssec", "vars" => ["error" => $error, "successful" => $successful, "secdnsds" => $secdnsds_newformat, "secdnskey" => $secdnskey_newformat, "maxsiglife" => $maxsiglife]];
}
...
function hexonet_SaveRegistrarLock($params)
{
    ...
    $params = $params["original"];
    $domain = $params["sld"] . "." . $params["tld"];
    $command = ["COMMAND" => "ModifyDomain", "DOMAIN" => $domain, "TRANSFERLOCK" => $params["lockenabled"] == "locked" ? "1" : "0"];
    $response = hexonet_call($command, hexonet_config($params));
    if ($response["CODE"] != 200) {
        $values["error"] = $response["DESCRIPTION"];
    }
...
function hexonet_GetEPPCode($params)
{
    ...
    $domain = $params["sld"] . "." . $params["tld"];
    if ($params["tld"] == "de") {
        $command = ["COMMAND" => "DENIC_CreateAuthInfo1", "DOMAIN" => $domain];
        hexonet_call($command, hexonet_config($params));
    }
    if ($params["tld"] == "eu") {
        $command = ["COMMAND" => "RequestDomainAuthInfo", "DOMAIN" => $domain];
        $response = hexonet_call($command, hexonet_config($params));
    } else {
        $command = ["COMMAND" => "StatusDomain", "DOMAIN" => $domain];
        $response = hexonet_call($command, hexonet_config($params));
...
function hexonet_SaveNameservers($params)
{
    ...
    $params = $params["original"];
    $domain = $params["sld"] . "." . $params["tld"];
    $command = ["COMMAND" => "ModifyDomain", "DOMAIN" => $domain, "NAMESERVER" => [$params["ns1"], $params["ns2"], $params["ns3"], $params["ns4"], $params["ns5"]], "INTERNALDNS" => 1];
    $response = hexonet_call($command, hexonet_config($params));
    if ($response["CODE"] != 200) {
        $values["error"] = $response["DESCRIPTION"];
    }
...
function hexonet_GetDNS($params)
{
    ...
    $domain = $params["sld"] . "." . $params["tld"];
    $command = ["COMMAND" => "ConvertIDN", "DOMAIN" => $domain];
    $response = hexonet_call($command, hexonet_config($params));
    $dnszone_idn = $response["PROPERTY"]["ACE"][0] . ".";
    $command = ["COMMAND" => "QueryDNSZoneRRList", "DNSZONE" => $dnszone, "EXTENDED" => 1];
    $response = hexonet_call($command, hexonet_config($params));
    $hostrecords = [];
    if ($response["CODE"] == 200) {
        $i = 0;
        foreach ($response["PROPERTY"]["RR"] as $rr) {
            ...
            if ($rrtype == "A") {
                $hostrecords[$i] = ["hostname" => $domain, "type" => $rrtype, "address" => $fields[0]];
            ...
            if ($rrtype == "AAAA") {
                $hostrecords[$i] = ["hostname" => $domain, "type" => "AAAA", "address" => $fields[0]];
                $i++;
            }
            if ($rrtype == "TXT") {
                $hostrecords[$i] = ["hostname" => $domain, "type" => $rrtype, "address" => implode(" ", $fields)];
                $i++;
            }
            if ($rrtype == "SRV") {
                $priority = array_shift($fields);
                $hostrecords[$i] = ["hostname" => $domain, "type" => $rrtype, "address" => implode(" ", $fields), "priority" => $priority];
                $i++;
            }
            if ($rrtype == "CNAME") {
                $hostrecords[$i] = ["hostname" => $domain, "type" => $rrtype, "address" => $fields[0]];
            ...
            if ($rrtype == "X-HTTP") {
                ...
                $hostrecords[$i] = ["hostname" => $domain, "type" => $url_type, "address" => implode(" ", $fields)];
                $i++;
            }
        }
        $command = ["COMMAND" => "QueryDNSZoneRRList", "DNSZONE" => $dnszone, "SHORT" => 1];
        $response = hexonet_call($command, hexonet_config($params));
        if ($response["CODE"] == 200) {
            foreach ($response["PROPERTY"]["RR"] as $rr) {
                $fields = explode(" ", $rr);
                ...
                if ($rrtype == "MX") {
                    if (preg_match("/^mxe-host-for-ip-(\\d+)-(\\d+)-(\\d+)-(\\d+)(\$|\\.)/i", $fields[1], $m)) {
                        $hostrecords[$i] = ["hostname" => $domain, "type" => "MXE", "address" => $m[1] . "." . $m[2] . "." . $m[3] . "." . $m[4]];
                    } else {
                        $hostrecords[$i] = ["hostname" => $domain, "type" => $rrtype, "address" => $fields[1], "priority" => $fields[0]];
                    }
                    $i++;
                }
            }
...
function hexonet_SaveDNS($params)
{
    ...
    $dnszone = $params["sld"] . "." . $params["tld"] . ".";
    $domain = $params["sld"] . "." . $params["tld"];
    $command = ["COMMAND" => "UpdateDNSZone", "DNSZONE" => $dnszone, "INCSERIAL" => 1, "EXTENDED" => 1, "DELRR" => ["% A", "% AAAA", "% CNAME", "% TXT", "% MX", "% X-HTTP", "% X-SMTP", "% SRV"], "ADDRR" => []];
    $mxe_hosts = [];
    ...
    $command2 = ["COMMAND" => "QueryDNSZoneRRList", "DNSZONE" => $dnszone, "EXTENDED" => 1];
    $response = hexonet_call($command2, hexonet_config($params));
    if ($response["CODE"] == 200) {
        foreach ($response["PROPERTY"]["RR"] as $rr) {
            $fields = explode(" ", $rr);
    ...
    if ($response["CODE"] == 545) {
        $creatednszone_command = ["COMMAND" => "ModifyDomain", "DOMAIN" => $domain, "INTERNALDNS" => 1];
...
function hexonet_GetEmailForwarding($params)
{
    ...
    $params = $params["original"];
    $dnszone = $params["sld"] . "." . $params["tld"] . ".";
    $command = ["COMMAND" => "QueryDNSZoneRRList", "DNSZONE" => $dnszone, "SHORT" => 1, "EXTENDED" => 1];
    $response = hexonet_call($command, hexonet_config($params));
    $result = [];
    if ($response["CODE"] == 200) {
        foreach ($response["PROPERTY"]["RR"] as $rr) {
            ...
            if ($rrtype == "X-SMTP" && $fields[1] == "MAILFORWARD") {
                ...
                $result[] = ["prefix" => $address, "forwardto" => $fields[2]];
            }
        }
    } else {
        $values["error"] = $response["DESCRIPTION"];
...
function hexonet_SaveEmailForwarding($params)
{
    ...
    $dnszone = $params["sld"] . "." . $params["tld"] . ".";
    $command = ["COMMAND" => "UpdateDNSZone", "DNSZONE" => $dnszone, "INCSERIAL" => 1, "EXTENDED" => 1, "DELRR" => ["@ X-SMTP"], "ADDRR" => []];
    foreach ($params["prefix"] as $key => $value) {
...
function hexonet_SaveContactDetails($params)
{
    ...
    $contactDetails = hexonet_call(["COMMAND" => "StatusDomain", "DOMAIN" => $domain->getDomain()], hexonet_config($origparams));
    $currentDetails = hexonet_get_contact_info($contactDetails["PROPERTY"]["OWNERCONTACT"][0], $params);
    $command = ["COMMAND" => "ModifyDomain", "DOMAIN" => $domain->getDomain()];
    $map = ["OWNERCONTACT0" => "Registrant", "ADMINCONTACT0" => "Admin", "TECHCONTACT0" => "Technical", "BILLINGCONTACT0" => "Billing"];
    ...
    if ($tldSegment == "it") {
        if ($newRegistrantDetails["First Name"] . " " . $newRegistrantDetails["Last Name"] != $currentDetails["First Name"] . " " . $currentDetails["Last Name"] && empty($newRegistrantDetails["Company Name"])) {
            $commandOverride = ["COMMAND" => "TradeDomain", "DOMAIN" => $domain->getDomain()];
        } else {
            if ($newRegistrantDetails["Company Name"] != $currentDetails["Company Name"]) {
                ...
                if ($newRegistrantDetails["Country"] != $currentDetails["Country"]) {
                    ...
                    $commandOverride = ["COMMAND" => "ModifyDomain", "DOMAIN" => $domain->getDomain()];
                }
            }
        }
        if (!($additionalDomainFields["Accept Section 3 of .IT registrar contract"] || $additionalDomainFields["Accept Section 5 of .IT registrar contract"] || $additionalDomainFields["Accept Section 6 of .IT registrar contract"] || $additionalDomainFields["Accept Section 7 of .IT registrar contract"])) {
            return ["error" => "You must accept Agreement Sections 3, 5, 6 and 7."];
        }
        $command = array_merge($command, $commandOverride);
    }
    if (in_array($tldSegment, ["ch", "li", "se", "sg"])) {
    ...
    if ($tldSegment == "ca") {
        ...
        if (!empty($additionalDomainFields["CIRA Agreement"])) {
            ...
            return ["error" => "You have to accept the CIRA Agreement."];
        }
    }
    ...
    if ($tldSegment == "se" && $command["COMMAND"] == "TradeDomain") {
        $values["pending"] = true;
        $values["pendingData"] = ["message" => "domains.changePendingFormRequired", "replacement" => [":form" => "<a href=\"https://www.domainform.net/form/se/search?view=ownerchange\" target=\"_blank\">domainform.net</a>"]];
    }
    return $values;
}
...
function hexonet_RegisterNameserver($params)
{
    ...
    $command = ["COMMAND" => "AddNameserver", "NAMESERVER" => $nameserver, "IPADDRESS0" => $params["ipaddress"]];
    $response = hexonet_call($command, hexonet_config($params));
    if ($response["CODE"] != 200) {
        $values["error"] = $response["DESCRIPTION"];
    }
...
function hexonet_ModifyNameserver($params)
{
    ...
    $params = $params["original"];
    $nameserver = $params["nameserver"];
    $command = ["COMMAND" => "ModifyNameserver", "NAMESERVER" => $nameserver, "DELIPADDRESS0" => $params["currentipaddress"], "ADDIPADDRESS0" => $params["newipaddress"]];
    $response = hexonet_call($command, hexonet_config($params));
    if ($response["CODE"] != 200) {
        $values["error"] = $response["DESCRIPTION"];
    }
...
function hexonet_DeleteNameserver($params)
{
    ...
    $params = $params["original"];
    $nameserver = $params["nameserver"];
    $command = ["COMMAND" => "DeleteNameserver", "NAMESERVER" => $nameserver];
    $response = hexonet_call($command, hexonet_config($params));
    if ($response["CODE"] != 200) {
        $values["error"] = $response["DESCRIPTION"];
    }
...
function hexonet_IDProtectToggle($params)
{
    ...
    $params = $params["original"];
    $domain = $params["domainObj"];
    $command = ["COMMAND" => "ModifyDomain", "DOMAIN" => $domain->getDomain(), "X-ACCEPT-WHOISTRUSTEE-TAC" => $params["protectenable"] ? "1" : "0"];
    if ($domain->getLastTLDSegment() == "ca") {
        $apiParams = ["COMMAND" => "StatusDomain", "DOMAIN" => $domain->getDomain()];
        ...
        $protectableCaLegalTypes = ["CCT", "RES", "ABO", "LGT"];
        if (in_array($caLegalType, $protectableCaLegalTypes)) {
            $whoisPrivate = "1";
            $commandOverrides = ["X-ACCEPT-WHOISTRUSTEE-TAC" => $whoisPrivate];
            $command = array_merge($command, $commandOverrides);
        } else {
            return ["error" => "LegalType " . $caLegalType[0] . " is not allowed for WHOIS Trustee. Domain ID: " . $params["domainid"]];
        }
    }
    $response = hexonet_call($command, hexonet_config($params));
    if ($response["CODE"] != 200) {
...
function hexonet_RegisterDomain($params)
{
    ...
    $tldSegment = $domain->getLastTLDSegment();
    $registrant = ["FIRSTNAME" => $params["firstname"], "LASTNAME" => $params["lastname"], "ORGANIZATION" => $params["companyname"], "STREET" => $params["address1"], "CITY" => $params["city"], "STATE" => $params["state"], "ZIP" => $params["postcode"], "COUNTRY" => $params["country"], "PHONE" => $params["phonenumber"], "EMAIL" => $params["email"]];
    if (strlen($params["address2"])) {
        $registrant["STREET"] .= " , " . $params["address2"];
    }
    $admin = ["FIRSTNAME" => $params["adminfirstname"], "LASTNAME" => $params["adminlastname"], "ORGANIZATION" => $params["admincompanyname"], "STREET" => $params["adminaddress1"], "CITY" => $params["admincity"], "STATE" => $params["adminstate"], "ZIP" => $params["adminpostcode"], "COUNTRY" => $params["admincountry"], "PHONE" => $params["adminphonenumber"], "EMAIL" => $params["adminemail"]];
    if (strlen($params["adminaddress2"])) {
        $admin["STREET"] .= " , " . $params["adminaddress2"];
    }
    $command = ["COMMAND" => "AddDomain", "DOMAIN" => $domain->getDomain(), "PERIOD" => $params["regperiod"], "NAMESERVER0" => $params["ns1"], "NAMESERVER1" => $params["ns2"], "NAMESERVER2" => $params["ns3"], "NAMESERVER3" => $params["ns4"], "OWNERCONTACT0" => $registrant, "ADMINCONTACT0" => $admin, "TECHCONTACT0" => $admin, "BILLINGCONTACT0" => $admin];
    if ($tldSegment == "it") {
                                                                        ...
                                                                        if ($tldSegment == "eu") {
                                                                            ...
                                                                            if ($tldSegment == "jobs") {
                                                                                ...
                                                                                if ($tldSegment == "pro") {
                                                                                    ...
                                                                                    if (in_array($tldSegment, ["fr", "re", "pm", "tf", "wf", "yt"])) {
    ...
    if ($premiumDomainsEnabled && !empty($premiumDomainsCost)) {
        $c = ["COMMAND" => "CheckDomains", "DOMAIN" => [$domain->getDomain()], "PREMIUMCHANNELS" => "*"];
    ...
    if ($tldSegment == "swiss" && $response["CODE"] == 200) {
        ...
        if (!empty($application_id)) {
            ...
            WHMCS\Database\Capsule::table("tbldomains")->where("id", "=", $params["domainid"])->update(["additionalnotes" => $addDomainNote]);
        }
        $values["pending"] = true;
...
function hexonet_TransferDomain($params)
{
    ...
    $command = ["COMMAND" => "TransferDomain", "DOMAIN" => $domain, "PERIOD" => $params["regperiod"], "NAMESERVER0" => $params["ns1"], "NAMESERVER1" => $params["ns2"], "NAMESERVER2" => $params["ns3"], "NAMESERVER3" => $params["ns4"], "OWNERCONTACT0" => $registrant, "ADMINCONTACT0" => $admin, "TECHCONTACT0" => $admin, "BILLINGCONTACT0" => $admin, "AUTH" => $eppcode];
    $tldsToExclude = ["nu", "dk", "ca", "us", "pt", "no", "se"];
...
function hexonet_RenewDomain($params)
{
    ...
    $params = $params["original"];
    $domain = $params["sld"] . "." . $params["tld"];
    $command = ["COMMAND" => "RenewDomain", "DOMAIN" => $domain, "PERIOD" => $params["regperiod"]];
    $response = hexonet_call($command, hexonet_config($params));
...
function hexonet_ReleaseDomain($params)
{
    ...
    $command = ["COMMAND" => "PushDomain", "DOMAIN" => $domain, "TARGET" => $target];
    $response = hexonet_call($command, hexonet_config($params));
    if ($response["CODE"] != 200) {
        $values["error"] = $response["DESCRIPTION"];
    }
...
function hexonet_RequestDelete($params)
{
    ...
    $params = $params["original"];
    $domain = $params["sld"] . "." . $params["tld"];
    $command = ["COMMAND" => "DeleteDomain", "DOMAIN" => $domain];
    $response = hexonet_call($command, hexonet_config($params));
    if ($response["CODE"] != 200) {
        $values["error"] = $response["DESCRIPTION"];
    }
...
function hexonet_TransferSync($params)
{
    ...
    if ($response["CODE"] == 200) {
        ...
        if ($params["idprotection"] == "1" || $params["idprotection"] == "on") {
            $command = ["COMMAND" => "ModifyDomain", "DOMAIN" => $domain, "X-ACCEPT-WHOISTRUSTEE-TAC" => "1"];
            hexonet_call($command, hexonet_config($params));
        }
    } else {
        if ($response["CODE"] == 545 || $response["CODE"] == 531) {
            $command = ["COMMAND" => "StatusDomainTransfer", "DOMAIN" => $domain];
            $response = hexonet_call($command, hexonet_config($params));
            if ($response["CODE"] == 545 || $response["CODE"] == 531) {
                $values["failed"] = true;
                $values["reason"] = "Transfer Failed";
                $loglist_command = ["COMMAND" => "QueryObjectLogList", "OBJECTCLASS" => "DOMAIN", "OBJECTID" => $domain, "ORDERBY" => "LOGDATEDESC", "LIMIT" => 1];
                ...
                if (isset($loglist_response["PROPERTY"]["LOGINDEX"])) {
                    foreach ($loglist_response["PROPERTY"]["LOGINDEX"] as $index => $logindex) {
                        ...
                        if ($type == "INBOUND_TRANSFER" && $status == "FAILED") {
                            $logstatus_command = ["COMMAND" => "StatusObjectLog", "LOGINDEX" => $logindex];
...
function hexonet_Sync($params)
{
    ...
    $domain = $params["sld"] . "." . $params["tld"];
    if ($params["tld"] == "swiss") {
        $domains = WHMCS\Database\Capsule::table("tbldomains")->where("registrar", "=", "hexonet")->where("status", "=", "pending")->where("domain", "like", "%." . $params["tld"])->get(["additionalnotes"])->all();
        foreach ($domains as $swissDomain) {
            preg_match("/\\.SWISS ApplicationID: (.+?)(?:\$|\\n)/i", $swissDomain->additionalnotes, $appId);
            if (!empty($appId[1])) {
                $command = ["COMMAND" => "StatusDomainApplication", "APPLICATION" => $appId[1]];
        ...
        if ($response["CODE"] == 200) {
            ...
            if (in_array($response["CODE"], [531, 545])) {
...
function hexonet_get_contact_info($contact, &$params)
{
    ...
    $domain = $params["sld"] . "." . $params["tld"];
    $values = [];
    $command = ["COMMAND" => "StatusContact", "CONTACT" => $contact];
    $response = hexonet_call($command, hexonet_config($params));
    if (true) {
...
{
    $additionalDomainFields = WHMCS\Database\Capsule::table("tbldomainsadditionalfields")->where("domainid", "=", $params["domainid"])->pluck("value", "name")->all();
...
function hexonet_config($params)
{
    ...
    if ($params["TestMode"] == 1 || $params["TestMode"] == "on") {
        $config["entity"] = "1234";
        $config["url"] = "https://api-ote.ispapi.net/api/call.cgi";
    }
        ...
        if (strtoupper($new_command["COMMAND"]) != "CONVERTIDN") {
            ...
            if (count($replace)) {
                if ($config["idns"] == "PHP") {
                    ...
                    $r = hexonet_call(["COMMAND" => "ConvertIDN", "DOMAIN" => $domains], $config);
...
function hexonet_parse_response($response)
{
    ...
    $hash = ["PROPERTY" => [], "CODE" => "423", "DESCRIPTION" => "Empty response from API"];
    if (!$response) {
        return $hash;
    }
    $rlist = explode("\n", $response);
    ...
    if (!$hash["CODE"] || !$hash["DESCRIPTION"]) {
        $hash = ["PROPERTY" => [], "CODE" => "423", "DESCRIPTION" => "Invalid response from API"];
    }
    return $hash;
}
