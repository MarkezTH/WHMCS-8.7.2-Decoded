...
class TPPW_AuthAPI extends TPPW_API
{
    ...
    {
        $postParams = ["AccountNo" => $accountNo, "UserId" => $userId, "Password" => $password];
...
class TPPW_ResourceAPI extends TPPW_API
{
    ...
    private function getDomainCredentials()
    {
        ...
        return ["AccountOption" => $accountOption, "AccountID" => $accountId];
    }
}
class TPPW_OrderAPI extends TPPW_API
{
    ...
    private function createContacts($all = true)
    {
        ...
        if ($contacts === false) {
            $contacts = ["Registrant" => $this->getContact(), "Admin" => $this->getContact("admin")];
        }
        $contactIds = [];
    ...
    private function getEligibilityDetails()
    {
        ...
        if ($eligibilityForm) {
            if (strpos($this->getTLD(), ".au") !== false) {
                ...
                return ["RegistrantName" => $registrantName, "EligibilityName" => $eligibilityName, "EligibilityID" => $eligibilityId, "EligibilityIDType" => $eligibilityIdType, "EligibilityType" => $eligibilityType, "EligibilityReason" => $eligibilityReason];
            }
            if (strpos($this->getTLD(), "asia") !== false) {
                ...
                return ["Country" => $country, "LegalType" => $legalType, "IdentityForm" => $identityForm, "IdentityNumber" => $identityNumber, "OtherIdentityForm" => $otherIdentityForm];
            }
        }
        return [];
    }
...
class TPPW_APIUtils
{
    public static $ELIGIBILITY_REASONS = ["Domain name is an Exact Match Abbreviation or Acronym of your Entity or Trading Name." => 1, "Close and substantial connection between the domain name and the operations of your Entity." => 2];
    ...
    public static $API_COMMON_PARAMS = ["Requester" => "WHMCS", "Version" => "2.4", "Type" => "Domains"];
    public static $ERROR_UNKNOWN = "Unknown error occurred";
    public static $ERROR_CODES = ["100" => "Missing parameters", "102" => "Authentication failed", "105" => "Request is coming from incorrect IP address", "202" => "Invalid API Type", "203" => "API call has not been implemented yet", "301" => "Invalid order ID", "302" => "Domain name is either invalid or not supplied", "303" => "Domain prices are not setted up", "304" => "Domain registration failed", "305" => "Domain renewal failed", "306" => "Domain transfer failed", "307" => "Incorrect auth code provided", "309" => "Invalid domain extension", "311" => "Domain does not exist in your reseller account", "312" => "Invalid username/password", "313" => "Account does not exist in your reseller profile", "401" => "Failed to connect to registry, please retry", "500" => "Prepaid account does not have enough funds to cover the cost of this order", "501" => "Invalid credit card type", "502" => "Invalid credit card number", "503" => "Invalid credit card expiry date", "505" => "Credit card transaction failed", "600" => "Failed to create/update contact", "601" => "Failed to create order", "602" => "Invalid hosts supplied", "603" => "Invalid eligibility fields supplied", "604" => "Invalid IP Address", "610" => "Failed to connect to registry, please retry", "611" => "Domain renewal/transfer failed", "612" => "Locking is not available for this domain", "614" => "Failed to lock/unlock domain", "615" => "Domain delegation failed", "700" => "Invalid Product", "701" => "Domain already exists", "888" => "Order is still being processed. Current Status", "889" => "No OrderID for this domain name was found"];
    public static $CONTACT_TYPES = ["Owner" => "Registrant", "Administration" => "Admin", "Technical" => "Tech", "Billing" => "Billing"];
    public static $CHECK_PERIOD_BEFORE_XFER = ["com.au", "net.au", "org.au", "asn.au", "id.au"];
    public static $CONTACT_FIELDS = ["FirstName" => "firstname", "LastName" => "lastname", "Address1" => "address1", "Address2" => "address2", "City" => "city", "Region" => "state", "PostalCode" => "postcode", "CountryCode" => "country", "Email" => "email", "PhoneNumber" => "phonenumber"];
    public static $TRANSFORM_CONTACT_FIELDS = ["First Name" => "FirstName", "Last Name" => "LastName", "Address 1" => "Address1", "Address 2" => "Address2", "Postcode" => "PostalCode", "Country" => "CountryCode", "Phone Number" => "PhoneNumber", "Organisation Name" => "OrganisationName", "Email Address" => "Email"];
    public static $ASIA_LEGAL_TYPES = ["naturalPerson" => "naturalPerson", "corporation" => "corporation", "cooperative" => "cooperative", "partnership" => "partnership", "government" => "government", "politicalParty" => "politicalParty", "society" => "society", "institution" => "institution"];
    public static $ASIA_IDENTITY_FORMS = ["passport" => "passport", "politicalPartyRegistry" => "politicalPartyRegistry", "societyRegistry" => "societiesRegistry", "legislation" => "legislation", "certificate" => "certificate"];
    public static $ELIGIBILITY_ID_TYPES = ["ACN" => 1, "ACT BN" => 2, "NSW BN" => 3, "NT BN" => 4, "QLD BN" => 5, "SA BN" => 6, "TAS BN" => 7, "VIC BN" => 8, "WA BN" => 9, "Trademark" => 10, "Other" => 11, "ABN" => 12, "Australian Company Number (ACN)" => 1, "ACT Business Number" => 2, "NSW Business Number" => 3, "NT Business Number" => 4, "QLD Business Number" => 5, "SA Business Number" => 6, "TAS Business Number" => 7, "VIC Business Number" => 8, "WA Business Number" => 9, "Trademark (TM)" => 10, "Other - Used to record an Incorporated Association number" => 11, "Australian Business Number (ABN)" => 12, "Business Registration Number" => 11];
    public static $ELIGIBILITY_TYPES = ["Charity" => 1, "Citizen/Resident" => 2, "Club" => 3, "Commercial Statutory Body" => 4, "Company" => 5, "Incorporated Association" => 6, "Industry Body" => 8, "Non-profit Organisation" => 9, "Other" => 10, "Partnership" => 11, "Pending TM Owner" => 12, "Political Party" => 13, "Registered Business" => 14, "Religious/Church Group" => 15, "Sole Trader" => 16, "Trade Union" => 17, "Trademark Owner" => 18, "Child Care Centre" => 19, "Government School" => 20, "Higher Education Institution" => 21, "National Body" => 22, "Non-Government School" => 23, "Pre-school" => 24, "Research Organisation" => 25, "Training Organisation" => 26];
...
{
    $configarray = ["FriendlyName" => ["Type" => "System", "Value" => "TPP Wholesale"], "AccountNo" => ["Type" => "text", "Size" => "20", "Description" => "Enter your " . TPPW_APIUtils::$BRAND_NAME . " API Account Number"], "Login" => ["Type" => "text", "Size" => "20", "Description" => "Enter your " . TPPW_APIUtils::$BRAND_NAME . " API Login"], "Password" => ["Type" => "password", "Size" => "20", "Description" => "Enter your " . TPPW_APIUtils::$BRAND_NAME . " API Password"], "DefaultAccount" => ["Type" => "text", "Size" => "20", "Description" => "<ul><li>Leave this field blank if you are happy for WHMCS to create client accounts on the TPP Wholesale systems and group each client's domain registrations within them. </li><li>&nbsp;</li><li>If you'd prefer to have all the domain names simply grouped in one account on the TPP Wholesale systems, enter that Account Reference here.</li></ul>"], "Debug" => ["Type" => "yesno", "Description" => "Check to log API calls to WHMCS' \"Module Log\" section. (Version: " . TPPW_APIUtils::$API_COMMON_PARAMS["Version"] . ")"], "" => ["Description" => "<b>Need Help?</b> For assistance configuring WHMCS at TPP Wholesale, please see this <a target=\"_blank\" href=\"" . TPPW_APIUtils::$URL_SUPPORT . "\">support article</a>."]];
    return $configarray;
}
...
function tppwregistrar_TransferSync($params)
{
    ...
    if ($results->isSuccess()) {
        return ["expirydate" => $results->get("ExpiryDate"), "active" => true];
    }
    if ($results->getModuleErrorCode() === 311) {
        ...
        if ($orderStatus->isSuccess() && stripos($orderStatus->getResponse(), "reject") !== false) {
            return ["failed" => true, "reason" => $orderStatus->getResponse()];
        }
        return [];
    }
    return $results->getModuleError();
...
function tppwregistrar_Sync($params)
{
    ...
    if ($results->isSuccess()) {
        ...
        if ($expiryEpoch !== false) {
            if (time() < $expiryEpoch) {
                return ["expirydate" => $expiry, "active" => true];
            }
            return ["expirydate" => $expiry, "expired" => true];
        }
    } else {
        return $results->getModuleError();
    }
...
{
    return ["Check Order Progress" => "checkOrder"];
}
