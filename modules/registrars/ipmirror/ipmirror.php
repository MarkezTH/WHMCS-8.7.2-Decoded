...
{
    $configarray = ["Username" => ["FriendlyName" => "User name", "Type" => "text", "Size" => "20", "Description" => "Enter your ccTLDBox username here"], "Password" => ["FriendlyName" => "Password", "Type" => "password", "Size" => "20", "Description" => "Enter your ccTLDBox password here"], "ccTLDBoxUrl" => ["FriendlyName" => "ccTLDBox URL", "Type" => "text", "Size" => "35", "Description" => "URL to ccTLDBox Registrar API", "Default" => "https://<partnerdomain>/RAPI/"], "AllowRegContactChange" => ["FriendlyName" => "Allow Registrant Contact Change", "Type" => "yesno", "Description" => "Check to allow a client to change the registrant contact of a domain", "Default" => true], "AllowAdminContactChange" => ["FriendlyName" => "Allow Admin Contact Change", "Type" => "yesno", "Description" => "Check to allow a client to change the administrative contact of a domain", "Default" => false], "AllowTechContactChange" => ["FriendlyName" => "Allow Technical Contact Change", "Type" => "yesno", "Description" => "Check to allow a client to change the technical contact of a domain", "Default" => false], "AllowBillContactChange" => ["FriendlyName" => "Allow Billing Contact Change", "Type" => "yesno", "Description" => "Check to allow a client to change the billing contact of a domain", "Default" => false], "DefaultRegContactId" => ["FriendlyName" => "Default Registrant Contact ID", "Type" => "text", "Size" => "20", "Description" => "ID of default ccTLDBox registrant contact (optional)"], "DefaultAdminContactId" => ["FriendlyName" => "Default Admin Contact ID", "Type" => "text", "Size" => "20", "Description" => "ID of default ccTLDBox administrative contact (optional)"], "DefaultTechContactId" => ["FriendlyName" => "Default Technical Contact ID", "Type" => "text", "Size" => "20", "Description" => "ID of default ccTLDBox technical contact (optional)"], "DefaultBillContactId" => ["FriendlyName" => "Default Billing Contact ID", "Type" => "text", "Size" => "20", "Description" => "ID of default ccTLDBox billing contact (optional)"], "SingICCustomFld" => ["FriendlyName" => "Singapore IC custom field", "Type" => "text", "Size" => "20", "Description" => "Custom client field name for Singapore IC, required for Admin contact of .sg domains"], "ConnectionTimeout" => ["FriendlyName" => "Connection timeout", "Type" => "text", "Size" => "2", "Description" => "Connection to ccTLDBox timeout [seconds] (default=20)", "Default" => 20], "ExecutionTimeout" => ["FriendlyName" => "Execution timeout", "Type" => "text", "Size" => "2", "Description" => "Execution of ccTLDBox command timeout [seconds] (default=60)", "Default" => 60]];
    return $configarray;
}
...
function _ipmirror_initGlobals($params)
{
    ...
    $_IPMIRROR_CONFIG["zoneRecTypesDisp"] = ["A", "CNAME", "MX", "TXT", "redirect", "cloak"];
    $_IPMIRROR_CONFIG["zoneRecTypesWHMCS"] = ["A", "CNAME", "MX", "TXT", "URL", "FRAME"];
    $_IPMIRROR_CONFIG["defNsAddr"] = "10.1.1.1";
    $_IPMIRROR_ERR_MESS = ["domain_already_reg" => "The domain is already registered", "unknown_curl_error" => "Unknown CURL error"];
    $_IPMIRROR_CONTACT_HDRS = ["Registrant", "Administrative", "Technical", "Billing"];
    $_IPMIRROR_CONTACT_LBLS = ["upd1" => ["ind" => "Individual", "org" => "Organisation", "id" => "Contact ID", "type" => "Contact Type", "rcbid" => "Comp. Reg. No.", "icno" => "IC Number", "company" => "Company Name", "title" => "Title", "firstname" => "First Name", "lastname" => "Last Name", "address1" => "Address 1", "address2" => "Address 2", "city" => "City", "state" => "State", "country" => "Country", "postcode" => "Postcode", "phone" => "Phone Number", "fax" => "Fax Number", "email" => "Email Address"], "upd2" => ["firstname" => "First Name", "lastname" => "Last Name", "address1" => "Address 1", "address2" => "Address 2", "city" => "City", "state" => "State", "postcode" => "Postcode", "phone" => "Phone Number", "email" => "Email"]];
}
...
function _ipmirror_callRapi($cmd, $params)
{
    ...
    if ($output === false || empty($output)) {
        $result = ["0", curl_errno($ch), trim(curl_error($ch)) == "" ? $_IPMIRROR_ERR_MSG["unknown_curl_error"] : curl_error($ch)];
    ...
    logModuleCall($_IPMIRROR_CONFIG["module"], $cmd, $url, $resultString, $resultString, [$_IPMIRROR_CONFIG["password"]]);
    return $result;
}
...
function _ipmirror_GetDNS($params)
{
    ...
    foreach ($zoneRecs as $zoneRec) {
        if (in_array($zoneRec["type"], $_IPMIRROR_CONFIG["zoneRecTypesDisp"])) {
            ...
            $values[] = ["id" => $zoneRec["id"], "hostname" => $zoneRec["source"], "type" => $whmcsType, "address" => $zoneRec["destination"], "priority" => $zoneRec["priority"]];
        }
    }
    session_start();
...
function _ipmirror_createTransferDomain($mode, $params)
{
    ...
    foreach (["Reg", "Admin", "Tech", "Bill"] as $contactCode) {
        $contactIDs[$contactCode] = _ipmirror_createContact($params, $contactCode);
        if (!$contactIDs[$contactCode]["contactId"]) {
...
function _ipmirror_createContact($params, $contactCode)
{
    ...
    if (!empty($defContactId)) {
        return ["contactId" => $defContactId, "hasDefault" => true];
    }
    ...
    if (_ipmirror_geterrormsg()) {
        return ["contactId" => false, "hasDefault" => false];
    }
    return ["contactId" => $results[2], "hasDefault" => false];
}
...
function _ipmirror_getClientCustFieldValue($clientId, $fieldName)
{
    ...
    $where = ["tblcustomfields.fieldname" => $fieldName, "tblcustomfieldsvalues.relid" => $clientId];
    $sort = "";
    $sortorder = "";
    $limits = "";
    ...
    logModuleCall($_IPMIRROR_CONFIG["module"], "_ipmirror_getClientCustFieldValue", "client id = " . $clientId . "; fld name = '" . $fieldName . "'", $resultString, $resultString, []);
    return $fieldValue;
}
...
function _ipmirror_TransferSync($params)
{
    ...
    $_IPMIRROR_TRANSF_IN_OK_STATUS = ["registered", "toLapse", "redemption", "expire", "pendingDelete"];
...
function _ipmirror_Sync($params)
{
    ...
    $cBoxStatus = $domResult[1];
    $cBoxExpiryDate = date("Y-m-d", strtotime($domResult[4]));
    $_IPMIRROR_ACTIVE_STATUS = ["registered", "toLapse", "redemption"];
    $_IPMIRROR_EXPIRED_STATUS = ["expire", "pendingDelete"];
...
{
    return _ipmirror_callrapi("queryContact", ["contactID" => $contactID]);
}
...
{
    $cmdParams = ["type" => $type, "rcbID" => $rcbID, "title" => $title, "firstName" => $firstName, "lastName" => $lastName, "street1" => $street1, "city" => $city, "postalCode" => $postalCode, "state" => $state, "country" => $country, "tel" => $tel, "fax" => $fax, "email" => $email, "languageCode" => "ENG"];
    if (!empty($street2)) {
...
{
    $cmdParams = ["contactID" => $contactId, "title" => $title, "street1" => $street1, "city" => $city, "postalCode" => $postalCode, "state" => $state, "tel" => $tel, "fax" => $fax, "email" => $email, "languageCode" => "ENG"];
    if ($type == "1") {
...
{
    return _ipmirror_callrapi("deleteContact", ["contactID" => $contactID]);
}

function _ipmirror_rapi_queryDomain($dName)
{
    return _ipmirror_callrapi("queryDomain", ["dName" => $dName]);
}

function _ipmirror_rapi_createDomain($dName, $term, $regID, $adminID, $techID, $billID, $ns1, $ns2, $ns3, $ns4, $idprotection)
{
    $cmdParams = ["dName" => $dName, "encoding" => "ENG", "term" => $term, "regID" => $regID, "adminID" => $adminID, "techID" => $techID, "billID" => $billID, "ns1" => $ns1, "ns2" => $ns2];
    if (!empty($ns3)) {
        $cmdParams["ns3"] = $ns3;
    }
    if (!empty($ns4)) {
...
{
    return _ipmirror_callrapi("deleteHost", ["ns" => $ns]);
}

function _ipmirror_rapi_queryZone($dName)
{
    return _ipmirror_callrapi("queryZone", ["dName" => $dName]);
}

function _ipmirror_rapi_queryZoneRecord($id)
{
    return _ipmirror_callrapi("queryZoneRecord", ["id" => $id]);
}

function _ipmirror_rapi_createZoneRecord($dName, $detailType, $source, $priority, $destination)
{
    $cmdParams = ["dName" => $dName, "detailType" => $detailType, "source" => $source, "priority" => $priority, "destination" => $destination];
...
{
    $cmdParams = ["dName" => $dName, "id" => $id, "source" => $source, "priority" => $priority, "destination" => $destination];
    return _ipmirror_callrapi("updateZoneRecord", $cmdParams);
}

function _ipmirror_rapi_deleteZoneRecord($dName, $id)
{
    $cmdParams = ["dName" => $dName, "id" => $id];
...
{
    $cmdParams = ["dName" => $dName, "detailType" => $detailType, "source" => $source, "destination" => $destination];
    if (!empty($title)) {
...
{
    $cmdParams = ["dName" => $dName, "id" => $id, "detailType" => $detailType, "source" => $source, "destination" => $destination];
    if (!empty($title)) {
        $cmdParams["title"] = $title;
    }
    if (!empty($metaDesc)) {
...
{
    $cmdParams = ["dName" => $dName, "source" => $source, "destination" => $destination];
...
{
    $cmdParams = ["dName" => $dName, "id" => $id, "source" => $source, "destination" => $destination];
