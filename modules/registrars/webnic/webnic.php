...
{
    $configarray = ["Source" => ["Type" => "text", "Size" => "20", "Description" => ""], "Password" => ["Type" => "password", "Size" => "20", "Description" => ""], "TestMode" => ["Type" => "yesno"]];
    return $configarray;
}
...
function webnic_GetNameservers($params)
{
    ...
    if ($results[0] == 0) {
        ...
        return ["ns1" => $arr["ns1"], "ns2" => $arr["ns2"], "ns3" => $arr["ns3"], "ns4" => $arr["ns4"], "ns5" => $arr["ns5"]];
    } else {
        if ($results[1]) {
    ...
    if (preg_match("/(asia|tw)\$/i", $params["tld"])) {
        ...
        switch ($legaltype) {
            ...
            default:
                ...
                foreach (["custom_reg3", "custom_adm3", "custom_tec3", "custom_bil3"] as $removefield) {
...
function webnic_GetContactDetails($params)
{
    ...
    if ($results[0] == 0) {
        ...
        return ["Registrant" => ["First Name" => $arr["registrant first name"], "Last Name" => $arr["registrant last name"], "Company Name" => $arr["registrant"], "Address 1" => $arr["registrant address 1"], "Address 2" => $arr["registrant address 2"], "City" => $arr["registrant city"], "State" => $arr["registrant state"], "Country" => $arr["registrant country"], "ZIP Code" => $arr["registrant zip"], "Phone Number" => $arr["registrant phone"], "Fax Number" => $arr["registrant fax"], "Email Address" => $arr["registrant email"]], "Admin" => ["First Name" => $arr["admin first name"], "Last Name" => $arr["admin last name"], "Company Name" => $arr["admin company"], "Address 1" => $arr["admin address 1"], "Address 2" => $arr["admin address 2"], "City" => $arr["admin city"], "State" => $arr["admin state"], "Country" => $arr["admin country"], "ZIP Code" => $arr["admin zip"], "Phone Number" => $arr["admin phone"], "Fax Number" => $arr["admin fax"], "Email Address" => $arr["admin email"]], "Technical" => ["First Name" => $arr["technical first name"], "Last Name" => $arr["technical last name"], "Company Name" => $arr["technical company"], "Address 1" => $arr["technical address 1"], "Address 2" => $arr["technical address 2"], "City" => $arr["technical city"], "State" => $arr["technical state"], "Country" => $arr["technical country"], "ZIP Code" => $arr["technical zip"], "Phone Number" => $arr["technical phone"], "Fax Number" => $arr["technical fax"], "Email Address" => $arr["technical email"]], "Billing" => ["First Name" => $arr["billing first name"], "Last Name" => $arr["billing last name"], "Company Name" => $arr["billing company"], "Address 1" => $arr["billing address 1"], "Address 2" => $arr["billing address 2"], "City" => $arr["billing city"], "State" => $arr["billing state"], "Country" => $arr["billing country"], "ZIP Code" => $arr["billing zip"], "Phone Number" => $arr["billing phone"], "Fax Number" => $arr["billing fax"], "Email Address" => $arr["billing email"]]];
    } else {
        return ["error" => $results[1]];
    }
}
...
function webnic_call($url, $rtype, $postfields, $params)
{
    ...
    $res_array = explode("\n", $res_data);
    logModuleCall("webnic", $rtype, $query_string, $res_data, $res_array, [$params["Source"]]);
    return $res_array;
}

function webnic_isCountryInAsia($countrycode)
{
    $asiacountrycodes = ["af", "am", "az", "bh", "bd", "bt", "bn", "kh", "cn", "cx", "cc", "io", "ge", "hk", "in", "id", "ir", "iq", "il", "jp", "jo", "kz", "kp", "kr", "kw", "kg", "la", "lb", "mo", "my", "mv", "mn", "mm", "np", "om", "pk", "ph", "qa", "sa", "sg", "lk", "sy", "tw", "tj", "th", "tr", "tm", "ae", "uz", "vn", "ye"];
