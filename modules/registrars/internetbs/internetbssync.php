    ...
    if (!$result) {
        ...
        while ($data = mysql_fetch_array($queryresult)) {
            ...
            if (isset($result[$domainname])) {
                ...
                if ($status == "ok") {
                    update_query("tbldomains", ["status" => "Active"], ["domain" => $domainname]);
                }
                if ($expirydate) {
                    update_query("tbldomains", ["expirydate" => $expirydate], ["domain" => $domainname]);
                    if ($SyncNextDueDate) {
                        update_query("tbldomains", ["nextduedate" => $expirydate], ["domain" => $domainname]);
                    }
...
function parseResult($data)
{
    ...
    for ($i = 0; $i < $totalDomains; $i++) {
        ...
        $ddat = ["expiry" => mktime(0, 0, 0, $m, $d, $y), "status" => $status];
