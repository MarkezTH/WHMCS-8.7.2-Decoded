...
function internetbs_runCommand($commandUrl, $postData)
{
    ...
    $action = str_replace([API_TESTSERVER_URL, API_SERVER_URL], "", $commandUrl);
    logModuleCall("InternetBS", $action, $postData, $data, $parsedResult, [$postData["apikey"], $postData["password"]]);
...
{
    $configarray = ["FriendlyName" => ["Type" => "System", "Value" => "Internet.bs"], "Username" => ["Type" => "text", "Size" => "50", "Description" => "Enter your Internet.bs ApiKey here"], "Password" => ["Type" => "password", "Size" => "50", "Description" => "Enter your Internet.bs password here"], "TestMode" => ["Type" => "yesno", "Description" => "Check if you want to connect to the test server"], "SyncNextDueDate" => ["Type" => "yesno", "Description" => "Check if you want the expiry date sync script to update both expiry and next due dates (cron must be configured). If left unchecked it will only update the domain expiration date."]];
    return $configarray;
}

function internetbs_config_validate($params)
{
    ...
    $data = ["apikey" => $username, "password" => $password];
...
function internetbs_GetNameservers($params)
{
    ...
    $data = ["apikey" => $username, "password" => $password, "domain" => $domainName];
    $result = internetbs_runcommand($commandUrl, $data);
    $errorMessage = internetbs_getlasterror();
    if ($result === false) {
...
function internetbs_SaveNameservers($params)
{
    ...
    $data = ["apikey" => $username, "password" => $password, "domain" => $domainName, "ns_list" => trim(implode(",", $nslist), ",")];
    $result = internetbs_runcommand($commandUrl, $data);
    $errorMessage = internetbs_getlasterror();
    if ($result === false) {
        $values["error"] = internetbs_getconnectionerrormessage($errorMessage);
...
function internetbs_GetDNS($params)
{
    ...
    if ($result === false) {
        ...
        for ($i = 0; $i < $totalRecords; $i++) {
            ...
            if (in_array(strtolower($recordType), ["a", "mx", "cname", "txt"])) {
                ...
                if (isset($result["records_" . $i . "_name"])) {
                    $hostrecords[] = ["hostname" => $recordHostname, "type" => $recordType, "address" => $recordAddress];
                }
            }
        }
...
function internetbs_RegisterDomain($params)
{
    ...
    if ($tld == "eu") {
        $europianLanguages = ["cs", "da", "de", "el", "en", "es", "et", "fi", "fr", "hu", "it", "lt", "lv", "mt", "nl", "pl", "pt", "sk", "sl", "sv", "ro", "bg", "ga"];
        if (!in_array($data["registrant_language"], $europianLanguages)) {
            $data["registrant_language"] = "en";
        }
        $europianCountries = ["AT", "AX", "BE", "BG", "CZ", "CY", "DE", "DK", "ES", "EE", "FI", "FR", "GR", "GB", "GF", "GI", "GP", "HU", "IE", "IT", "LT", "LU", "LV", "MT", "MQ", "NL", "PL", "PT", "RE", "RO", "SE", "SK", "SI"];
    ...
    if ($tld == "it") {
        $EUCountries = ["AT", "BE", "BG", "CZ", "CY", "DE", "DK", "ES", "EE", "FI", "FR", "GR", "GB", "HU", "IE", "IT", "LT", "LU", "LV", "MT", "NL", "PL", "PT", "RO", "SE", "SK", "SI"];
        $EntityTypes = ["Italian and foreign natural persons" => 1, "Companies/one man companies" => 2, "Freelance workers/professionals" => 3, "non-profit organizations" => 4, "public organizations" => 5, "other subjects" => 6, "non natural foreigners" => 7];
        ...
        if (strtoupper($data["technical_countrycode"]) == "IT") {
            $data["technical_dotitprovince"] = $AdminStateProvince;
        }
...
function internetbs_TransferDomain($params)
{
    ...
    $data = ["apikey" => $username, "password" => $password, "domain" => $domainName, "transferAuthInfo" => $transfersecret, "registrant_firstname" => $RegistrantFirstName, "registrant_lastname" => $RegistrantLastName, "registrant_street" => $RegistrantAddress1, "registrant_street2" => $RegistrantAddress2, "registrant_city" => $RegistrantCity, "registrant_countrycode" => $RegistrantCountry, "registrant_postalcode" => $RegistrantPostalCode, "registrant_email" => $RegistrantEmailAddress, "registrant_phonenumber" => $RegistrantPhone, "technical_firstname" => $AdminFirstName, "technical_lastname" => $AdminLastName, "technical_street" => $AdminAddress1, "technical_street2" => $AdminAddress2, "technical_city" => $AdminCity, "technical_countrycode" => $AdminCountry, "technical_postalcode" => $AdminPostalCode, "technical_email" => $AdminEmailAddress, "technical_phonenumber" => $AdminPhone, "admin_firstname" => $AdminFirstName, "admin_lastname" => $AdminLastName, "admin_street" => $AdminAddress1, "admin_street2" => $AdminAddress2, "admin_city" => $AdminCity, "admin_countrycode" => $AdminCountry, "admin_postalcode" => $AdminPostalCode, "admin_email" => $AdminEmailAddress, "admin_phonenumber" => $AdminPhone, "billing_firstname" => $AdminFirstName, "billing_lastname" => $AdminLastName, "billing_street" => $AdminAddress1, "billing_street2" => $AdminAddress2, "billing_city" => $AdminCity, "billing_countrycode" => $AdminCountry, "billing_postalcode" => $AdminPostalCode, "billing_email" => $AdminEmailAddress, "billing_phonenumber" => $AdminPhone];
    if (!empty($RegistrantCompany)) {
        $data["Registrant_Organization"] = $RegistrantCompany;
    }
    if (!empty($AdminCompany)) {
    ...
    if ($ext == "fr" || $ext == "re" || $ext == "pm" || $ext == "tf" || $ext == "wf" || $ext == "yt") {
        ...
        switch (strtolower($holderType)) {
            ...
            default:
                ...
                if ($tld == "it") {
                    $EUCountries = ["AT", "BE", "BG", "CZ", "CY", "DE", "DK", "ES", "EE", "FI", "FR", "GR", "GB", "HU", "IE", "IT", "LT", "LU", "LV", "MT", "NL", "PL", "PT", "RO", "SE", "SK", "SI"];
                    $EntityTypes = ["1. Italian and foreign natural persons" => 1, "2. Companies/one man companies" => 2, "3. Freelance workers/professionals" => 3, "4. non-profit organizations" => 4, "5. public organizations" => 5, "6. other subjects" => 6, "7. foreigners who match 2 - 6" => 7];
...
function internetbs_RegisterNameserver($params)
{
    ...
    $data = ["apikey" => $username, "password" => $password, "host" => $nameserver, "ip_list" => $ipaddress];
    $result = internetbs_runcommand($commandUrl, $data);
    $errorMessage = internetbs_getlasterror();
    if ($result === false) {
        $values["error"] = internetbs_getconnectionerrormessage($errorMessage);
...
function internetbs_ModifyNameserver($params)
{
    ...
    $data = ["apikey" => $username, "password" => $password, "host" => $nameserver, "ip_list" => $newipaddress];
    $result = internetbs_runcommand($commandUrl, $data);
    $errorMessage = internetbs_getlasterror();
    if ($result === false) {
        $values["error"] = internetbs_getconnectionerrormessage($errorMessage);
...
function internetbs_DeleteNameserver($params)
{
    ...
    $data = ["apikey" => $username, "password" => $password, "host" => $nameserver];
    $result = internetbs_runcommand($commandUrl, $data);
    $errorMessage = internetbs_getlasterror();
    if ($result === false) {
        $values["error"] = internetbs_getconnectionerrormessage($errorMessage);
...
function internetbs_get_utf8_parameters($params)
{
    ...
    if (!in_array(strtolower($charset), ["utf8", "utf-8"])) {
        return $params;
    }
    $domainData = WHMCS\Domain\Domain::find($params["domainid"]);
    if (!$domainData) {
        return $params;
    }
    if ($domainData->order->contactId) {
        $contactData = $domainData->order->contact;
        foreach ($contactData->toArray() as $key => $value) {
            $params[$key] = $value;
        }
    } else {
        if ($domainData->order->userId) {
            $clientData = $domainData->order->client;
            foreach ($clientData->toArray() as $key => $value) {
                $params[$key] = $value;
            }
        }
    }
    if (WHMCS\Config\Setting::getValue("RegistrarAdminUseClientDetails")) {
        ...
        $regData = WHMCS\Config\Setting::allAsArray();
        $params["adminfirstname"] = $regData["RegistrarAdminFirstName"];
        $params["adminlastname"] = $regData["RegistrarAdminLastName"];
        $params["admincompanyname"] = $regData["RegistrarAdminCompanyName"];
        $params["adminemail"] = $regData["RegistrarAdminEmailAddress"];
        $params["adminaddress1"] = $regData["RegistrarAdminAddress1"];
        $params["adminaddress2"] = $regData["RegistrarAdminAddress2"];
        $params["admincity"] = $regData["RegistrarAdminCity"];
        $params["adminstate"] = $regData["RegistrarAdminStateProvince"];
        $params["adminpostcode"] = $regData["RegistrarAdminPostalCode"];
        $params["admincountry"] = $regData["RegistrarAdminCountry"];
        $params["adminphonenumber"] = $regData["RegistrarAdminPhone"];
    }
    return $params;
}
