...
{
    $configarray = ["FriendlyName" => ["Type" => "System", "Value" => "SecureTrading"], "username" => ["FriendlyName" => "Username", "Type" => "text", "Size" => "20"], "password" => ["FriendlyName" => "Password", "Type" => "text", "Size" => "20"], "siteref" => ["FriendlyName" => "Site Reference", "Type" => "text", "Size" => "20"]];
    return $configarray;
}
...
function securetrading_capture($params)
{
    ...
    $headers = ["HTTP/1.1", "Host: webservices.securetrading.net", "Accept: text/xml", "Authorization: " . $authstr, "User-Agent: WHMCS Gateway Module", "Content-type: text/xml;charset=\"utf-8\"", "Content-length: " . strlen($xml), "Connection: close"];
    $ch = curl_init();
    ...
    if ($xmldata["RESPONSEBLOCK"]["RESPONSE"]["ERROR"]["CODE"] == "0") {
        ...
        return ["status" => "success", "transid" => $results["transid"], "rawdata" => $data];
    }
    if ($xmldata["RESPONSEBLOCK"]["RESPONSE"]["ERROR"]["CODE"] == "99999") {
        $results["status"] = "error";
        return ["status" => "error", "rawdata" => $data];
    }
    return ["status" => "declined", "rawdata" => $data];
}
