...
function asiapay_config()
{
    ...
    $hashKey = WHMCS\Module\GatewaySetting::getValue("asiapay", "secureHashKey");
    if (empty($hashKey)) {
        $secureHashDescription = "Secure Hash Key is required for additional security with Asia Pay.";
    }
    $configArray = ["FriendlyName" => ["Type" => "System", "Value" => "AsiaPay"], "merchantid" => ["FriendlyName" => "Merchant ID", "Type" => "text", "Size" => "20"], "secureHashKey" => ["FriendlyName" => "Secure Hash Key (if enabled)", "Type" => "text", "Size" => "40", "Description" => $secureHashDescription], "testmode" => ["FriendlyName" => "Test Mode", "Type" => "yesno"]];
    return $configArray;
}
...
function asiapay_link($params)
{
    ...
    $payType = "N";
    $lang = "E";
    $currCodeArr = ["HKD" => 344, "SGD" => 702, "CNY" => 156, "JPY" => 392, "TWD" => 901, "AUD" => "036", "EUR" => 978, "GBP" => 826, "CAD" => 124, "MOP" => 446, "PHP" => 608, "THB" => 764, "MYR" => 458, "IDR" => 360, "KRW" => 410, "SAR" => 682, "NZD" => 784, "BND" => "096"];
    ...
    if (isset($params["secureHashKey"]) && 0 < strlen(trim($params["secureHashKey"]))) {
        $hashArr = [$merchantId, $orderRef, $currCode, $amount, $payType, $params["secureHashKey"]];
