...
function skrill_link($params)
{
    ...
    if ($params["clientdetails"]["gatewayid"]) {
        $postFields = ["email" => $params["emailAddress"], "password" => md5($params["apiMqiPassword"]), "action" => "prepare", "amount" => _skrill_amount($params["amount"]), "currency" => $params["currency"], "ondemand_note" => $params["description"], "frn_trn_id" => $params["invoiceid"] . "-" . time(), "rec_payment_id" => $params["clientdetails"]["gatewayid"], "merchant_fields" => "platform,invoice_id", "platform" => "96648253", "invoice_id" => $params["invoiceid"]];
        try {
            $transaction = _skrill_one_tap_request($postFields);
            if ($transaction["STATUS"] == "-2") {
                logTransaction("skrill", array_merge(["response" => $transaction, "request" => $postFields], $params), "Declined", $params);
            ...
            logTransaction("skrill", array_merge(["response" => $transaction, "request" => $postFields], $params), "Success", $params);
            ...
            logTransaction("skrill", array_merge(["response" => $e->getMessage(), "request" => $postFields], $params), "Error on Payment", $params);
...
function skrill_capture($params)
{
    try {
        if (!$params["gatewayid"]) {
            throw new WHMCS\Exception\Module\InvalidConfiguration("No 1-Tap Payment Information Stored");
        }
        $postFields = ["email" => $params["emailAddress"], "password" => md5($params["apiMqiPassword"]), "action" => "status_od", "amount" => _skrill_amount($params["amount"]), "trn_id" => $params["gatewayid"]];
        ...
        $transaction = _skrill_one_tap_request($postFields);
        if ($transaction["STATUS"] == "-2") {
            return ["status" => "error", "rawdata" => $transaction];
        }
        return ["status" => "success", "transid" => $transaction["ID"], "amount" => $transaction["AMOUNT"], "rawdata" => $transaction];
    } catch (WHMCS\Exception\Billing\BillingException $e) {
        return ["status" => "max_amount", "rawdata" => $e->getMessage()];
    } catch (Exception $e) {
        return ["status" => "error", "rawdata" => $e->getMessage()];
    }
}

function skrill_storeremote($params)
{
    if ($params["action"] == "delete") {
        try {
            $postFields = ["email" => $params["emailAddress"], "password" => md5($params["apiMqiPassword"]), "action" => "cancel_od", "amount" => 0, "trn_id" => $params["gatewayid"]];
            $url = "https://www.skrill.com/app/query.pl";
            $rawResponse = curlCall($url, $postFields);
            if (substr($rawResponse, 0, 10) == "CURL Error") {
                throw new WHMCS\Exception\Module\NotServicable($rawResponse);
            ...
            $response = $response[2];
            if ($response == "OK") {
                return ["status" => "success", "rawdata" => ["request" => $postFields, "response" => $rawResponse]];
            }
    ...
    return ["status" => "success", "rawdata" => ["info" => "No Action Performed"]];
}

function skrill_refund($params)
{
    try {
        $postFields = ["email" => $params["emailAddress"], "password" => md5($params["apiMqiPassword"]), "action" => "prepare", "amount" => _skrill_amount($params["amount"]), "transaction_id" => $params["transid"], "merchant_fields" => "platform", "platform" => "96648253"];
        ...
        $postFields = ["sid" => $sessionId, "action" => "refund"];
        $rawResponse = curlCall($url, $postFields);
        if (substr($rawResponse, 0, 10) == "CURL Error") {
            throw new WHMCS\Exception\Module\NotServicable($rawResponse);
        }
        ...
        return ["status" => "success", "rawdata" => $response, "transid" => $response["MB_TRANSACTION_ID"]];
    } catch (Exception $e) {
        return ["status" => "error", "rawdata" => $e->getMessage()];
    }
}
...
function _skrill_language($language)
{
    ...
    $languages = ["bulgarian" => "BG", "chinese" => "ZH", "czech" => "CS", "danish" => "DA", "dutch" => "NL", "english" => "EN", "finnish" => "FI", "french" => "FR", "german" => "DE", "greek" => "EL", "italian" => "IT", "japanese" => "JA", "polish" => "PL", "romanian" => "RO", "russian" => "RU", "spanish" => "ES", "swedish" => "SV", "turkish" => "TR"];
    return array_key_exists($language, $languages) ? $languages[$language] : "EN";
}

function _skrill_one_tap_request($postFields)
{
    ...
    $response = $response["RESPONSE"];
    if (array_key_exists("ERROR", $response)) {
        if (in_array($response["ERROR"]["ERROR_MSG"], ["MAX_AMOUNT_REACHED"])) {
            throw new WHMCS\Exception\Billing\BillingException($response["ERROR"]["ERROR_MSG"]);
        }
        if (in_array($response["ERROR"]["ERROR_MSG"], ["ONDEMAND_CANCELLED"])) {
            throw new WHMCS\Exception\Information($response["ERROR"]["ERROR_MSG"]);
        }
        throw new WHMCS\Exception\Module\NotServicable($response["ERROR"]["ERROR_MSG"]);
    }
    $sessionId = $response["SID"];
    $postFields = ["sid" => $sessionId, "action" => "request"];
    $rawResponse = curlCall($url, $postFields);
    if (substr($rawResponse, 0, 10) == "CURL Error") {
        throw new WHMCS\Exception\Module\NotServicable($rawResponse);
    }
