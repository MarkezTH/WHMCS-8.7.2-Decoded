...
class mpgGlobals
{
    public $Globals = ["MONERIS_PROTOCOL" => "https", "MONERIS_HOST" => "esqa.moneris.com", "MONERIS_PORT" => "443", "MONERIS_FILE" => "/gateway2/servlet/MpgRequest", "API_VERSION" => "MpgApi Version 2.03(php)", "CLIENT_TIMEOUT" => "60"];
...
class mpgRequest
{
    protected $txnTypes = ["purchase" => ["order_id", "cust_id", "amount", "pan", "expdate", "crypt_type"], "refund" => ["order_id", "amount", "txn_number", "crypt_type"], "idebit_purchase" => ["order_id", "cust_id", "amount", "idebit_track2"], "idebit_refund" => ["order_id", "amount", "txn_number"], "ind_refund" => ["order_id", "cust_id", "amount", "pan", "expdate", "crypt_type"], "preauth" => ["order_id", "cust_id", "amount", "pan", "expdate", "crypt_type"], "completion" => ["order_id", "comp_amount", "txn_number", "crypt_type"], "purchasecorrection" => ["order_id", "txn_number", "crypt_type"], "opentotals" => ["ecr_number"], "batchclose" => ["ecr_number"], "cavv_purchase" => ["order_id", "cust_id", "amount", "pan", "expdate", "cavv"], "cavv_preauth" => ["order_id", "cust_id", "amount", "pan", "expdate", "cavv"]];
    ...
    public function toXML()
    {
        ...
        $txnType = array_shift($transactionData);
        $tmpTxnTypes = $this->txnTypes;
        $txnTypeArray = $tmpTxnTypes[$txnType];
        $txnTypeArrayLen = count($txnTypeArray);
        $txnXMLString = "";
        for ($i = 0; $i < $txnTypeArrayLen; $i++) {
            $txnXMLString .= "<" . $txnTypeArray[$i] . ">" . $transactionData[$txnTypeArray[$i]] . "</" . $txnTypeArray[$i] . ">";
        }
        $txnXMLString = "<" . $txnType . ">" . $txnXMLString;
        $recur = $this->transaction->getRecur();
        if ($recur != NULL) {
            $txnXMLString .= $recur->toXML();
        }
        $avsInfo = $this->transaction->getAvsInfo();
        if ($avsInfo != NULL) {
            $txnXMLString .= $avsInfo->toXML();
        }
        $cvdInfo = $this->transaction->getCvdInfo();
        if ($cvdInfo != NULL) {
            $txnXMLString .= $cvdInfo->toXML();
        }
        $custInfo = $this->transaction->getCustInfo();
        if ($custInfo != NULL) {
...
{
    $configarray = ["FriendlyName" => ["Type" => "System", "Value" => "Moneris"], "store_id" => ["FriendlyName" => "Store ID", "Type" => "text", "Size" => "12", "Description" => "A value that identifies your company when your send a transaction"], "api_token" => ["FriendlyName" => "API token", "Type" => "text", "Size" => "20", "Description" => "A unique key that when matched with your store_id creates a secure method of authenticating your store_id"], "order_id_format" => ["FriendlyName" => "Order ID format", "Type" => "text", "Size" => "20", "Description" => "Enter the format for the Moneris order_id numbers eg. WHMCS-%s. Token will be replaced with actual invoice id."], "testmode" => ["FriendlyName" => "Test Environment", "Type" => "yesno", "Description" => "When set, the transaction will be a test transaction only"]];
    return $configarray;
}

function moneris_capture($params)
{
    $txnArray = ["type" => "purchase", "crypt_type" => 7];
    $store_id = $params["testmode"] ? "store5" : $params["store_id"];
...
{
    $txnArray = ["type" => "ind_refund", "crypt_type" => 7];
