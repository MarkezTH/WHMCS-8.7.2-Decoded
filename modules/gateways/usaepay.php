...
function _usaepay_calculate_UMhash($params, $postfields)
{
    ...
    $hashBaseParts = [$postfields["UMcommand"], $params["pin"], $postfields["UMamount"], $postfields["UMinvoice"], $seed];
...
function usaepay_capture($params)
{
    ...
    if ($tmp["UMresult"] == "A") {
        return ["status" => "success", "transid" => $tmp["UMrefNum"], "rawdata" => $tmp];
    }
    if ($tmp["UMresult"] == "E") {
        return ["status" => "error", "rawdata" => $tmp];
    }
    return ["status" => "declined", "rawdata" => $tmp];
}
