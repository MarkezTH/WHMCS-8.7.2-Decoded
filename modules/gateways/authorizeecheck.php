...
{
    return ["FriendlyName" => ["Type" => "System", "Value" => "Authorize.net Echeck"], "loginid" => ["FriendlyName" => "Login ID", "Type" => "text", "Size" => "20"], "transkey" => ["FriendlyName" => "Transaction Key", "Type" => "password", "Size" => "20"], "testMode" => ["FriendlyName" => "Test Mode", "Type" => "yesno"]];
}
...
function authorizeecheck_capture($params)
{
    ...
    if ($resultsarray["Response Code"] == 1) {
        return ["status" => "success", "transid" => $resultsarray["Transaction ID"], "rawdata" => $resultsarray];
    }
    return ["status" => "declined", "rawdata" => $resultsarray, "declinereason" => $resultsarray["Response Reason Text"]];
}
