...
{
    $configarray = ["FriendlyName" => ["Type" => "System", "Value" => "Quantum Gateway"], "loginid" => ["FriendlyName" => "Login ID", "Type" => "text", "Size" => "20"], "transkey" => ["FriendlyName" => "Restrict Key", "Type" => "text", "Size" => "20", "Description" => "In the Processing Settings area of your QG Account"], "md5hash" => ["FriendlyName" => "MD5 Hash", "Type" => "text", "Size" => "20", "Description" => "In the Processing Settings area of your QG Account"], "maxmind" => ["FriendlyName" => "MaxMind Fraud Control", "Type" => "yesno", "Description" => "Check to use MaxMind Fraud Control"]];
    return $configarray;
}
...
function quantumgateway_refund($params)
{
    if (!$params["cardlastfour"]) {
        ...
        if ($preresults["QGWREQUEST"]["RESULT"]["PAYMENTTYPE"] == "CC") {
            ...
            return ["status" => "error", "rawdata" => "Original Payment not made by CC " . $predata];
        }
    }
    $url = "https://secure.quantumgateway.com/cgi/xml_requester.php";
