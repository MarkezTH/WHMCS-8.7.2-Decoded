...
{
    $configarray = ["FriendlyName" => ["Type" => "System", "Value" => "WorldPay FuturePay"], "installationid" => ["FriendlyName" => "Installation ID", "Type" => "text", "Size" => "20", "Description" => "Installation ID for Signups"], "installationidcharges" => ["FriendlyName" => "Installation ID for Charges", "Type" => "text", "Size" => "20", "Description" => "(Optional)"], "authpw" => ["FriendlyName" => "Auth Password", "Type" => "text", "Size" => "20", "Description" => ""], "authmode" => ["FriendlyName" => "Auth Mode", "Type" => "yesno", "Description" => "Check to enable authorizations only"], "testmode" => ["FriendlyName" => "Test Mode", "Type" => "yesno"]];
    return $configarray;
}
...
function worldpayfuturepay_capture($params)
{
    ...
    if (substr($gatewayresult, 0, 1) == "Y") {
        $returndata = explode(",", $gatewayresult);
        return ["status" => "success", "transid" => $returndata[1], "rawdata" => $returndata];
    }
    return ["status" => "failed", "rawdata" => $returndata];
}
...
function worldpayfuturepay_adminstatusmsg($vars)
{
    ...
    if ($futurepayid) {
        return ["type" => "info", "title" => "FuturePay Agreement ID", "msg" => "This customer has a WorldPay FuturePay Agreement setup for automated billing with ID " . $futurepayid];
    }
}
