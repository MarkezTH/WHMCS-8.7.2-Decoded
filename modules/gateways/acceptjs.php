...
{
    return ["RemoteInputFields" => false];
}

function acceptjs_config()
{
    return ["FriendlyName" => ["Type" => "System", "Value" => "Authorize.net Accept.js"], "apiLoginId" => ["FriendlyName" => "API Login ID", "Type" => "text", "Size" => "20", "Description" => "This can be found by navigating to Account > Security Settings > API Credentials & Keys within your Authorize.net account."], "transactionKey" => ["FriendlyName" => "Transaction Key", "Type" => "password", "Size" => "20", "Description" => "This can be found by navigating to Account > Security Settings > API Credentials & Keys within your Authorize.net account."], "publicKey" => ["FriendlyName" => "Public Client Key", "Type" => "text", "Size" => "40", "Description" => "This can be found by navigating to Account > Security Settings > Manage Public Client Key within your Authorize.net account."], "testMode" => ["FriendlyName" => "Test Mode", "Type" => "yesno"], "noAccount" => ["Type" => "info", "Description" => "<div class=\"alert alert-info\" style=\"margin-bottom: 0;\">Don't have an account? <a href=\"https://www.whmcs.com/start-accepting-credit-cards\" class=\"alert-link autoLinked\">Apply for an account for free</a></div>"]];
}

function acceptjs_config_validate($params = [])
{
...
function acceptjs_capture($params = [])
{
        ...
        if ((!is_array($gatewayId) || json_last_error() !== JSON_ERROR_NONE) && $params["cardnum"]) {
            try {
                ...
                if ($response != NULL && $response->getMessages()->getResultCode() == "Ok") {
                    ...
                    return ["status" => "error", "rawdata" => "Invalid Response: " . $errorMessage];
                }
            } catch (Exception $e) {
                return ["status" => "error", "rawdata" => "Invalid Response: " . $e->getMessage()];
            }
        }
    }
    if (!is_array($gatewayId) || !($gatewayId["customer"] && $gatewayId["payment"])) {
        return ["status" => "error", "rawdata" => "No Data Stored for Authorize.net Accept JS"];
    }
    try {
        $merchantAuthentication = new net\authorize\api\contract\v1\MerchantAuthenticationType();
        $merchantAuthentication->setName($params["apiLoginId"]);
        ...
        if ($response != NULL) {
            if ($response->getMessages()->getResultCode() == "Ok") {
                ...
                if ($tresponse != NULL && $tresponse->getMessages() != NULL) {
                    ...
                    return ["status" => "success", "transid" => $tresponse->getTransId(), "amount" => $params["amount"], "rawdata" => (array) $rawData];
                } else {
                    return ["status" => "declined", "rawdata" => ["Error Code" => $tresponse->getErrors()[0]->getErrorCode(), "Error Message" => $tresponse->getErrors()[0]->getErrorText()]];
                }
            } else {
                $tresponse = $response->getTransactionResponse();
                ...
                return ["status" => "error", "rawdata" => ["Error Code" => $errorCode, "Error Message" => $errorMessage]];
            }
        } else {
            return ["status" => "error", "rawdata" => ["Error" => "No response returned \n"]];
        }
    } catch (Exception $e) {
        return ["status" => "error", "rawdata" => ["File Error" => $e->getFile(), "File Number" => $e->getLine(), "Error Message" => $e->getMessage()]];
    }
}

function acceptjs_refund($params = [])
{
    $gatewayId = json_decode(WHMCS\Input\Sanitize::decode($params["gatewayid"]), true);
    $apiUrl = net\authorize\api\constants\ANetEnvironment::PRODUCTION;
    if ($params["testMode"]) {
...
function acceptjs_storeremote($params = [])
{
        ...
        if ($dataValue && $dataDescriptor) {
            try {
                ...
                if ($response != NULL && $response->getMessages()->getResultCode() == "Ok") {
                    ...
                    $gatewayId = ["customer" => $customerProfileId, "payment" => $customerPaymentProfileId];
                    ...
                    if ($response != NULL) {
                        if ($response->getMessages()->getResultCode() == "Ok") {
                            ...
                            return ["nodelete" => true, "cardlastfour" => $cardLastFour, "cardtype" => $cardType, "cardexpiry" => $cardExpiry, "gatewayid" => json_encode($gatewayId), "status" => "success", "rawdata" => $rawData];
                        } else {
                            $errorMessages = $response->getMessages()->getMessage();
                ...
                return ["status" => "error", "rawdata" => ["dataDescriptor" => $dataDescriptor, "dataValue" => $dataValue, "error" => $e->getMessage()]];
            }
        }
    } else {
            ...
            if ($gatewayId && is_array($gatewayId) && $params["action"] === "update") {
                try {
                    ...
                    if ($response != NULL && $response->getMessages()->getResultCode() == "Ok") {
                        ...
                        return ["nodelete" => true, "cardlastfour" => $payment->getLastFour(), "cardtype" => $payment->getCardType(), "cardexpiry" => $params["cardexp"], "gatewayid" => json_encode($gatewayId), "status" => "success", "rawdata" => $rawData];
                    } else {
                        $errorMessages = $response->getMessages()->getMessage();
                        throw new WHMCS\Exception("Response : " . $errorMessages[0]->getCode() . "  " . $errorMessages[0]->getText());
                    }
                ...
                if ($gatewayId && is_array($gatewayId) && $params["action"] === "delete") {
                    try {
                        ...
                        return ["status" => "success", "rawdata" => $rawData];
                    } catch (Exception $e) {
    ...
    return ["status" => "error", "rawdata" => ["dataDescriptor" => $dataDescriptor, "dataValue" => $dataValue, "error" => "An unknown Error Occurred"]];
}

function acceptjs_post_checkout($params = [])
{
    try {
        $dataDescriptor = WHMCS\Session::getAndDelete("dataDescriptor");
        $dataValue = WHMCS\Session::getAndDelete("dataValue");
            ...
            if ($response != NULL) {
                if ($response->getMessages()->getResultCode() == "Ok") {
                    ...
                    if ($tresponse != NULL && $tresponse->getMessages() != NULL) {
                        ...
                        $returnData["rawdata"] = ["Error Code" => $tresponse->getErrors()[0]->getErrorCode(), "Error Message" => $tresponse->getErrors()[0]->getErrorText()];
                    }
                } else {
                    $tresponse = $response->getTransactionResponse();
                    if ($tresponse != NULL && $tresponse->getErrors() != NULL) {
                    ...
                    $returnData["rawdata"] = ["Error Code" => $errorCode, "Error Message" => $errorMessage];
                }
            }
        }
        return $returnData;
...
function acceptjs_cc_validation($params = [])
{
...
function acceptjs_credit_card_input($params = [])
{
