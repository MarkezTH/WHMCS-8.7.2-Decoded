...
if (isset($_POST["ajax"]) && $_POST["ajax"] == "true") {
    $response = _quantumvault_http_post("secure.quantumgateway.com", "/cgi/ilf_refresh.php", ["ip" => $_POST["ip"], "k" => $_POST["k"]], 443);
}

function quantumvault_config()
{
    return ["FriendlyName" => ["Type" => "System", "Value" => "Quantum Vault"], "loginid" => ["FriendlyName" => "Login ID", "Type" => "text", "Size" => "20"], "transkey" => ["FriendlyName" => "Restrict Key", "Type" => "text", "Size" => "20", "Description" => "In the Processing Settings area of your QG Account"], "apiusername" => ["FriendlyName" => "API Username", "Type" => "text", "Size" => "20", "Description" => "Go to Processing Settings > Inline Frame API, set API Enabled = Y and generate Username & API Key"], "apikey" => ["FriendlyName" => "API Key", "Type" => "text", "Size" => "20"], "vaultkey" => ["FriendlyName" => "Vault Key", "Type" => "text", "Size" => "20", "Description" => "Set in Processing Tools > Secure Vault > Vault Config"], "md5hash" => ["FriendlyName" => "MD5 Hash", "Type" => "text", "Size" => "20", "Description" => "Also in the Processing Settings area of your Quantum Account"], "testmode" => ["FriendlyName" => "Test Module", "Type" => "yesno"]];
}
...
{
    if (!$params["gatewayid"]) {
        return ["status" => "failed", "rawdata" => "No Card Stored for this Client in Vault"];
    }
    ...
    if ($results["QGWREQUEST"]["RESULT"]["STATUS"] == "APPROVED") {
        return ["status" => "success", "transid" => $results["QGWREQUEST"]["RESULT"]["TRANSACTIONID"], "rawdata" => $results["QGWREQUEST"]["RESULT"]];
    }
...
{
    $thereturn = [];
    $random = rand(1111111111, 0);
    $random = (int) $random;
    $response = _quantumvault_http_post("secure.quantumgateway.com", "/cgi/ilf_authenticate.php", ["API_Username" => $API_Username, "API_Key" => $API_Key, "randval" => $random, "lastip" => $_SERVER["REMOTE_ADDR"]], 443);
...
function quantumvault_adminstatusmsg($vars)
{
    ...
    if ($gatewayid) {
        return ["type" => "info", "title" => "Quantum Vault Profile", "msg" => "This customer has a Quantum Vault Profile storing their card details for automated recurring billing with ID " . $gatewayid];
    }
}
