...
function securepayau_capture($params)
{
    ...
    if ($responseCode == "00" || $responseCode == "08" || $responseCode == "77") {
        return ["status" => "success", "transid" => $txnID, "rawdata" => $transreport];
    }
    return ["status" => "declined", "rawdata" => $transreport];
}
...
function securepayau_makeXMLTree($rawxml)
{
    ...
    $options = [XML_OPTION_CASE_FOLDING => 0, XML_OPTION_SKIP_WHITE => 1];
