...
class monerisvault_mpgHttpsPost
{
    ...
    public function __construct($environment, $storeid, $apitoken, $monerisvault_mpgRequestOBJ)
    {
        ...
        if ($environment == "live") {
            $globalarr = ["MONERIS_PROTOCOL" => "https", "MONERIS_HOST" => "www3.moneris.com", "MONERIS_PORT" => "443", "MONERIS_FILE" => "/gateway2/servlet/MpgRequest", "API_VERSION" => " CA PHP Api v.2.0.4 (Resolver)", "CLIENT_TIMEOUT" => "60"];
        } else {
            $globalarr = ["MONERIS_PROTOCOL" => "https", "MONERIS_HOST" => "esqa.moneris.com", "MONERIS_PORT" => "443", "MONERIS_FILE" => "/gateway2/servlet/MpgRequest", "API_VERSION" => " CA PHP Api v.2.0.4 (Resolver)", "CLIENT_TIMEOUT" => "60"];
        }
...
{
    public $txnTypes = ["purchase" => ["order_id", "cust_id", "amount", "pan", "expdate", "crypt_type"], "refund" => ["order_id", "amount", "txn_number", "crypt_type"], "idebit_purchase" => ["order_id", "cust_id", "amount", "idebit_track2"], "idebit_refund" => ["order_id", "amount", "txn_number"], "ind_refund" => ["order_id", "cust_id", "amount", "pan", "expdate", "crypt_type"], "preauth" => ["order_id", "cust_id", "amount", "pan", "expdate", "crypt_type"], "completion" => ["order_id", "comp_amount", "txn_number", "crypt_type"], "purchasecorrection" => ["order_id", "txn_number", "crypt_type"], "opentotals" => ["ecr_number"], "batchclose" => ["ecr_number"], "cavv_purchase" => ["order_id", "cust_id", "amount", "pan", "expdate", "cavv"], "cavv_preauth" => ["order_id", "cust_id", "amount", "pan", "expdate", "cavv"], "recur_update" => ["order_id", "cust_id", "pan", "expdate", "recur_amount", "add_num_recurs", "total_num_recurs", "hold", "terminate"], "res_add_cc" => ["cust_id", "phone", "email", "note", "pan", "expdate", "crypt_type"], "res_update_cc" => ["data_key", "cust_id", "phone", "email", "note", "pan", "expdate", "crypt_type"], "res_delete" => ["data_key"], "res_lookup_full" => ["data_key"], "res_lookup_masked" => ["data_key"], "res_get_expiring" => [], "res_purchase_cc" => ["data_key", "order_id", "cust_id", "amount", "crypt_type"], "res_preauth_cc" => ["data_key", "order_id", "cust_id", "amount", "crypt_type"], "res_ind_refund_cc" => ["data_key", "order_id", "cust_id", "amount", "crypt_type"], "res_iscorporatecard" => ["data_key"]];
...
{
    public $level3template = ["cust_info" => ["email", "instructions", "billing" => ["first_name", "last_name", "company_name", "address", "city", "province", "postal_code", "country", "phone_number", "fax", "tax1", "tax2", "tax3", "shipping_cost"], "shipping" => ["first_name", "last_name", "company_name", "address", "city", "province", "postal_code", "country", "phone_number", "fax", "tax1", "tax2", "tax3", "shipping_cost"], "item" => ["name", "quantity", "product_code", "extended_amount"]]];
...
{
    public $params = NULL;
    public $avsTemplate = ["avs_street_number", "avs_street_name", "avs_zipcode"];
...
{
    $configarray = ["FriendlyName" => ["Type" => "System", "Value" => "Moneris Vault"], "store_id" => ["FriendlyName" => "Store ID", "Type" => "text", "Size" => "20", "Description" => "The Store ID for your Moneris Account"], "api_token" => ["FriendlyName" => "API Token", "Type" => "text", "Size" => "20", "Description" => "The API Token for authenticating with the API"], "testmode" => ["FriendlyName" => "Test Mode", "Type" => "yesno"]];
    return $configarray;
}
...
{
    if (!$params["action"]) {
        return ["status" => "skipped", "rawdata" => "No Action Found. Skipped"];
    }
    ...
    if ($params["action"] == "delete") {
        $type = "res_delete";
        $txnArray = ["type" => $type, "data_key" => $data_key];
        ...
        if ($monerisvault_mpgResponse->getResponseCode() != "null" && $monerisvault_mpgResponse->getResponseCode() < 50) {
            return ["status" => "success", "rawdata" => $monerisvault_mpgResponse->responseData];
        }
        return ["status" => "failed", "rawdata" => $monerisvault_mpgResponse->responseData];
    }
    if ($params["action"] == "update") {
        $type = "res_update_cc";
        $txnArray = ["type" => $type, "data_key" => $data_key, "cust_id" => $cust_id, "phone" => $phone, "email" => $email, "note" => $note, "pan" => $pan, "expdate" => $expiry_date, "crypt_type" => $crypt_type];
        $avsTemplate = ["avs_street_number" => $avs_street_number, "avs_street_name" => $avs_street_name, "avs_zipcode" => $avs_zipcode];
        ...
        $monerisvault_mpgResponse = $mpgHttpPost->getmonerisvault_mpgResponse();
        if ($monerisvault_mpgResponse->getResponseCode() != "null" && $monerisvault_mpgResponse->getResponseCode() < 50) {
            return ["status" => "success", "gatewayid" => $monerisvault_mpgResponse->getDataKey(), "rawdata" => $monerisvault_mpgResponse->responseData];
        }
        return ["status" => "failed", "rawdata" => $monerisvault_mpgResponse->responseData];
    }
    if ($params["action"] == "create") {
        $type = "res_add_cc";
        $txnArray = ["type" => $type, "cust_id" => $cust_id, "phone" => $phone, "email" => $email, "note" => $note, "pan" => $pan, "expdate" => $expiry_date, "crypt_type" => $crypt_type];
        $avsTemplate = ["avs_street_number" => $avs_street_number, "avs_street_name" => $avs_street_name, "avs_zipcode" => $avs_zipcode];
        $monerisvault_mpgAvsInfo = new monerisvault_mpgAvsInfo($avsTemplate);
        $mpgTxn = new monerisvault_mpgTransaction($txnArray);
        $mpgTxn->setAvsInfo($monerisvault_mpgAvsInfo);
        ...
        $monerisvault_mpgResponse = $mpgHttpPost->getmonerisvault_mpgResponse();
        if ($monerisvault_mpgResponse->getResponseCode() != "null" && $monerisvault_mpgResponse->getResponseCode() < 50) {
            return ["status" => "success", "gatewayid" => $monerisvault_mpgResponse->getDataKey(), "rawdata" => $monerisvault_mpgResponse->responseData["Message"]];
        }
        return ["status" => "failed", "rawdata" => $monerisvault_mpgResponse->responseData["Message"]];
    }
}
...
function monerisvault_capture($params)
{
    ...
    if (!$params["gatewayid"] && $params["cardnum"]) {
        ...
        if ($storeremotedata["status"] == "success") {
            ...
            if ($params["gatewayid"]) {
                ...
                return ["status" => "failed", "rawdata" => "Error storing card with Moneris Vault. Received Empty Datakey. Capture Skipped. Error Detail: " . $storeremotedata["rawdata"]];
            }
        } else {
            return ["status" => "failed", "rawdata" => "Error storing card with Moneris Vault. Capture Skipped. Error Detail: " . $storeremotedata["rawdata"]];
        }
    }
    if (!$params["gatewayid"] && !$params["cardnum"]) {
        return ["status" => "skipped", "rawdata" => "No Card is stored with Moneris Vault. Capture Skipped. Try adding a card first"];
    }
    $store_id = $params["store_id"];
    $api_token = $params["api_token"];
    $data_key = $params["gatewayid"];
    ...
    $crypt_type = "1";
    $txnArray = ["type" => "res_purchase_cc", "data_key" => $data_key, "order_id" => $orderid, "cust_id" => $custid, "amount" => $amount, "crypt_type" => $crypt_type];
    $mpgTxn = new monerisvault_mpgTransaction($txnArray);
    $monerisvault_mpgRequest = new monerisvault_mpgRequest($mpgTxn);
    $mpgHttpPost = new monerisvault_mpgHttpsPost($environment, $store_id, $api_token, $monerisvault_mpgRequest);
    $monerisvault_mpgResponse = $mpgHttpPost->getmonerisvault_mpgResponse();
    if ($monerisvault_mpgResponse->getResponseCode() != "null" && $monerisvault_mpgResponse->getResponseCode() < 50) {
        return ["status" => "success", "transid" => $monerisvault_mpgResponse->getReferenceNum(), "rawdata" => $monerisvault_mpgResponse->responseData["Message"]];
    }
    return ["status" => "declined", "rawdata" => $monerisvault_mpgResponse->responseData["Message"]];
}
...
function monerisvault_refund($params)
{
    ...
    $custid = $params["clientdetails"]["userid"];
    $crypt_type = "1";
    $txnArray = ["type" => "res_ind_refund_cc", "data_key" => $data_key, "order_id" => $orderid, "cust_id" => $custid, "amount" => $amount, "crypt_type" => $crypt_type];
    $mpgTxn = new monerisvault_mpgTransaction($txnArray);
    $monerisvault_mpgRequest = new monerisvault_mpgRequest($mpgTxn);
    $mpgHttpPost = new monerisvault_mpgHttpsPost($environment, $store_id, $api_token, $monerisvault_mpgRequest);
    $monerisvault_mpgResponse = $mpgHttpPost->getmonerisvault_mpgResponse();
    ...
    return ["status" => "declined", "rawdata" => safe_serialize($monerisvault_mpgResponse->responseData["Message"])];
}
