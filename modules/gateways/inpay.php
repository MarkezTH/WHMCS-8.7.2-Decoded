...
{
    $configarray = ["FriendlyName" => ["Type" => "System", "Value" => "inpay"], "username" => ["FriendlyName" => "Merchant ID", "Type" => "text", "Size" => "20"], "secretkey" => ["FriendlyName" => "Secret ket", "Type" => "text", "Size" => "20"], "testmode" => ["FriendlyName" => "Test Mode", "Type" => "yesno", "Description" => "Use test gateway"]];
    return $configarray;
}
...
{
    $sk = $order["secretkey"];
    $q = http_build_query(["merchant_id" => $order["username"], "order_id" => $order["invoiceid"], "amount" => $order["amount"], "currency" => $order["currency"], "order_text" => $order["description"], "flow_layout" => "multi_page", "secret_key" => $sk], "", "&");
    $md5v = md5($q);
    return $md5v;
}
