...
{
    return ["FriendlyName" => ["Type" => "System", "Value" => "Authorize.net CIM"], "loginid" => ["FriendlyName" => "Login ID", "Type" => "text", "Size" => "25"], "transkey" => ["FriendlyName" => "Transaction Key", "Type" => "password", "Size" => "25"], "validationmode" => ["FriendlyName" => "Validation Mode", "Type" => "dropdown", "Options" => "none,live"], "testmode" => ["FriendlyName" => "Test Mode", "Type" => "yesno"]];
}

function authorizecim_config_validate($params = [])
{
...
function authorizecim_capture($params)
{
    ...
    if (!$gatewayids[0]) {
        return ["status" => "error", "rawdata" => "No Client Profile ID Found"];
    }
    if (!$gatewayids[1]) {
        return ["status" => "error", "rawdata" => "No Client Payment Profile ID Found"];
    }
    ...
    if ($nameaddresshash != $storednameaddresshash) {
        ...
        $data = curlCall($url, $xml, ["HEADER" => ["Content-Type: text/xml"]]);
    ...
    if ($xmldata["CREATECUSTOMERPROFILETRANSACTIONRESPONSE"]["MESSAGES"]["RESULTCODE"] == "Ok") {
        ...
        $transid = $transid[6];
        return ["status" => "success", "transid" => $transid, "rawdata" => $data];
    }
    $response = $xmldata["CREATECUSTOMERPROFILETRANSACTIONRESPONSE"]["TRANSACTIONRESPONSE"];
    return ["status" => "error", "rawdata" => $data, "declinereason" => $response["ERRORS"]["ERROR"]["ERRORTEXT"]];
}
...
function authorizecim_storeremote($params)
{
    ...
    if (!empty($params["gatewayid"])) {
        $gatewayids = explode(",", (string) $params["gatewayid"]);
    }
    if ($params["action"] == "delete") {
        ...
        $debugdata = ["Action" => "DeleteCard", "XMLData" => $data];
        return ["status" => $status, "rawdata" => $debugdata];
    }
    $address = authorizecim_addressFix($params["clientdetails"]["address1"]);
    if ($params["action"] == "update") {
        ...
        $data = curlCall($url, $xml, ["HEADER" => ["Content-Type: text/xml"]]);
        $xmldata = XMLtoArray($data);
        $debugdata = ["Action" => "UpdateCustomer", "XMLData" => $data];
        if ($xmldata["UPDATECUSTOMERPAYMENTPROFILERESPONSE"]["MESSAGES"]["RESULTCODE"] == "Ok") {
            ...
            return ["status" => "success", "gatewayid" => $gatewayid, "cardexpiry" => $params["cardexp"], "rawdata" => $debugdata];
        }
        if ($xmldata["UPDATECUSTOMERPAYMENTPROFILERESPONSE"]["MESSAGES"]["MESSAGE"]["TEXT"] == "The record cannot be found.") {
            $params["gatewayid"] = "";
        } else {
            return ["status" => "failed", "rawdata" => $debugdata];
        }
    }
    if ($params["action"] == "create") {
        ...
        $data = curlCall($url, $xml, ["HEADER" => ["Content-Type: text/xml"]]);
        $xmldata = XMLtoArray($data);
        $debugdata = ["Action" => "CreateCustomer", "XMLData" => $data];
        if ($xmldata["CREATECUSTOMERPROFILERESPONSE"]["MESSAGES"]["RESULTCODE"] == "Ok") {
            ...
            return ["status" => "success", "gatewayid" => $gatewayid, "rawdata" => $debugdata];
        }
        if ($xmldata["CREATECUSTOMERPROFILERESPONSE"]["MESSAGES"]["MESSAGE"]["CODE"] == "E00039") {
        }
        return ["status" => "failed", "rawdata" => $debugdata];
    }
    return ["status" => "skipped", "rawdata" => ["Error" => "No Action Found"]];
}
...
function authorizecim_adminstatusmsg($vars)
{
    ...
    if ($gatewayids) {
        ...
        return ["type" => "info", "title" => "Authorize.net CIM Profile", "msg" => "This customer has a remote Authorize.net CIM Profile storing their card details for automated recurring billing with ID " . $gatewayids[0]];
    }
}
