...
{
    $configarray = ["FriendlyName" => ["Type" => "System", "Value" => "Merchant Warrior"], "merchantUUID" => ["FriendlyName" => "Merchant UUID", "Type" => "text", "Size" => "20"], "apiKey" => ["FriendlyName" => "API Key", "Type" => "text", "Size" => "20"], "apiPassphrase" => ["FriendlyName" => "API Passphrase", "Type" => "text", "Size" => "20"], "testmode" => ["FriendlyName" => "Test Mode", "Type" => "yesno", "Description" => "Check to send requests to the test environment."]];
    return $configarray;
}
...
function mwarrior_capture($params)
{
    ...
    $results = ["status" => $status, "transID" => isset($xml["transactionID"]) ? $xml["transactionID"] : NULL, "transAmount" => $params["amount"], "endpoint" => $endpoint, "xml" => $xmlObj instanceof SimpleXMLElement ? $xmlObj->asXML() : NULL];
    if ($results["status"] == "success") {
        $tarnsID = $results["transID"] . "|" . $params["amount"];
        return ["status" => "success", "transid" => $transID, "rawdata" => $results];
    }
    if ($results["status"] == "declined") {
        return ["status" => "declined", "rawdata" => $results];
    }
...
function mwarrior_refund($params)
{
    ...
    $results = ["status" => $status, "transID" => isset($xml["transactionID"]) ? $xml["transactionID"] : NULL, "endpoint" => $endpoint, "xml" => $xmlObj instanceof SimpleXMLElement ? $xmlObj->asXML() : NULL];
    if ($results["status"] == "success") {
