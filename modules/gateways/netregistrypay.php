...
$GATEWAYMODULE = ["netregistrypayname" => "netregistrypay", "netregistrypayvisiblename" => "Netregistry Pay", "netregistrypaytype" => "CC"];
...
function netregistrypay_capture($params)
{
    ...
    $txnref = "Unknown";
    $params = ["COMMAND" => "purchase", "LOGIN" => $gatewayusername . "/" . $gatewaypassword, "AMOUNT" => number_format($amount, 2, ".", ""), "CCNUM" => $cardnumber, "CCEXP" => substr($cardexpiry, 0, 2) . "/" . substr($cardexpiry, 2, 3), "COMMENT" => $firstname . $lastname . " WHMCS Invoice ID:" . $invoiceid];
    $curl = curl_init();
    ...
    if (isset($success) && $success) {
        return ["status" => "success", "transid" => $txnref, "rawdata" => $result];
    }
    if (isset($declined) && $declined) {
        return ["status" => "declined", "rawdata" => $result];
    }
