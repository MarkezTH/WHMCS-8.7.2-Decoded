...
function payflowpro_MetaData($params = [])
{
    $metaData = ["DisplayName" => "PayPal Payflow Pro", "APIVersion" => "1.1"];
    $useReference = WHMCS\Module\GatewaySetting::getValue("payflowpro", "usereftrans");
    if ($useReference) {
...
{
    return ["FriendlyName" => ["Type" => "System", "Value" => "Payflow Pro (PayPal)"], "partner" => ["FriendlyName" => "Partner", "Type" => "text", "Size" => "20"], "vendor" => ["FriendlyName" => "Merchant Login", "Type" => "text", "Size" => "40"], "username" => ["FriendlyName" => "Username", "Type" => "text", "Size" => "40"], "password" => ["FriendlyName" => "Password", "Type" => "text", "Size" => "40"], "processorid" => ["FriendlyName" => "Processor ID", "Type" => "text", "Size" => "20", "Description" => "Cardinal 3D Secure Details"], "merchantid" => ["FriendlyName" => "Merchant ID", "Type" => "text", "Size" => "20"], "transpw" => ["FriendlyName" => "Transaction PW", "Type" => "text", "Size" => "20"], "apiusername" => ["FriendlyName" => "API Username", "Type" => "text", "Size" => "40", "Description" => "API fields only required for refunds"], "apipassword" => ["FriendlyName" => "API Password", "Type" => "text", "Size" => "40"], "apisignature" => ["FriendlyName" => "API Signature", "Type" => "text", "Size" => "40"], "usereftrans" => ["FriendlyName" => "Use Reference Transactions", "Type" => "yesno", "Description" => "Check to enable reference transactions and not store card details locally"], "payflowmode" => ["FriendlyName" => "PayFlow Mode", "Type" => "yesno", "Description" => "Check this if you don't use a PayPal merchant account"], "testmode" => ["FriendlyName" => "Test Mode", "Type" => "yesno"]];
}
...
function payflowpro_3dsecure($params)
{
    ...
    if ($params["processorid"]) {
        ...
        $storage->setValue("order_data", ["Centinel_OrderId" => $orderid, "Centinel_TransactionId" => $transid]);
    }
    if ($errorno == 0) {
        if ($enrolled == "Y") {
            logTransaction($params["paymentmethod"], $xmlarray, "3D Auth");
            $storage->setValue("Centinel_Details", ["cardtype" => $params["cardtype"], "cardnum" => $params["cardnum"], "cardexp" => $params["cardexp"], "cccvv" => $params["cccvv"], "cardstart" => $params["cardstart"], "cardissuenum" => $params["cardissuenum"]]);
...
function payflowpro_capture($params, $auth = "")
{
    ...
    if ($pfpro["RESULT"] == "0") {
        ...
        return ["status" => "success", "transid" => $transid, "rawdata" => $pfpro];
    }
    return ["status" => "declined", "rawdata" => $pfpro, "declinereason" => $pfpro["RESPMSG"]];
}
...
function payflowpro_refund($params)
{
    if ($params["payflowmode"]) {
        ...
        if ($pfpro["RESULT"] == "0") {
            return ["status" => "success", "rawdata" => $pfpro, "transid" => $pfpro["PNREF"]];
        }
        return ["status" => "error", "rawdata" => $pfpro];
    } else {
