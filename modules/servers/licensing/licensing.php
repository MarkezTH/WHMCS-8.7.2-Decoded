...
{
    return ["DisplayName" => "License Software", "APIVersion" => "1.1", "RequiresServer" => false, "addonLicenseRequired" => true, "addonLicenseName" => "Licensing Addon", "AutoGenerateUsernameAndPassword" => false];
}

function licensing_ConfigOptions()
{
    if (!LICENSINGADDONLICENSE) {
        return ["License Required" => ["Type" => "na", "Description" => "You need to purchase the licensing addon from <a href=\"http://go.whmcs.com/94/licensing-addon\" target=\"_blank\">www.whmcs.com/addons/licensing-addon</a> before you can use this functionality. If you just purchased it recently, please <a href=\"configproducts.php?action=edit&id=" . App::getFromRequest("id") . "&tab=3&larefresh=1\">click here</a> " . "to refresh this message"]];
    }
    $id = App::getFromRequest("id");
    $addonsCollection = WHMCS\Database\Capsule::table("tbladdons")->orderBy("weight", "asc")->orderBy("name", "asc")->get()->all();
    $supportUpdateAddons = [];
    ...
    $configarray = ["Key Length" => ["Type" => "text", "Size" => "10", "Description" => "String Length eg. 10"], "Key Prefix" => ["Type" => "text", "Size" => "20", "Description" => "eg. Leased-"], "Allow Reissue" => ["Type" => "yesno", "Description" => "Check to allow clients to self-reissue from the client area"], "Allow Domain Conflict" => ["Type" => "yesno", "Description" => "Check to not validate Domains"], "Allow IP Conflict" => ["Type" => "yesno", "Description" => "Check to not validate IPs"], "Allow Directory Conflict" => ["Type" => "yesno", "Description" => "Check to not validate installation path"], "Support/Updates Addon" => ["Type" => "dropdown", "Options" => $supportUpdateAddons], "Secret Key" => ["Type" => "text", "Size" => "20", "Description" => "Used in MD5 Verification"], "Free Trial" => ["Type" => "yesno", "Description" => "Restricts license to one instance per Domain"]];
    return $configarray;
}
...
function licensing_CreateAccount($params)
{
    ...
    $licensekey = licensing_genkey($length, $prefix);
    WHMCS\Database\Capsule::table("mod_licensing")->insert(["serviceid" => $params["serviceid"], "addon_id" => $addonId, "licensekey" => $licensekey, "validdomain" => "", "validip" => "", "validdirectory" => "", "reissues" => "0", "status" => "Reissued"]);
    updateService(["domain" => $licensekey, "username" => "", "password" => ""]);
    ...
    if ($addonid) {
        ...
        switch ($addon->billingCycle) {
            case "recurring":
                $serviceBillingCycle = strtolower(str_replace(["-", " "], "", $hostingModel->billingCycle));
                $setupFeeField = substr($serviceBillingCycle, 0, 1) . "setupfee";
                if (!in_array($serviceBillingCycle, ["free", "freeaccount", "onetime"]) && -1 < $pricing->{$serviceBillingCycle}) {
                    $addonSetupFee = $pricing->{$setupFeeField};
                    $addonRecurring = $pricing->{$serviceBillingCycle};
                    $addonBillingCycle = $hostingModel->billingCycle;
                } else {
                    if (-1 < $pricing->monthly) {
                        if (-1 < $pricing->quarterly) {
                            if (-1 < $pricing->semiannually) {
                                if (-1 < $pricing->annually) {
                                    if (-1 < $pricing->biennially) {
                                        if (-1 < $pricing->triennially) {
                                            $addonSetupFee = 0;
                                            $addonRecurring = 0;
...
{
    $buttonarray = ["Reissue License" => "reissue", "Reset Reissues" => "reissuereset", "Revoke License" => "revoke", "Manage" => "manage"];
    return $buttonarray;
}
...
function licensing_ClientAreaCustomButtonArray($params)
{
    ...
    $buttonarray = [];
    if ($allowReissue === "on") {
        $buttonarray = ["Reissue License" => "reissue"];
    }
    return $buttonarray;
}
...
function licensing_reissue($params)
{
    ...
    WHMCS\Database\Capsule::table("mod_licensing")->where("id", "=", $license->id)->increment("reissues", 1, ["status" => "Reissued"]);
    run_hook("LicensingAddonReissue", ["licenseid" => $license->id, "serviceid" => $params["serviceid"], "addon_id" => $addonId]);
    return "success";
}
...
function licensing_reissuereset($params)
{
    ...
    WHMCS\Database\Capsule::table("mod_licensing")->where("id", "=", $license->id)->update(["reissues" => 0]);
    return "success";
}
...
function licensing_AdminServicesTabFields($params)
{
    global $aInt;
    if (!LICENSINGADDONLICENSE) {
        return ["Error" => "Your WHMCS license key is not enabled to use the Licensing Addon yet. Navigate to Addons > Licensing Manager to resolve."];
    }
    $license = licensing_get_license($params);
    if ($license) {
        ...
        $licenseLogs = WHMCS\Database\Capsule::table("mod_licensinglog")->where("licenseid", "=", $licenseId)->orderBy("id", "DESC")->limit(10)->offset(0)->get()->all();
        $tableData = [];
...
{
    update_query("mod_licensing", ["validdomain" => licensing_valid_input_clean($_POST["modulefields"][0]), "validip" => licensing_valid_input_clean($_POST["modulefields"][1]), "validdirectory" => licensing_valid_input_clean($_POST["modulefields"][2]), "status" => $_POST["modulefields"][3]], ["serviceid" => $params["serviceid"]]);
}
...
function licensing_ChangePackage($params)
{
    ...
    $addonid = $addonid[0];
    if ($addonid) {
        $currentaddon = get_query_val("tblhostingaddons", "id", ["hostingid" => $params["serviceid"], "addonid" => $addonid, "status" => "Active"]);
        if (!$currentaddon) {
            $hostingModel = $params["model"] instanceof WHMCS\Service\Addon ? $params["model"]->service : $params["model"];
            $paymentMethod = $hostingModel->paymentGateway;
            $billingCycle = strtolower(str_replace(" ", "", $hostingModel->billingCycle));
            $orderId = WHMCS\Database\Capsule::table("tblupgrades")->where("type", "=", "package")->where("relid", "=", $hostingModel->id)->where("newvalue", "=", $params["packageid"] . "," . $billingCycle)->value("orderid");
            $addon = WHMCS\Product\Addon::find($addonid);
            $pricing = WHMCS\Database\Capsule::table("tblpricing")->where("relid", "=", $addon->id)->where("type", "=", "addon")->where("currency", "=", $params["clientsdetails"]["currency"])->first();
            switch ($addon->billingCycle) {
                case "recurring":
