...
require "../../../init.php";
$result = select_query("mod_licensing", "", ["licensekey" => $_POST["licensekey"]]);
...
if (!$licenseid) {
    ...
    insert_query("mod_licensinglog", ["licenseid" => $licenseid, "domain" => $_POST["domain"], "ip" => $_POST["ip"], "path" => $_POST["dir"], "message" => "Invalid Key - " . $_POST["licensekey"], "datetime" => "now()"]);
    exit;
}
update_query("mod_licensing", ["lastaccess" => "now()"], ["id" => $licenseid]);
if ($status == "Expired") {
    echo "<status>Expired</status>";
    licensing_getlicreturndata($licenseid);
    insert_query("mod_licensinglog", ["licenseid" => $licenseid, "domain" => $_POST["domain"], "ip" => $_POST["ip"], "path" => $_POST["dir"], "message" => "License Expired", "datetime" => "now()"]);
    exit;
}
if ($status == "Suspended") {
    echo "<status>Suspended</status>";
    licensing_getlicreturndata($licenseid);
    insert_query("mod_licensinglog", ["licenseid" => $licenseid, "domain" => $_POST["domain"], "ip" => $_POST["ip"], "path" => $_POST["dir"], "message" => "License Suspended", "datetime" => "now()"]);
    exit;
}
if ($status == "Reissued") {
    ...
    $validip = $ip;
    $validdirectory = $dir;
    update_query("mod_licensing", ["validdomain" => $validdomain, "validip" => $validip, "validdirectory" => $validdirectory, "status" => "Active"], ["id" => $licenseid]);
    if (0 < $reissues) {
        insert_query("mod_licensinglog", ["licenseid" => $licenseid, "domain" => $_POST["domain"], "ip" => $_POST["ip"], "path" => $_POST["dir"], "message" => "License Reissued", "datetime" => "now()"]);
    }
}
if ($status == "Reissued" || $status == "Active") {
    if ($licensing_freetrial) {
        ...
        if (count($trialmatches)) {
            echo "<status>Suspended</status>";
            licensing_getlicreturndata($licenseid);
            update_query("mod_licensing", ["status" => "Suspended"], ["id" => $licenseid]);
            ...
            insert_query("mod_licensinglog", ["licenseid" => $licenseid, "domain" => $_POST["domain"], "ip" => $_POST["ip"], "path" => $_POST["dir"], "message" => "License Suspended for Duplicate Trials Use (" . implode(",", $trialmatches) . ")", "datetime" => "now()"]);
            exit;
        }
    }
    $result = select_query("mod_licensingbans", "", ["value" => $domain]);
    $data = mysql_fetch_array($result);
    $banid = $data["id"];
    $bannotes = $data["notes"];
    if ($banid) {
        ...
        insert_query("mod_licensinglog", ["licenseid" => $licenseid, "domain" => $_POST["domain"], "ip" => $_POST["ip"], "path" => $_POST["dir"], "message" => "Banned Domain/IP (" . $bannotes . ")", "datetime" => "now()"]);
        exit;
    }
}
...
if (!$allowdomainconflict && !in_array($domain, $validdomains)) {
    ...
    insert_query("mod_licensinglog", ["licenseid" => $licenseid, "domain" => $_POST["domain"], "ip" => $_POST["ip"], "path" => $_POST["dir"], "message" => "Domain Invalid", "datetime" => "now()"]);
} else {
    if (!$allowipconflict && !in_array($ip, $validips)) {
        echo "<status>Invalid</status>\n<message>IP Address Invalid</message>";
        insert_query("mod_licensinglog", ["licenseid" => $licenseid, "domain" => $_POST["domain"], "ip" => $_POST["ip"], "path" => $_POST["dir"], "message" => "IP Address Invalid", "datetime" => "now()"]);
    } else {
        if (!$allowdirectoryconflict && !in_array($dir, $validdirs)) {
            echo "<status>Invalid</status>\n<message>Directory Invalid</message>";
            insert_query("mod_licensinglog", ["licenseid" => $licenseid, "domain" => $_POST["domain"], "ip" => $_POST["ip"], "path" => $_POST["dir"], "message" => "Directory Invalid", "datetime" => "now()"]);
        } else {
...
function licensing_getlicreturndata($licenseid)
{
    ...
    $result = select_query("mod_licensing", "", ["id" => $licenseid]);
    $data = mysql_fetch_array($result);
    $serviceid = $data["serviceid"];
    $addonId = $data["addon_id"];
    ...
    foreach ($model->customFieldValues as $customFieldValue) {
        $customfields .= $customFieldValue->customField->fieldName . "=" . $customFieldValue->value . "|";
    }
    ...
    echo $xmlresp;
    $hookresults = run_hook("LicensingAddonVerify", ["licenseid" => $licenseid, "serviceid" => $serviceid, "addonid" => $addonId, "xmlresponse" => "<status>Active</status>\n" . $xmlresp]);
