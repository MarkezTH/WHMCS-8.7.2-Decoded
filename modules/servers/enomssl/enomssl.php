...
function enomssl_SSLStepOne($params)
{
    ...
    if (!$_SESSION["enomsslcert"][$orderid]["id"]) {
        ...
        foreach ($result["INTERFACE-RESPONSE"]["CERTGETCERTS"]["CERTS"] as $key => $certificate) {
            if (substr($key, 0, 4) == "CERT" && is_array($certificate)) {
...
function enomssl_CustomActions($CustomActionCollection, $params)
{
    ...
    $customActionCollection->add(WHMCS\Module\Server\CustomAction::factory("enomssl", "sslconfigurenow", function ($sslCertificate) {
