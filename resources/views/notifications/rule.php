...
foreach ($providers as $provider) {
    ...
    foreach ($obj->notificationSettings() as $setting => $options) {
        ...
        if (isset($rule->provider_config[$setting])) {
            $providerConfigSetting = $rule->provider_config[$setting];
        }
        ...
        switch ($options["Type"]) {
            case "dynamic":
                $settingParts = ["", ""];
                if (0 < strlen($providerConfigSetting)) {
                    $settingParts = explode("|", $providerConfigSetting, 2);
                }
                echo "<select class=\"form-control dynamic-lookup\"\n    data-provider=\"" . $provider->name . "\"\n    data-field=\"" . $setting . "\"\n    >\n    <option value=\"" . $settingParts[0] . "\">" . $settingParts[1] . "</option>\n</select>\n<input type=\"hidden\"\n    name=\"provider_config[" . $provider->name . "][" . $setting . "]\"\n    value=\"" . $providerConfigSetting . "\"\n    id=\"providerconfig-" . $setting . "\"\n    >";
                break;
            case "yesno":
                $checked = !empty($providerConfigSetting) ? "checked" : "";
            ...
            case "textarea":
                ...
                if (empty($providerConfigSetting) && isset($options["Default"])) {
                    $providerConfigSetting = $options["Default"];
                }
