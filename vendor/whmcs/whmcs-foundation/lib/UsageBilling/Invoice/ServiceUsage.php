...
class ServiceUsage
{
    ...
    public static function markUsageAsInvoiced($invoiceId, $invoiceLineItems = [])
    {
        $statIds = [];
        ...
        if (!empty($statIds)) {
            \WHMCS\UsageBilling\Metrics\Server\Stat::whereIn("id", $statIds)->where("type", "!=", \WHMCS\UsageBilling\Contracts\Metrics\MetricInterface::TYPE_SNAPSHOT)->update(["invoice_id" => $invoiceId]);
        }
    }
    ...
    public function createUsageInvoice()
    {
        ...
        createInvoices($clientId, "", "", ["serviceUsage" => [$this->service->id]]);
    }
