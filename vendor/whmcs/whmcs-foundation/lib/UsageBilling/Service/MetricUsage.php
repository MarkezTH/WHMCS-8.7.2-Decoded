...
class MetricUsage extends \WHMCS\Model\AbstractModel
{
    ...
    protected $fillable = ["rel_type", "rel_id", "module", "module_type", "metric"];
    ...
    public function createTable($drop = false)
    {
        ...
        if (!$schemaBuilder->hasTable($this->getTable())) {
            $schemaBuilder->create($this->getTable(), function ($table) {
                ...
                $table->timestamps();
                $table->index(["rel_type", "rel_id"], "tblusage_items_rel_type_id");
                $table->index(["module_type"], "tblusage_items_module_type");
                $table->index(["module"], "tblusage_items_module");
                $table->index(["metric"], "tblusage_items_metric");
            });
        }
    }
    ...
    {
        return $this->hasMany("WHMCS\\UsageBilling\\Pricing\\Fixed\\Bracket", "rel_id")->orderBy("floor", "asc")->orderBy("ceiling", "desc");
    }
    ...
    public function factoryInvoiceItem()
    {
        ...
        if ($module->functionExists("metric_unit_value")) {
            $units = $this->module->call("metric_unit_value", ["metricUsage" => $this]);
            $this->value = $units;
            $this->save();
        } else {
            $units = $this->value;
        ...
        if ($module->functionExists("metric_price_calculation")) {
            $surchargeCalculation = $module->call("metric_price_calculation", ["metricUsage" => $this, "service" => $service, "billingName" => $billingName, "units" => $units, "metricName" => $metricName]);
