...
class UsageItem extends \WHMCS\Model\AbstractModel
{
    ...
    protected $fillable = ["rel_type", "rel_id", "module", "module_type", "metric", "included", "is_hidden"];
    ...
    {
        $pricingDetails = [["floor" => 0, "ceiling" => 0, "type" => \WHMCS\UsageBilling\Contracts\Pricing\PricingSchemaInterface::TYPE_SIMPLE, "price" => []]];
    ...
    public function createPriceSchema($pricingDetails = [])
    {
        ...
        foreach ($pricingDetails as $bracketDetail) {
            ...
            foreach ($bracketDetail["price"] as $currencyId => $cyclePricing) {
                ...
                if (!$setupPrice) {
                    $setupPrice = new \WHMCS\UsageBilling\Pricing\Product\Pricing(["type" => $priceStub->pricingType(), "relid" => $bracket->id, "currency" => $currencyId]);
                }
