...
class Download extends \WHMCS\Model\AbstractModel
{
    ...
    protected $columnMap = ["downloadCategoryId" => "category", "timesDownloaded" => "downloads", "fileLocation" => "location", "clientDownloadOnly" => "clientsonly", "isHidden" => "hidden", "isProductDownload" => "productdownload"];
    protected $booleans = ["clientDownloadOnly", "isHidden", "isProductDownload"];
    ...
    public static function boot()
    {
        ...
        Download::saved(function (Download $download) {
            if (\WHMCS\Config\Setting::getValue("EnableTranslations")) {
                $translation = \WHMCS\Language\DynamicTranslation::firstOrNew(["related_type" => "download.{id}.description", "related_id" => $download->id, "language" => \WHMCS\Config\Setting::getValue("Language"), "input_type" => "textarea"]);
                $translation->translation = $download->getRawAttribute("description");
                $translation->save();
                $translation = \WHMCS\Language\DynamicTranslation::firstOrNew(["related_type" => "download.{id}.title", "related_id" => $download->id, "language" => \WHMCS\Config\Setting::getValue("Language"), "input_type" => "text"]);
        ...
        Download::deleted(function (Download $download) {
            if (\WHMCS\Config\Setting::getValue("EnableTranslations")) {
                \WHMCS\Language\DynamicTranslation::whereIn("related_type", ["download.{id}.description", "download.{id}.title"])->where("related_id", "=", $download->id)->delete();
            }
        });
