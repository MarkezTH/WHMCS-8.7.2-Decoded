...
class Properties
{
    ...
    protected static $fieldsToCustomFieldName = ["username" => "Username", "password" => "Password", "domain" => "Domain", "license" => "License Key", "dedicatedip" => "Dedicated IP", "diskusage" => "Disk Usage", "disklimit" => "Disk Limit", "bwusage" => "Bandwidth Usage", "bwlimit" => "Bandwidth Limit", "lastupdate" => "Last Update", "subscriptionid" => "Subscription ID"];
    ...
    public function get($fieldName)
    {
        if ($this->model->isService() && $this->isNativeField($fieldName)) {
            ...
            return get_query_val("tblhosting", $fieldName, ["id" => $this->model->id]);
        }
        $fieldName = $this->getNativeFieldDisplayName($fieldName);
        $customField = $this->getCustomField($this->model, $fieldName);
        if (is_null($customField)) {
        ...
        $customFieldValue = \WHMCS\CustomField\CustomFieldValue::firstOrNew(["fieldid" => $customField->id, "relid" => $this->model->id]);
