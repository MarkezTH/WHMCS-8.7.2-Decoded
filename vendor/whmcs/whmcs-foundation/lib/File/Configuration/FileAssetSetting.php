...
class FileAssetSetting extends \WHMCS\Model\AbstractModel
{
    ...
    public function scopeHasLocalConfiguration(\Illuminate\Database\Eloquent\Builder $query)
    {
        return $query->whereHas("configuration", function (\Illuminate\Database\Eloquent\Builder $query) {
            $query->local();
        });
    }
}
