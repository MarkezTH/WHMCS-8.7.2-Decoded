...
class S3StorageProvider implements StorageProviderInterface
{
    ...
    {
        $keys = ["key" => true, "secret" => true, "bucket" => true, "region" => true, "endpoint_url" => false];
        $relevantSettings = [];
        $errorFields = [];
        $friendlyNames = [];
        ...
        foreach ($keys as $key => $required) {
            ...
            if ($settingValue != "" || !$required) {
                ...
                $errorFields[] = \AdminLang::trans("validation.required", [":attribute" => $friendlyNames[$key]]);
            }
        }
    ...
    {
        return [["Name" => "key", "FriendlyName" => \AdminLang::trans("storage.s3.key"), "Type" => "text"], ["Name" => "secret", "FriendlyName" => \AdminLang::trans("storage.s3.secret"), "Type" => "text"], ["Name" => "bucket", "FriendlyName" => \AdminLang::trans("storage.s3.bucket"), "Type" => "text"], ["Name" => "region", "FriendlyName" => \AdminLang::trans("storage.s3.region"), "Type" => "text"], ["Name" => "endpoint_url", "FriendlyName" => \AdminLang::trans("storage.s3.endpointUrl"), "Type" => "text", "Placeholder" => \AdminLang::trans("global.optional"), "Description" => \AdminLang::trans("storage.s3.endpointUrlDescription")]];
    }
    ...
    {
        $settings = ["credentials" => ["key" => $this->config["key"], "secret" => $this->config["secret"]], "region" => $this->config["region"], "version" => "latest"];
    ...
    {
        $assetPrefixes = [\WHMCS\File\FileAsset::TYPE_CLIENT_FILES => "client_files", \WHMCS\File\FileAsset::TYPE_DOWNLOADS => "downloads", \WHMCS\File\FileAsset::TYPE_EMAIL_ATTACHMENTS => "email_attachments", \WHMCS\File\FileAsset::TYPE_EMAIL_TEMPLATE_ATTACHMENTS => "template_attachments", \WHMCS\File\FileAsset::TYPE_EMAIL_IMAGES => "email_images", \WHMCS\File\FileAsset::TYPE_KB_IMAGES => "kb_images", \WHMCS\File\FileAsset::TYPE_PM_FILES => "projects", \WHMCS\File\FileAsset::TYPE_TICKET_ATTACHMENTS => "ticket_attachments"];
