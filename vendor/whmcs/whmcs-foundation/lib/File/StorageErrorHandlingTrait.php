...
trait StorageErrorHandlingTrait
{
    ...
    private function wrapStorageCall($methodName, $args)
    {
        try {
            ...
            if ($this->assetSetting) {
                try {
                    ...
                    $configuration->last_error = ["message" => $assetTypeName . ": " . $exceptionErrorMessage, "timestamp" => \WHMCS\Carbon::now()->toDateTimeString()];
                    $configuration->save();
                } catch (\Exception $e) {
                }
            }
