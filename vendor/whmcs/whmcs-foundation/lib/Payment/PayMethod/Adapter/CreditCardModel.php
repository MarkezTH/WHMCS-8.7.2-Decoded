...
abstract class CreditCardModel extends BaseAdapterModel implements \WHMCS\Payment\Contracts\CreditCardDetailsInterface
{
    ...
    {
        run_hook("CCUpdate", ["userid" => $this->payMethod->client->id ?? NULL, "cardtype" => $this->getCardType(), "cardnum" => $this->getCardNumber(), "cardcvv" => $this->getCardCvv(), "expdate" => $this->getExpiryDate() ? $this->getExpiryDate()->toCreditCard() : "", "cardstart" => $this->getStartDate() ? $this->getStartDate()->toCreditCard() : "", "issuenumber" => $this->getIssueNumber()]);
    }
}
