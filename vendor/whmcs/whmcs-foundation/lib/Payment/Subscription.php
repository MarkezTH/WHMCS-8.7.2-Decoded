...
class Subscription
{
    public static function getInfo($relatedItem)
    {
        try {
            ...
            if (!in_array($instanceType, ["WHMCS\\Service\\Addon", "WHMCS\\Domain\\Domain", "WHMCS\\Service\\Service"])) {
            ...
            $response = ["body" => view("admin.client.profile.subscription-info", ["isActive" => strtolower($subscriptionInfo["Status"]) == "active", "subscriptionDetails" => $subscriptionDetails])];
        } catch (\Exception $e) {
            $response = ["body" => view("admin.client.profile.subscription-info", ["errorMsg" => $e->getMessage()])];
        }
    ...
    public static function cancel($relatedItem)
    {
        try {
            ...
            if (is_array($cancelResult) && $cancelResult["status"] == "success") {
                ...
                $relatedItem->save();
                if ($relatedItem instanceof \WHMCS\Service\Addon) {
                    \WHMCS\Service\Addon::where("subscriptionid", $subscriptionId)->where("paymentmethod", $paymentMethod)->where("userid", $relatedItem->clientId)->update(["subscriptionid" => ""]);
                } else {
                    if ($relatedItem instanceof \WHMCS\Domain\Domain) {
                        \WHMCS\Domain\Domain::where("subscriptionid", $subscriptionId)->where("paymentmethod", $paymentMethod)->where("userid", $relatedItem->clientId)->update(["subscriptionid" => ""]);
                    } else {
                        if ($relatedItem instanceof \WHMCS\Service\Service) {
                            \WHMCS\Service\Service::where("subscriptionid", $subscriptionId)->where("paymentmethod", $paymentMethod)->where("userid", $relatedItem->clientId)->update(["subscriptionid" => ""]);
                        }
                    }
                }
                ...
                $response = ["success" => true, "successMsgTitle" => \AdminLang::trans("global.success"), "successMsg" => \AdminLang::trans("services.cancelSubscriptionSuccess")];
            } else {
            ...
            $response = ["errorMsg" => $e->getMessage(), "errorMsgTitle" => \AdminLang::trans("global.erroroccurred")];
        }
        return new \WHMCS\Http\Message\JsonResponse($response);
    }
}
