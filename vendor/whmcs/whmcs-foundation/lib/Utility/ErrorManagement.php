...
class ErrorManagement
{
    ...
    public static function isDisplayErrorCurrentlyVisible()
    {
        ...
        if (in_array($displayErrors, ["on", "true", "1", "stdout"]) || $displayErrors === 1 || $displayErrors === true) {
            return true;
        }
        return false;
    }
    ...
    {
        if (!is_numeric(self::ERROR_LEVEL_NONE_VALUE)) {
    ...
    {
        $errorLevels = [self::ERROR_LEVEL_NONE_KEY => self::ERROR_LEVEL_NONE_VALUE, self::ERROR_LEVEL_INHERIT_KEY => self::ERROR_LEVEL_INHERIT_VALUE, self::ERROR_LEVEL_ERRORS_KEY => self::ERROR_LEVEL_ERRORS_VALUE, self::ERROR_LEVEL_WARNINGS_KEY => self::ERROR_LEVEL_WARNINGS_VALUE];
        return $errorLevels;
    }
    ...
    public static function errorsAsExceptions($config)
    {
        return !empty($config->surface_errors_as_exceptions);
    }
}
