...
class TokenManager
{
    ...
    public function isRequestTokenValid()
    {
        return $this->isValidToken($this->getRequestToken());
    }
    ...
    {
        return ["WHMCS.default" => $this->defaultNamespaceValue, "WHMCS.admin.default" => $this->defaultNamespaceValue, "WHMCS.domainchecker" => false];
    }

    public function getStoredNamespaceSettings($whmcs)
    {
        $serialized_namespace = $whmcs->get_config("token_namespaces");
        $namespace_settings = $serialized_namespace ? safe_unserialize($serialized_namespace) : [];
