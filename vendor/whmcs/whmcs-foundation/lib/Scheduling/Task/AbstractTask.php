...
class AbstractTask extends \Illuminate\Database\Eloquent\Model implements \WHMCS\Cron\DecoratorItemInterface, TaskInterface
{
    ...
    protected $guarded = [];
    protected $casts = ["is_enabled" => "boolean", "is_periodic" => "boolean"];
    ...
    public static function firstOfClassOrNew($attributes = [])
    {
        $attributes = array_merge(["class_name" => static::class], $attributes);
    ...
    {
        return ["priority" => $this->defaultPriority, "class_name" => static::class, "is_enabled" => $this->defaultIsEnabled, "is_periodic" => $this->defaultIsPeriodic, "frequency" => $this->defaultFrequency, "description" => $this->defaultDescription, "name" => $this->defaultName];
    }
    ...
    {
        if (!$status) {
            $status = Status::firstOrCreate(["task_id" => $this->id]);
        }
        return $status;
    }
