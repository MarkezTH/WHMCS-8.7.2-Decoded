...
class DailyCronReport extends \WHMCS\Log\Register
{
    ...
    {
        $since = $this->start;
        return ["id" => $task->id, "name" => $task->getName(), "output_ids" => $task->getOutputsSince($since)->pluck("id"), "completed" => $completed];
    }

    public function start()
    {
        $this->start = \WHMCS\Carbon::now();
        $dateTime = $this->start->toDateTimeString();
        $this->write(["startTime" => $dateTime, "tasks" => [], "finishTime" => $dateTime]);
    }
    ...
    public function taskDataToHtml($taskData)
    {
        $html = "";
        ...
        if (!empty($taskData["output_ids"])) {
            ...
            if (!is_array($successOutputIds)) {
                $successOutputIds = [$successOutputIds];
            }
            foreach ($taskData["output_ids"] as $id) {
                ...
                if ($output) {
                    ...
                    if (in_array($outputNamespace, $successOutputIds, true)) {
                        $value += (int) $output->getValue();
                    }
                }
            }
            $html .= str_replace(["%task_name%", "%value%", "%success_keyword%"], [$task->getName(), $value, $task->getSuccessKeyword()], $template);
        }
        return $html;
    }
