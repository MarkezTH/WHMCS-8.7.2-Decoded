...
class Domain extends \WHMCS\Mail\Emailer
{
    protected function getEntitySpecificMergeData($domainId)
    {
        ...
        $email_merge_fields = [];
        $email_merge_fields["domain_id"] = $domainId;
        ...
        $email_merge_fields["domain_renewal_url"] = \App::getSystemUrl() . "index.php?rp=/domain/" . $domain . "/renew";
        ...
        $email_merge_fields["expiring_domains"] = [];
        $email_merge_fields["domains"] = [];
        $this->massAssign($email_merge_fields);
        if (in_array($this->message->getTemplateName(), ["Upcoming Domain Renewal Notice", "Domain Expiry Notice"])) {
            $clientEmail = get_query_val("tblclients", "email", ["id" => $userid]);
