...
class Invoice extends \WHMCS\Mail\Emailer
{
    protected function getEntitySpecificMergeData($invoiceId)
    {
        ...
        if (!$data["payMethod"] && in_array($type, ["creditcard", "bankaccount"])) {
        ...
        if (in_array($type, ["creditcard", "bankaccount"])) {
        ...
        if ($gatewayInterface->supportsAutoCapture() && !$clientProcessingDisabled) {
            if ($payMethod && !$payMethod->trashed()) {
                ...
                if (!empty($autoCaptureParams["gatewayid"]) || !empty($autoCaptureParams["cardnum"]) || !empty($autoCaptureParams["bankacct"])) {
                    $autoCapture = true;
                }
            }
        }
