...
class Admin extends \WHMCS\Mail\Emailer
{
    ...
    public function __construct($message, $entityId, $extraParams = [])
    {
    ...
    protected function getEntitySpecificMergeData($userId, $extra)
    {
        ...
        $this->massAssign(["whmcs_admin_url" => $adminUrl, "whmcs_admin_link" => "<a href=\"" . $adminUrl . "\">" . $adminUrl . "</a>"]);
    }

    public function determineAdminRecipientsAndSender($to, $deptid, $adminid, $ticketnotify)
    {
        if ($deptid) {
            $result = select_query("tblticketdepartments", "name,email", ["id" => $deptid]);
        ...
        if ($adminid) {
            ...
            if (in_array($to, ["ticket_changes", "mentions"])) {
                return false;
            }
