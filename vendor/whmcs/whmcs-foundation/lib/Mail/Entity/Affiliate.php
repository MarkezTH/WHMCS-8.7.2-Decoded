...
class Affiliate extends \WHMCS\Mail\Emailer
{
    ...
    {
        $referralsTable = "";
        $affiliateData = \WHMCS\Database\Capsule::table("tblaffiliates")->find($affiliateId, ["id", "clientid", "visitors", "balance", "withdrawn"]);
        ...
        $affiliatesAccounts = \WHMCS\Database\Capsule::table("tblaffiliatesaccounts")->where("affiliateid", "=", $affiliateID)->whereBetween("tblhosting.regdate", [date("Y-m-d", $firstOfLastMonth), date("Y-m-d", $lastOfLastMonth)])->join("tblhosting", "tblhosting.id", "=", "tblaffiliatesaccounts.relid")->join("tblproducts", "tblproducts.id", "=", "tblhosting.packageid")->join("tblclients", "tblclients.id", "=", "tblhosting.userid")->orderBy("regdate", "DESC")->get(["tblaffiliatesaccounts.*", "tblproducts.name", "tblhosting.packageid", "tblhosting.userid", "tblhosting.domainstatus", "tblhosting.amount", "tblhosting.firstpaymentamount", "tblhosting.regdate", "tblhosting.billingcycle"])->all();
        ...
        $email_merge_fields["affiliate_referral_url"] = $systemURL . "aff.php?aff=" . $id;
