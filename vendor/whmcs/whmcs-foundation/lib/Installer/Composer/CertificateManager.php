...
class CertificateManager
{
    ...
    private $certsDir = "";
    private $certMetadata = [];
    private $environmentErrors = [];
    ...
    private function initEnvironment()
    {
        ...
        if (empty($this->environmentErrors)) {
            ...
            $directoriesToCreate = [$certsDirectory, $certsDirectory . DIRECTORY_SEPARATOR . Certificate::CACHE_SIGNING_SUBDIR, $certsDirectory . DIRECTORY_SEPARATOR . Certificate::CACHE_INTERMEDIATE_SUBDIR];
            foreach ($directoriesToCreate as $newDir) {
                if (!is_dir($newDir) && !@mkdir($newDir)) {
                    $this->environmentErrors[] = \AdminLang::trans("update.notWritablePath", [":path" => $newDir]);
                }
            }
        }
    ...
    private function getUrl($url)
    {
        ...
        $guzzle = new \WHMCS\Http\Client\HttpClient(["verify" => true]);
        try {
