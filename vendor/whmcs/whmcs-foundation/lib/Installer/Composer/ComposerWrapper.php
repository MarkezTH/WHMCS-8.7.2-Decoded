...
class ComposerWrapper
{
    ...
    private $lastRunPackageMetadata = [];
    ...
    public function initEnvironment()
    {
        ...
        if (empty($this->environmentErrors)) {
            ...
            if (!is_dir($composerDirectory) && !@mkdir($composerDirectory)) {
                $this->environmentErrors[] = \AdminLang::trans("update.notWritablePath", [":path" => $composerDirectory]);
            }
        }
        if (empty($this->environmentErrors)) {
    ...
    public function update()
    {
        try {
            ...
            $params = ["command" => "update", "packages" => [self::PACKAGE_NAME], "--working-dir" => $this->targetDir, "--dry-run" => $this->getDryRun(), "--prefer-dist" => true, "--no-dev" => true, "--no-interaction" => true, "--no-progress" => true, "--no-autoloader" => true];
