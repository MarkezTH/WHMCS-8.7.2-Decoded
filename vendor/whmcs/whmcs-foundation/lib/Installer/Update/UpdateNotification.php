...
class UpdateNotification
{
    ...
    public function __construct($title = "", $body = "", $style = self::STYLE_WARNING, $icon = "fa-asterisk", $requiresAcceptance = "", $overrides = [], $id = "")
    {
        $this->setTitle($title);
        $this->setBody($body);
        if (!empty($id)) {
    ...
    {
        $this->id = substr(sha1($this->title . "\0" . $this->body), 0, 8);
    }
