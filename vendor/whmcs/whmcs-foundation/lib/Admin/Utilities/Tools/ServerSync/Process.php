...
class Process
{
    public static function import(SyncItem $syncItem, \WHMCS\Module\Server $moduleInterface, $serverId, $additional = [])
    {
        ...
        if (!$client) {
            ...
            $user = \WHMCS\User\User::where("email", $client->email)->first();
            if (!$user) {
                $user = \WHMCS\User\User::createUser("Name", "Placeholder", $client->email, \Illuminate\Support\Str::random(16));
        ...
        if (!$product) {
            ...
            if (!$productGroup) {
                ...
                $productGroup->disabledPaymentGateways = [];
            ...
            $productName = str_replace([$moduleInterface->getParam("serverusername") . "_", $moduleInterface->getParam("serverusername")], "", $productName);
            ...
            $product->freeSubDomains = [];
            $product->autoSetup = "";
            $product->freeDomain = "";
            $product->freeDomainPaymentTerms = [];
            $product->freeDomainTlds = [];
            ...
            $product->enableOverageBillingAndUnits = [];
        ...
        if ($uniqueIdField == "username") {
            ...
            if (substr($uniqueIdField, 0, 12) == "customfield.") {
                ...
                $service->serviceProperties->save([$customFieldName => $syncItem->getUniqueIdentifier()]);
            } else {
    ...
    public static function sync(SyncItem $syncItem, ServiceItem $serviceItem, $serverId)
    {
        ...
        if ($syncItem->getStatus() != $service->domainStatus) {
            $previousStatus = $service->domainStatus;
            $service->domainStatus = $syncItem->getStatus();
