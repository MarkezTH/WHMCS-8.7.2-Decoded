...
class HomepageController
{
    ...
    public function refreshWidget(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        $aInt->sidebar = "home";
        $aInt->icon = "home";
        $aInt->requiredFiles(["clientfunctions", "invoicefunctions", "gatewayfunctions", "ccfunctions", "processinvoices", "reportfunctions"]);
        ...
        try {
            ...
            return new \WHMCS\Http\Message\JsonResponse(["success" => true, "widgetOutput" => $widgetOutput . $js]);
        } catch (\Exception $e) {
            return new \WHMCS\Http\Message\JsonResponse(["success" => false, "exceptionMsg" => $e->getMessage()]);
        }
    }
    ...
    public function saveNotes(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        update_query("tbladmins", ["notes" => $notes], ["id" => \WHMCS\Session::get("adminid")]);
    ...
    public function toggleWidgetDisplay(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        try {
            ...
            return new \WHMCS\Http\Message\JsonResponse(["success" => false, "widget" => $widget]);
        }
    }
    ...
    public function index(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        $templatevars["licenseinfo"] = ["registeredname" => $licensing->getRegisteredName(), "productname" => $licensing->getProductName(), "expires" => $licensing->getExpiryDate(), "currentversion" => $whmcs->getVersion()->getCasual(), "latestversion" => $updater->getLatestVersion()->getCasual(), "updateavailable" => $updater->isUpdateAvailable() && $aInt->hasPermission("Update WHMCS")];
        ...
        if (\WHMCS\Module\GatewaySetting::gatewayType(\WHMCS\Module\Gateway::GATEWAY_CREDIT_CARD)->exists()) {
        ...
        $templatevars["generateInvoices"] = $aInt->modal("GenerateInvoices", \AdminLang::trans("invoices.geninvoices"), \AdminLang::trans("invoices.geninvoicessendemails"), [["title" => \AdminLang::trans("global.yes"), "onclick" => "window.location=\"index.php?generateinvoices=true" . generate_token("link") . "\"", "class" => "btn-primary"], ["title" => \AdminLang::trans("global.no"), "onclick" => "window.location=\"index.php?generateinvoices=true&noemails=true" . generate_token("link") . "\""]]);
        $templatevars["creditCardCapture"] = $aInt->modal("CreditCardCapture", \AdminLang::trans("invoices.attemptcccaptures"), \AdminLang::trans("invoices.attemptcccapturessure"), [["title" => \AdminLang::trans("global.yes"), "onclick" => "window.location=\"index.php?attemptccpayments=true" . generate_token("link") . "\"", "class" => "btn-primary"], ["title" => \AdminLang::trans("global.no")]]);
        $addons_html = run_hook("AdminHomepage", []);
        $templatevars["addons_html"] = $addons_html;
        $roleId = get_query_val("tbladmins", "roleid", ["id" => (int) \WHMCS\Session::get("adminid")]);
        if ($roleId == 1) {
    ...
    public function mentions(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        foreach ($adminUsers as $adminUser) {
            $gravatar = "<img src=\"https://www.gravatar.com/avatar/" . $adminUser->gravatarHash . "?s=24&d=mm\" width=\"24\" height=\"24\" alt=\"Gravatar\" />";
            $admins[] = ["id" => $adminUser->id, "email" => $adminUser->email, "name" => $adminUser->fullName, "username" => $adminUser->username, "gravatar" => $gravatar];
        }
    ...
    public function marketingConversion(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        if ($marketingType != "") {
            ...
            if ($marketingType == "convert") {
                \WHMCS\User\Client::where("emailoptout", 0)->update(["marketing_emails_opt_in" => 1]);
            }
            \WHMCS\Config\Setting::setValue("MarketingEmailConvert", "on");
            return new \WHMCS\Http\Message\JsonResponse(["body" => "<script type=\"text/javascript\">jQuery(\"#marketingConsentAlert\").slideUp(\"fast\");</script>", "successMsg" => \AdminLang::trans("marketingConsent.conversionComplete"), "successMsgTitle" => \AdminLang::trans("global.success"), "dismiss" => true]);
        }
    ...
    public function dismissMarketConnectProductPromo(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        return new \WHMCS\Http\Message\JsonResponse(["messageTitle" => \AdminLang::trans("products.promoDismissed"), "message" => \AdminLang::trans("products.promoDismissedMessage")]);
    }
