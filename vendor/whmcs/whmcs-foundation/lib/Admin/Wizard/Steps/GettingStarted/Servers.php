...
class Servers
{
    public function getTemplateVariables()
    {
        ...
        return ["autoConfigServers" => $server->getAutoPopulateServers()];
    }
    ...
    public function verifyConnection($data)
    {
        try {
            ...
            throw new \WHMCS\Exception(\AdminLang::trans("validation.regex", [":attribute" => \AdminLang::trans("fields.hostnameip")]));
        }
        $server = new \WHMCS\Admin\Setup\Servers();
        $module = $data["module"];
    ...
    public function save($data)
    {
        $server = new \WHMCS\Admin\Setup\Servers();
        $accesshash = "";
        $serverId = $server->add($data["name"], $data["module"], $data["primaryip"], "", $data["hostname"], "", "", "", $data["nameserver1"], "", $data["nameserver2"], "", "", "", "", "", "", "", 200, $data["username"], $data["password"], $accesshash, true, "", [], 0);
