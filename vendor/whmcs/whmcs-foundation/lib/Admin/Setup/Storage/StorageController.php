...
class StorageController
{
    ...
    public function index(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        $viewAssetSettings = [];
        $dbAssetSettings = \WHMCS\File\Configuration\FileAssetSetting::with(["configuration", "migrateToConfiguration"])->get();
        ...
        foreach ($dbAssetSettings as $assetSetting) {
            if ($assetSetting->migrateToConfiguration) {
                ...
                if ($migrationProgress) {
                    ...
                    $migration = ["active" => $migrationProgress->active, "progress" => $progress, "last_failure" => $this->truncateString($migrationProgress->last_failure_reason, self::MAX_FAILURE_REASON_LENGTH)];
                } else {
                    $migration = ["active" => true, "progress" => 0, "last_failure" => ""];
                }
            } else {
                $migration = NULL;
            }
            $viewAssetSettings[] = array_merge($assetSetting->toArray(), ["can_migrate" => \WHMCS\File\FileAsset::canMigrate($assetSetting->asset_type), "migration" => $migration]);
        }
        ...
        foreach (\WHMCS\File\Configuration\StorageConfiguration::all() as $config) {
            ...
            $storageConfigurations[] = array_merge($config->toArray(), ["provider_name" => $provider->getName(), "icon" => $provider->getIcon(), "config_summary" => $provider->getConfigSummaryHtml(), "error_message" => \WHMCS\Input\Sanitize::encode($errorMessage)]);
        }
        ...
        $params = ["assetSettings" => $viewAssetSettings, "storageConfigurations" => $storageConfigurations, "providers" => $providers, "csrfToken" => generate_token("plain"), "action" => $action];
    ...
    public function editConfiguration(\WHMCS\Http\Message\ServerRequest $request, $errorMsg = NULL, $duplicate = false)
    {
        ...
        $output = view("admin.setup.storage.provider-config", ["id" => !$duplicate ? $configId : 0, "duplicate_configuration_id" => $duplicate ? $configId : 0, "provider" => $provider, "inUse" => $inUse && !$duplicate, "settings" => $settings, "errorMsg" => $errorMsg]);
    ...
    public function saveConfiguration(\WHMCS\Http\Message\ServerRequest $request)
    {
        try {
            ...
            return new \WHMCS\Http\Message\JsonResponse(["dismiss" => true, "reloadPage" => routePath("admin-setup-storage-index", "confirmSave")]);
    ...
    public function testConfiguration(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        try {
            ...
            $config->save();
            return new \WHMCS\Http\Message\JsonResponse(["successMsgTitle" => \AdminLang::trans("global.success"), "successMsg" => \AdminLang::trans("storage.configTestOk")]);
        } catch (\Exception $e) {
            return new \WHMCS\Http\Message\JsonResponse(["errorMsgTitle" => \AdminLang::trans("global.error"), "errorMsg" => \AdminLang::trans("storage.configTestError") . " " . \WHMCS\Input\Sanitize::encode($e->getMessage())]);
        }
    }
    ...
    public function deleteConfiguration(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        try {
            ...
            $config->delete();
            return new \WHMCS\Http\Message\JsonResponse(["successMsg" => \AdminLang::trans("storage.config.confirmDelete")]);
        } catch (\Exception $e) {
            return new \WHMCS\Http\Message\JsonResponse(["errorMsgTitle" => \AdminLang::trans("global.error"), "errorMsg" => \AdminLang::trans("storage.configDeleteError") . " " . $e->getMessage()]);
        }
    }
    ...
    public function dismissError(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        try {
            ...
            $config->last_error = NULL;
            $config->save();
            return new \WHMCS\Http\Message\JsonResponse(["successMsg" => \AdminLang::trans("global.success")]);
        } catch (\Exception $e) {
            return new \WHMCS\Http\Message\JsonResponse(["errorMsgTitle" => \AdminLang::trans("global.error"), "errorMsg" => \AdminLang::trans("global.error")]);
        }
    }
    ...
    public function cancelMigration(\WHMCS\Http\Message\ServerRequest $request)
    {
        try {
            ...
            $assetSetting->save();
            return new \WHMCS\Http\Message\JsonResponse(["successMsgTitle" => \AdminLang::trans("global.success"), "successMsg" => \AdminLang::trans("storage.migration.cancelOk")]);
        } catch (\Exception $e) {
            return new \WHMCS\Http\Message\JsonResponse(["errorMsgTitle" => \AdminLang::trans("global.error"), "errorMsg" => $e->getMessage()]);
        }
    }
    ...
    public function switchAssetStorage(\WHMCS\Http\Message\ServerRequest $request)
    {
        try {
            ...
            return new \WHMCS\Http\Message\JsonResponse(["successMsgTitle" => \AdminLang::trans("global.success"), "successMsg" => \AdminLang::trans("storage.migration.switchOk")]);
        } catch (\Exception $e) {
            return new \WHMCS\Http\Message\JsonResponse(["errorMsgTitle" => \AdminLang::trans("global.error"), "errorMsg" => $e->getMessage()]);
        }
    }
    ...
    public function startMigration(\WHMCS\Http\Message\ServerRequest $request)
    {
        try {
            ...
            if ($migrationProcessor->migrate()) {
                $response = ["migrationCompleted" => true];
            } else {
                ...
                $response = ["migrationInProgress" => true, "progress" => $percentDone];
