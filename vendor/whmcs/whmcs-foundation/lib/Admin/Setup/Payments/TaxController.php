...
class TaxController
{
    public function saveSettings(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        foreach ($taxSettings as $k => $v) {
            if ($k != "TaxEnabled" && \WHMCS\Config\Setting::getValue($k) != $v) {
                ...
                if (in_array($k, ["TaxType", "SequentialInvoiceNumberFormat", "TaxCustomInvoiceNumberFormat"])) {
    ...
    public function create(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        $taxRate += 0;
        $response = ["status" => "error", "title" => \AdminLang::trans("global.error"), "message" => "An unknown error occurred"];
        if (!$name) {
            $name = "Tax";
        }
        if ($countryType == "any" && $stateType != "any") {
            ...
            $taxId = \WHMCS\Database\Capsule::table("tbltax")->insertGetId(["level" => $level, "name" => $name, "state" => $state, "country" => $country, "taxrate" => $taxRate]);
            ...
            $response = ["status" => "success", "id" => $taxId, "level" => $level, "name" => $name, "state" => $state, "country" => $country, "rate" => number_format($taxRate, 3)];
        }
    ...
    public function delete(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        return new \WHMCS\Http\Message\JsonResponse(["status" => "error"]);
    }
    ...
    protected function pageSummaryHtml()
    {
        ...
        if ($taxCustomField) {
            $customFields = \WHMCS\Database\Capsule::table("tblcustomfields")->where("type", "client")->pluck("fieldname", "id")->all();
            if ($customFields) {
    ...
    protected function getTaxHtmlTableData($level = 1)
    {
        ...
        $ruleSet = $query->orderBy("country", "asc")->orderBy("state", "asc")->orderBy("country", "asc")->get()->all();
        foreach ($ruleSet as $data) {
            ...
            $tableData[] = [(string) $data->name, $country, $state, (string) $taxRate . "%", "<a class=\"deleteRule\" href=\"#\"  data-href=\"" . routePath("admin-setup-payments-tax-delete") . "\" " . " data-id=\"" . (string) $data->id . "\" " . "\">" . "<img src=\"images/delete.gif\" border=\"0\"></a>"];
        }
        return $tableData;
    }
