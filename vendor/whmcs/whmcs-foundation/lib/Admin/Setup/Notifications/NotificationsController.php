...
class NotificationsController
{
    public function index(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        $output = view("notifications.index", ["notificationProviders" => $notificationProviders, "activateModuleName" => $moduleNameToActivate]);
            ...
            foreach (\WHMCS\Notification\Rule::orderBy("updated_at", "desc")->get() as $rule) {
                ...
                if (!is_null($event)) {
                    ...
                    foreach ($rule->conditions as $key => $value) {
                        if ($value && !in_array($value, ["contains", "exact", "greater", "less"])) {
                            $displayValue = $value;
    ...
    protected function manageRuleOutput(\WHMCS\Http\Message\ServerRequest $request, $title, $rule)
    {
        ...
        $output = view("notifications.rule", ["rule" => $rule, "eventTypes" => $eventTypes, "events" => $events, "conditions" => $conditions, "providers" => $providers]);
        return new \WHMCS\Http\Message\JsonResponse(["title" => $title, "body" => $output]);
    }
    ...
    protected function buildConditionField($eventType, $name, $values, $ruleConditions)
    {
        ...
        if (!isset($ruleConditions[$name])) {
            $ruleConditions[$name] = NULL;
        }
        if (!isset($ruleConditions[$name . "_filter"])) {
            $ruleConditions[$name . "_filter"] = NULL;
        }
        $condition = "";
        if ($values["Type"] == "text") {
    ...
    public function saveRule(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        if (!trim($description)) {
            return new \WHMCS\Http\Message\JsonResponse(["errorMsgTitle" => "", "errorMsg" => \AdminLang::trans("notifications.validationNameRequired")]);
        }
        $eventsList = [];
        foreach (\WHMCS\Notification\Events::all() as $event) {
        ...
        if (!array_key_exists($eventType, $eventsList)) {
            return new \WHMCS\Http\Message\JsonResponse(["errorMsgTitle" => "", "errorMsg" => \AdminLang::trans("notifications.validationEventTypeRequired")]);
        }
        ...
        if (count($events) < 1) {
            return new \WHMCS\Http\Message\JsonResponse(["errorMsgTitle" => "", "errorMsg" => \AdminLang::trans("notifications.validationEventRequired")]);
        }
        $provider = \WHMCS\Notification\Provider::active()->where("name", $provider)->first();
        if (is_null($provider)) {
            return new \WHMCS\Http\Message\JsonResponse(["errorMsgTitle" => "", "errorMsg" => \AdminLang::trans("notifications.validationProviderRequired")]);
        }
        $providerName = $provider->name;
        $providerConfig = isset($provider_config[$providerName]) && is_array($provider_config[$providerName]) ? $provider_config[$providerName] : [];
        ...
        foreach ($providerInterface->notificationSettings() as $field => $values) {
            if (isset($values["Required"]) && $values["Required"] && !$providerConfig[$field]) {
                return new \WHMCS\Http\Message\JsonResponse(["errorMsgTitle" => "", "errorMsg" => \AdminLang::trans("notifications.validationProviderFieldRequired") . " " . $values["FriendlyName"]]);
            }
        }
        ...
        if (method_exists($providerInterface, "postRuleSave")) {
            ...
            if ($message) {
                return new \WHMCS\Http\Message\JsonResponse(["body" => "<div class=\"alert alert-" . $type . "\" role=\"alert\">" . $message . "</div>", "hideSubmit" => $hideSubmit]);
            }
        }
        return new \WHMCS\Http\Message\JsonResponse(["successMsgTitle" => "", "successMsg" => $successMessage, "dismiss" => true]);
    }
    ...
    public function manageProvider(\WHMCS\Http\Message\ServerRequest $request, $errorMsg = NULL)
    {
        ...
        if (is_null($settings)) {
            ...
            $settings = is_null($provider) ? [] : $provider->settings;
        }
        $output = view("notifications.manage-provider", ["provider" => $notificationProvider, "settings" => $settings, "errorMsg" => $errorMsg]);
        return new \WHMCS\Http\Message\JsonResponse(["body" => $output]);
    }

    public function saveProvider(\WHMCS\Http\Message\ServerRequest $request)
    {
        $requestProvider = $request->get("provider");
        $provider = \WHMCS\Notification\Provider::firstOrNew(["name" => $requestProvider]);
        try {
        ...
        $provider->save();
        return new \WHMCS\Http\Message\JsonResponse(["successMsgTitle" => "", "successMsg" => \AdminLang::trans("notifications.settingsUpdatedConfirmation"), "dismiss" => true]);
    }
    ...
    public function getDynamicField(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        $provider = \WHMCS\Notification\Provider::firstOrNew(["name" => $requestProvider]);
        if (is_null($provider)) {
            return new \WHMCS\Http\Message\JsonResponse(["error" => "Invalid Provider Name"]);
        }
        try {
    ...
    public function getProvidersStatus()
    {
        ...
        foreach ($configuredProviders as $provider => $active) {
            $providers[$provider] = (int) $active;
        }
        return new \WHMCS\Http\Message\JsonResponse(["providers" => $providers, "success" => true]);
    }
    ...
    public function disableProvider(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        $provider->save();
        return new \WHMCS\Http\Message\JsonResponse(["successMsgTitle" => "", "successMsg" => \AdminLang::trans("notifications.providerDisabledConfirmation"), "hideSubmit" => true]);
    }
}
