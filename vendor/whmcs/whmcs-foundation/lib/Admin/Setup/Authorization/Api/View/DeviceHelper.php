...
class DeviceHelper
{
    public function getTableData($devices)
    {
        ...
        foreach ($devices as $device) {
            ...
            $tableData[] = [$device->identifier, $this->getDescriptionBtn($device), $device->admin->fullName . " (" . $device->admin->username . ")", $roleNames, $this->getLastAccessText($device), $this->getActionBtnGroup($device->id)];
        }
        return $tableData;
    }
