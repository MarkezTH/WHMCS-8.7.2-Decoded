...
class RoleController
{
    public function listRoles(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        return new \WHMCS\Http\Message\JsonResponse(["data" => $helper->getTableData($roles)]);
    }
    ...
    public function manage(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        $htmlPartial = view("authorization.partials.api-role-detail", ["apiCatalog" => \WHMCS\Api\V1\Catalog::get(), "role" => $role, "csrfToken" => $csrfToken]);
    ...
    public function get(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        if (!$role) {
            return new \WHMCS\Http\Message\JsonResponse(["error" => "Role Not Found"]);
        }
    ...
    public function create(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        if (!$roleName) {
            return new \WHMCS\Http\Message\JsonResponse(["status" => "error", "errorMsg" => "Role name cannot be empty."]);
        }
        if (\WHMCS\Api\Authorization\ApiRole::where("role", "=", $roleName)->count()) {
            return new \WHMCS\Http\Message\JsonResponse(["status" => "error", "errorMsg" => "A role with that name already exists. A role name must be unique."]);
        }
        $roleDesc = $request->get("roleDescription", "");
        $requestedAllow = $request->get("allow", []);
        $catalogActions = \WHMCS\Api\V1\Catalog::get()->getActions();
        $role = new \WHMCS\Api\Authorization\ApiRole(["role" => $roleName, "description" => $roleDesc]);
        $allowed = [];
        ...
        if ($role->save()) {
            ...
            logActivity($msg);
            $data = ["status" => "success", "data" => $role->toArray(), "dismiss" => true];
        } else {
            $statusCode = 500;
            $data = ["status" => "error", "errorMsg" => "unknown error"];
        }
    ...
    public function delete(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        if ($role) {
            ...
            if ($role->delete()) {
                logActivity($msg);
                $data = ["status" => "success", "data" => $role->toArray()];
            } else {
                $statusCode = 500;
                $data = ["status" => "error", "errorMessage" => "Failed to delete role: unknown error"];
            }
        } else {
            $statusCode = 400;
            $data = ["status" => "error", "errorMessage" => "Failed to delete role: Unknown role id"];
        }
        return new \WHMCS\Http\Message\JsonResponse($data, $statusCode);
    }
    ...
    public function update(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        if ($role) {
            ...
            $statusCode = 400;
            $data = ["status" => "error", "errorMsg" => "Unknown roled id " . (int) $roleId];
        }
        return new \WHMCS\Http\Message\JsonResponse($data, $statusCode);
    }
    ...
    public function selectOptions(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        return new \WHMCS\Http\Message\JsonResponse(["data" => $selectOptions]);
    }
}
