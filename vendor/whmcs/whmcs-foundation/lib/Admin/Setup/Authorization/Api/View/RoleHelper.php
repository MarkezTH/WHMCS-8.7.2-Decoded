...
class RoleHelper
{
    public function getTableData($roles)
    {
        ...
        foreach ($roles as $role) {
            ...
            $tableData[] = ["btnExpand" => "<i class=\"fas fa-caret-right text-muted\" aria-hidden=\"true\"></i>", "name" => $role->role, "description" => $role->description, "btnGroup" => $this->getActionBtnGroup($role->id), "allowedActions" => $allowedActions];
        }
        return $tableData;
    }
