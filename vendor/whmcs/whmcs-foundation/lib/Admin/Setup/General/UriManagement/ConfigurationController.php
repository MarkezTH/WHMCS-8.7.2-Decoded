...
class ConfigurationController
{
    ...
    public function detectRouteEnvironment(\WHMCS\Http\Message\ServerRequest $request)
    {
        if (!$this->isAuthorizedRequest($request)) {
            return new \WHMCS\Http\Message\JsonResponse(["status" => "error", "errorMessage" => "Access Denied"], "403");
        }
        ...
        $data = ["mode" => $this->getModeFromUri($requestUri)];
    ...
    public function remoteDetectEnvironmentMode(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        if (!empty($pathComparison) && in_array($pathComparison, [self::PATH_COMPARISON_TEST, self::PATH_COMPARISON_INDEX])) {
            $this->setRelativePath($pathComparison);
        }
        $mode = $this->queryEnvironmentMode();
        $responseData = ["status" => "okay", "data" => ["mode" => $mode]];
        if ($request->get("setDetected")) {
            $subRequest = (new \WHMCS\Http\Message\ServerRequest())->withParsedBody(["mode" => $mode, "setOverride" => false]);
    ...
    public function setEnvironmentMode(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        try {
            ...
            $responseData = ["status" => "okay", "successMessage" => "Path Mode Value set to \"" . $valueName . "\""];
        } catch (\Exception $e) {
            $responseData = ["status" => "error", "errorMessage" => $e->getMessage()];
        }
        return new \WHMCS\Http\Message\JsonResponse($responseData);
    }
    ...
    {
        return [self::ROUTE_TYPE_REWRITE, self::ROUTE_TYPE_ACCEPTPATHINFO, self::ROUTE_TYPE_BASIC];
    }
    ...
    protected function queryEnvironmentMode()
    {
        try {
            ...
            $client = new \WHMCS\Http\Client\HttpClient(["verify" => false, \GuzzleHttp\RequestOptions::HTTP_ERRORS => false, "timeout" => 10]);
            $modes = \WHMCS\Route\UriPath::getAllKnownModes();
            $relativePath = $this->getRelativePath();
            foreach ($this->getAllKnowRouteTypes() as $path) {
                $path = $baseUrl . $relativePath . $path;
                $response = $client->get($path, ["query" => ["token" => $token]]);
    ...
    public function updateUriManagementSetting(\WHMCS\Http\Message\ServerRequest $request)
    {
        if (!$request->has("state")) {
            return new \WHMCS\Http\Message\JsonResponse(["status" => "error", "errorMessage" => "Missing required \"state\" parameter"]);
        }
        if (!$request->has("setting")) {
            return new \WHMCS\Http\Message\JsonResponse(["status" => "error", "errorMessage" => "Missing required \"setting\" parameter"]);
        }
        $settingToAffect = $request->get("setting");
        if (!in_array($settingToAffect, [static::SETTING_MODE_OVERRIDE, static::SETTING_AUTO_MANAGE])) {
            return new \WHMCS\Http\Message\JsonResponse(["status" => "error", "errorMessage" => "Invalid \"setting\" parameter"]);
        }
        $state = $request->get("state");
        $state = in_array($state, ["true", 1, "1", "on", true], true) ? true : false;
    ...
    private function setUriPathModeSetting($state, $queryEnvironment = true)
    {
        ...
        $result = ["status" => "okay", "successMessage" => "Path Mode Override " . $stateName];
    ...
    private function setUriRewriteSetting($state)
    {
        ...
        return new \WHMCS\Http\Message\JsonResponse(["status" => "okay", "successMessage" => "Auto-Managed Rewrite " . $stateName]);
    }
            ...
            if ($e instanceof \WHMCS\Exception\Information) {
                ...
                try {
                    ...
                    if ($rewriteFile->isExclusivelyWhmcs() || $rewriteFile->isEmpty()) {
                        $responseData = ["status" => "prompt", "promptMessage" => "Rewrite Auto-Management must be enabled before any modification to " . $htaccessFile, "promptTitle" => "Enable Rewrite Auto-Management"];
                    } else {
                        if ($rewriteFile) {
                            $rewriteFile->flock(LOCK_UN);
                            unset($rewriteFile);
    ...
    public function view(\Psr\Http\Message\ServerRequestInterface $request)
    {
        ...
        $desc = "URI Path Management determines how links will be generated on page render, as well as how WHMCS will process and route HTTP requests.";
        ...
        $desc2 = nl2br($desc2);
        $content .= "<p>" . $desc . "</p>";
        $content .= $aInt->beginAdminTabs(["Path Mode", "Rewrite File"], true, "UriMgmt");
        $whmcsRules = implode("<br/>", array_merge([\WHMCS\Route\Rewrite\File::MARKER_BEGIN], \WHMCS\Input\Sanitize::makeSafeForOutput((new \WHMCS\Route\Rewrite\RuleSet())->generateRuleSet()), [\WHMCS\Route\Rewrite\File::MARKER_END]));
    ...
    {
        $rewriteFile = "";
        $result = ["status" => "okay", "successMessage" => "Rewrite Rules Updated", "state" => "Up To Date"];
        try {
            ...
            $result = ["status" => "error", "errorMessage" => $e->getMessage()];
        }
        if ($rewriteFile) {
            $rewriteFile->flock(LOCK_UN);
            unset($rewriteFile);
