...
class Model
{
    ...
    public function getHero(\WHMCS\Apps\App\Collection $apps)
    {
        $heros = (new \WHMCS\Apps\Hero\Collection($this->data["heros"] ?? []))->get();
        foreach ($heros as $hero) {
            if (!$apps->get($hero->getTargetAppKey())->isActive()) {
                return $hero;
            }
        }
        return NULL;
