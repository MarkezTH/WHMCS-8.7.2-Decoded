...
class DailyCronHelper implements \Symfony\Component\Console\Helper\HelperInterface
{
    ...
    public function sendDailyCronDigest()
    {
        ...
        $hookResults = run_hook("DailyCronJobPreEmail", []);
        ...
        if (\WHMCS\Config\Setting::getValue("DisableEmailSending")) {
            $sendReport = false;
            $reason = " per the email sending disabled option";
        }
        if ($this->io->isDebug()) {
    ...
    {
        $this->status->setLastDailyCronEndTime();
