...
class UpdateDomainExpiryStatus extends \WHMCS\Scheduling\Task\AbstractTask
{
    ...
    protected $outputs = ["expired" => ["defaultValue" => 0, "identifier" => "expired", "name" => "Domains Set to Expired"], "grace" => ["defaultValue" => 0, "identifier" => "grace", "name" => "Domains Set to Grace Period"], "redemption" => ["defaultValue" => 0, "identifier" => "redemption", "name" => "Domains Set to Redemption Grace Period"]];
    ...
    private function updateDomainsByStatus(\Illuminate\Database\Eloquent\Collection $domains, $status)
    {
        ...
        if ($affectedDomainsCount) {
            ...
            \WHMCS\Domain\Domain::whereIn("id", $affectedDomainIds)->update(["status" => $status]);
        }
        $allowedOutputKeys = ["expired", "grace", "redemption"];
    ...
    {
        return \WHMCS\Domain\Domain::whereIn("status", ["Active", "Grace", "Redemption"])->where("expirydate", "<", date("Y-m-d"))->where("expirydate", "!=", "00000000")->get();
    }
