...
class OverageBilling extends \WHMCS\Scheduling\Task\AbstractTask
{
    ...
    protected $outputs = ["invoice.created" => ["defaultValue" => 0, "identifier" => "invoice.created", "name" => "Total Overage Billing Invoices"]];
    ...
    public function __invoke()
    {
        ...
        $result = select_query("tblproducts", "id,name,overagesenabled,overagesdisklimit,overagesbwlimit,overagesdiskprice,overagesbwprice", ["overagesenabled" => ["sqltype" => "NEQ", "value" => ""]]);
        while ($data = mysql_fetch_array($result)) {
            ...
            if (!$overagesenabled) {
                \WHMCS\Database\Capsule::table("tblproducts")->where("id", $pid)->update(["overagesenabled" => ""]);
            } else {
                ...
                while ($data = mysql_fetch_array($result2)) {
                    ...
                    $result3 = select_query("tblcurrencies", "rate", ["id" => $currency]);
                    ...
                    if (0 < $diskoverage) {
                        ...
                        insert_query("tblbillableitems", ["userid" => $userid, "description" => $diskoveragedesc, "amount" => $diskoverageamount, "recur" => 0, "recurcycle" => 0, "recurfor" => 0, "invoiceaction" => $invoiceaction, "duedate" => date("Y-m-d")]);
                    }
                    if (0 < $bwoverage) {
                        if ($bwoverage < 0) {
                            $bwoverage = 0;
                        ...
                        insert_query("tblbillableitems", ["userid" => $userid, "description" => $bwoveragedesc, "amount" => $bwoverageamount, "recur" => 0, "recurcycle" => 0, "recurfor" => 0, "invoiceaction" => $invoiceaction, "duedate" => date("Y-m-d")]);
                    }
                }
            }
        }
