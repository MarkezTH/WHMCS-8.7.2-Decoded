...
class AddLateFees extends \WHMCS\Scheduling\Task\AbstractTask
{
    ...
    protected $outputs = ["invoice.latefees" => ["defaultValue" => 0, "identifier" => "invoice.latefees", "name" => "Late Fee Invoices"], "action.detail" => ["defaultValue" => "", "identifier" => "action.detail", "name" => "Action Detail"]];
    ...
    protected function addLateFeesToInvoices()
    {
        ...
        if ($configInvoiceLateFeeAmount != "0.00") {
            ...
            while ($data = mysql_fetch_array($result)) {
                ...
                $total = $data["total"];
                $lateFeeInvoiceCount = get_query_val("tblinvoiceitems", "COUNT(id)", ["type" => "LateFee", "invoiceid" => $invoiceid]);
                if (!$lateFeeInvoiceCount) {
                    ...
                    getUsersLang($userid);
                    insert_query("tblinvoiceitems", ["userid" => $userid, "type" => "LateFee", "invoiceid" => $invoiceid, "description" => sprintf("%s (%s %s)", \Lang::trans("latefee"), \Lang::trans("latefeeadded"), fromMySQLDate(date("Y-m-d"))), "amount" => $latefeeamount, "duedate" => $duedate, "paymentmethod" => $paymentmethod, "taxed" => $taxlatefee]);
                    if (!function_exists("updateInvoiceTotal")) {
                        include_once ROOTDIR . "/includes/invoicefunctions.php";
                    }
                    updateInvoiceTotal($invoiceid);
                    run_hook("AddInvoiceLateFee", ["invoiceid" => $invoiceid]);
                    try {
                        sendMessage(\WHMCS\Mail\Template::where("name", "Invoice Modified")->firstOrFail(), $invoiceid);
                    } catch (\Exception $e) {
                    }
                    $invoiceids[] = $invoiceid;
                    $this->addSuccess(["invoice", $invoiceid, ""]);
                }
            }
        }
