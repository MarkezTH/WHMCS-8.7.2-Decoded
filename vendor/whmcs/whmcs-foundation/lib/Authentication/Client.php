...
class Client
{
    ...
    protected function dispatchLoginShare($username, $password)
    {
        ...
        $data = ["matchFound" => false, "newUser" => false, "user" => NULL];
        foreach ($hookResults as $hookData) {
            if ($hookData) {
                ...
                if ($hookId) {
                    ...
                    $tmpClient = \WHMCS\User\Client::firstOrNew(["email" => $hookEmail]);
                }
                if ($tmpClient->exists) {
                    $data["user"] = $tmpClient;
                } else {
    ...
    {
        $whmcs = \DI::make("app");
        $userIp = $whmcs->get_config("DisableSessionIPCheck") ? "" : \WHMCS\Utility\Environment\CurrentRequest::getIP();
