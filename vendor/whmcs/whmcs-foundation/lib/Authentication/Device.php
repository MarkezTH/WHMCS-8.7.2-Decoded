...
class Device extends \WHMCS\Model\AbstractModel
{
    ...
    protected $fillable = ["identifier", "secret", "compat_secret", "user_id", "is_admin", "role_ids", "description"];
    protected $booleans = ["is_admin"];
    protected $dates = ["last_access"];
    protected $casts = ["role_ids" => "json"];
    ...
    public static function boot()
    {
        ...
        static::deleted(function ($model) {
            if ($model->exists && $model->secret) {
                $model->update(["secret" => "", "compat_secret" => ""]);
            }
        });
    ...
    public static function newAdminDevice(\WHMCS\User\Admin $admin, $description = "")
    {
        ...
        $attributes = ["identifier" => static::generateIdentifier(), "secret" => $secret, "user_id" => $admin->id, "is_admin" => 1, "description" => (string) $description];
    ...
    {
        $this->role_ids = array_diff($this->role_ids, [(int) $role->getId()]);
        return $this;
    }
