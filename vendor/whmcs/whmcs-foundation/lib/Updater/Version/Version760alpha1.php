...
class Version760alpha1 extends IncrementalVersion
{
    ...
    {
        $existingConfiguration = \WHMCS\Database\Capsule::table("tblpaymentgateways")->where("gateway", "tco")->whereIn("setting", ["disablerecur", "forcerecur", "purchaseroutine"]);
        ...
        \WHMCS\Database\Capsule::table("tblpaymentgateways")->insert(["gateway" => "tco", "setting" => "integrationMethod", "value" => $integrationMethod]);
        \WHMCS\Database\Capsule::table("tblpaymentgateways")->insert(["gateway" => "tco", "setting" => "recurringBilling", "value" => $recurringBilling]);
        return $this;
    }
    ...
    protected function conditionallyActivateNewWeeblyPlans()
    {
        ...
        if ($weeblyServices->first()) {
            $weeblyServices->update(["product_ids" => "weebly_lite,weebly_starter,weebly_pro,weebly_business,weebly_performance"]);
    ...
    protected function updateWhmcsWhoisToWhmcsDomains()
    {
        ...
        if (!$query->count()) {
            \WHMCS\Database\Capsule::table("tblconfiguration")->insert(["setting" => "domainLookupProvider", "value" => "WhmcsDomains"]);
        } else {
            $query = \WHMCS\Database\Capsule::table("tblconfiguration")->where("setting", "domainLookupProvider")->whereIn("value", ["WhmcsWhois", "", "BasicWhois"]);
            if ($query->count()) {
                $query->update(["value" => "WhmcsDomains"]);
            }
        }
        return $this;
    }
    ...
    protected function conditionallyUpdateMaxMindTableValues()
    {
        ...
        $settings = $maxMindModule->pluck("setting")->toArray();
        if (count($settings)) {
            if (!in_array("userId", $settings)) {
                \WHMCS\Database\Capsule::table("tblfraud")->insert(["fraud" => "maxmind", "setting" => "userId", "value" => ""]);
            }
            if (!in_array("serviceType", $settings)) {
                \WHMCS\Database\Capsule::table("tblfraud")->insert(["fraud" => "maxmind", "setting" => "serviceType", "value" => "Score"]);
            }
            if (!in_array("ignoreAddressValidation", $settings)) {
                \WHMCS\Database\Capsule::table("tblfraud")->insert(["fraud" => "maxmind", "setting" => "ignoreAddressValidation", "value" => ""]);
            }
        }
    }
}
