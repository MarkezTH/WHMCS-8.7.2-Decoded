...
{
    protected $updateActions = ["addPrimaryKeysToTables", "migrateEUVatAddon", "createInvoiceDataTable", "correctSpamExpertsEmailTemplate", "migrateStorageConfigurations", "updateRecaptchaOptions", "createSslStatusTable", "createTransactionHistoryTable"];
    ...
    private function normalizePrimaryKey($tableName)
    {
        ...
        if ($pkColumn) {
            ...
            \WHMCS\Database\Capsule::schema()->table($tableName, function (\Illuminate\Database\Schema\Blueprint $table) {
                if ($idColumn) {
                    $table->primary("id");
                } else {
    ...
    {
        $tablesToCheck = ["mod_invoicedata", "tbladminperms", "tblaffiliates", "tblconfiguration", "tblknowledgebaselinks", "tbloauthserver_access_token_scopes", "tbloauthserver_authcode_scopes", "tbloauthserver_client_scopes", "tbloauthserver_user_authz_scopes", "tblpaymentgateways", "tblproductconfiglinks", "tblservergroupsrel"];
    ...
    {
        $oldFieldToNewField = ["enablevalidation" => "TaxEUTaxValidation", "vatcustomfield" => "TaxVatCustomFieldId", "homecountry" => "TaxEUHomeCountry", "taxexempt" => "TaxEUTaxExempt", "notaxexempthome" => "TaxEUHomeCountryNoExempt", "enablecustominvoicenum" => "TaxCustomInvoiceNumbering", "custominvoicenumber" => "TaxNextCustomInvoiceNumber", "enableinvoicedatepayment" => "TaxSetInvoiceDateOnPayment", "custominvoicenumautoreset" => "TaxAutoResetNumbering", "sequentialpaidautoreset" => "TaxAutoResetPaidNumbering", "custominvoicenumformat" => "TaxCustomInvoiceNumberFormat"];
        $existingConfig = \WHMCS\Database\Capsule::table("tbladdonmodules")->where("module", "eu_vat")->get()->all();
        if ($existingConfig) {
        ...
        (new \WHMCS\Module\LegacyModuleCleanup())->removeModulesIfInstalledAndUnused(["addons" => ["eu_vat"]]);
        return $this;
    }
    ...
    protected function correctSpamExpertsEmailTemplate()
    {
        ...
        foreach ($emailTemplates as $emailTemplate) {
            $search = ["{foreach \$required_mx_records as \$mx_host => \$mx_priority}", "{foreach \$required_mx_records as \$mx_host =&gt; \$mx_priority}"];
    ...
    public function migrateStorageConfigurations()
    {
        ...
        $assetTypeLocalPaths = [\WHMCS\File\FileAsset::TYPE_CLIENT_FILES => $attachmentsConfiguration, \WHMCS\File\FileAsset::TYPE_DOWNLOADS => $downloadsConfiguration, \WHMCS\File\FileAsset::TYPE_EMAIL_ATTACHMENTS => $attachmentsConfiguration, \WHMCS\File\FileAsset::TYPE_EMAIL_TEMPLATE_ATTACHMENTS => $downloadsConfiguration, \WHMCS\File\FileAsset::TYPE_PM_FILES => $pmFilesConfiguration, \WHMCS\File\FileAsset::TYPE_TICKET_ATTACHMENTS => $attachmentsConfiguration];
        ...
        \WHMCS\Database\Capsule::table("tbladminperms")->insert(["roleid" => 1, "permid" => 147]);
        return $this;
    }
    ...
    protected function updateRecaptchaOptions()
    {
        ...
        \WHMCS\Config\Setting::whereIn("setting", ["ReCAPTCHAInvisible", "ReCAPTCHAForms"])->delete();
        return $this;
    }
