...
{
    protected $updateActions = ["insertGatewayModuleHooks", "pruneOrphanedSslOrders", "registerRemoveTicketAttachmentsCronTask", "updateLinkInSpamExpertsWelcomeEmail", "removeGooglePlus1", "removeUnusedLegacyModules", "updateCreditCardInvoiceCreatedEmailTemplate"];
    ...
    public function insertGatewayModuleHooks()
    {
        ...
        $value = implode(",", $value);
        \WHMCS\Database\Capsule::table("tblconfiguration")->insert(["setting" => "GatewayModuleHooks", "value" => $value, "created_at" => date("Y-m-d H:i:s"), "updated_at" => date("Y-m-d H:i:s")]);
        return $this;
    }

    public function getUnusedLegacyModules()
    {
        return ["gateways" => ["camtech", "cyberbit", "imsp", "fasthosts", "ntpnow", "paymex", "payza", "ematters"]];
    }
