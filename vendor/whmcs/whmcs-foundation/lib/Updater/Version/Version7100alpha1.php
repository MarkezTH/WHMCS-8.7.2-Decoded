...
{
    protected $updateActions = ["removeUnusedLegacyModules", "createKbImageTable", "createKbFileAssetSetting", "createSuggestedTldsSetting", "convertEmailMarketerData", "addWeeblyFreeProductAndAddon", "updateWeeblySettingsWithIncludeFree", "addMarketgooWelcomeEmail", "updateCreditCardExpiringSoon"];
    ...
    public function createSuggestedTldsSetting()
    {
        ...
        if ($domainLookupProvider == "Registrar" && !empty($domainLookupRegistrar) && (is_array($availableTlds) || !empty($availableTlds))) {
            $suggestedTldsSetting = \WHMCS\Domains\DomainLookup\Settings::firstOrNew(["registrar" => \WHMCS\Config\Setting::getValue("domainLookupRegistrar"), "setting" => "suggestTlds"]);
