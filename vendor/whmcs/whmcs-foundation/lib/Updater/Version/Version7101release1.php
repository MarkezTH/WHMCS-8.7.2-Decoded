...
{
    protected $updateActions = ["addWeeblyFreeProductId", "fixWeeblyFreeAddonMissingModuleConfig", "fixWeeblySettingIncludeFreeLocation", "fixWeeblyProductSortOrder", "fixWeeblyFreeStockControl"];
    ...
    {
        $sortingMap = ["weebly_free", "weebly_lite", "weebly_starter", "weebly_pro", "weebly_business"];
        foreach ($sortingMap as $order => $configoption1) {
            \WHMCS\Product\Product::where("servertype", "marketconnect")->where("configoption1", $configoption1)->update(["order" => $order + 1]);
        }
        return $this;
    }
