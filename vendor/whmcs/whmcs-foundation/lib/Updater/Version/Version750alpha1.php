...
class Version750alpha1 extends IncrementalVersion
{
    ...
    protected function replaceTicketTidInEmailNotificationsSubject()
    {
        ...
        foreach ($emailTemplates as $emailTemplate) {
            if (stripos($emailTemplate->subject, "\$tickettid") !== false) {
                $emailTemplate->subject = str_replace(["\$tickettid"], ["\$ticket_tid"], $emailTemplate->subject);
                $emailTemplate->save();
            }
        }
        return $this;
    ...
    {
        return ["addons" => ["fixed_invoice_data"], "admin" => ["ip_manager", "ip_monitor"], "fraud" => ["telesign", "varilogix_fraudcall"], "gateways" => ["alertpay", "bidpay", "egold", "eway", "ewayuk", "googlecheckout", "ideal", "internetsecure", "libertyreserve", "myideal", "openecho", "payoffline", "stormpay"], "registrars" => ["directi", "dottk", "netregistry", "planetdomains", "registerfly", "resellerclubbeta", "tppinternet", "ventraip"], "servers" => ["castcontrol", "dotnetpanel", "enkompass", "enomtruste", "ensimx", "fluidvm", "plesk10", "plesk8", "plesk9", "pleskreseller"]];
    }
    ...
    {
        $columns = ["dnsmanagement", "emailforwarding", "idprotection", "eppcode"];
    ...
    protected function convertConfigurationFileDomainRenewalGracePeriodsToDatabase()
    {
        ...
        ob_end_clean();
        $fullDomainRenewalGracePeriods = array_merge([".com" => "30", ".net" => "30", ".org" => "30", ".info" => "15", ".biz" => "30", ".mobi" => "30", ".name" => "30", ".asia" => "30", ".tel" => "30", ".in" => "15", ".mn" => "30", ".bz" => "30", ".cc" => "30", ".tv" => "30", ".eu" => "0", ".co.uk" => "97", ".org.uk" => "97", ".me.uk" => "97", ".us" => "30", ".ws" => "0", ".me" => "30", ".cn" => "30", ".nz" => "0", ".ca" => "30"], $DomainRenewalGracePeriods);
    ...
    {
        $md5Values = ["7556f88474b1aca229b73b6683735625", "c977a0f0f691a2f18f2a95fb16867bb8"];
    ...
    protected function removeThreeYearMarketConnectSslTerms()
    {
        ...
        foreach (\WHMCS\Product\Product::ssl()->get() as $product) {
            ...
            if (!is_null($triennial)) {
                \WHMCS\Database\Capsule::table("tblpricing")->where("type", "product")->where("relid", $product->id)->update(["tsetupfee" => "-1", "triennially" => "-1"]);
            }
        }
        return $this;
    }
