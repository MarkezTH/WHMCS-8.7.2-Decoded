...
class Version720alpha1 extends IncrementalVersion
{
    ...
    {
        $addons = \WHMCS\Database\Capsule::table("tblhostingaddons")->join("tblhosting", "tblhostingaddons.hostingid", "=", "tblhosting.id")->get(["tblhostingaddons.id", "tblhosting.userid"])->all();
        foreach ($addons as $addon) {
            \WHMCS\Database\Capsule::table("tblhostingaddons")->where("id", "=", $addon->id)->update(["userid" => $addon->userid]);
        }
        return $this;
    }
    ...
    {
        \WHMCS\Mail\Template::where("name", "=", "Quote Accepted Notification")->update(["type" => "admin"]);
        return $this;
    }

    protected function updateWhmcsUrlInClientSignUpEmailToBeALink()
    {
        $emailMd5s = ["4f19b67d8c47f8bbc3462248b9fbcde5", "8de6eb28b01cd20869a6bbacad1c2e13", "03ac3993c49f759cfddcab7f14798ba7"];
    ...
    {
        $registrationConfirmationEmailMd5s = ["f162fe721a621aae30968efe36ac0897", "7961987629acf56d7d67f3b0e15bbe22", "cddfa2d4ad6e4e2b1b6f3b6039f13ecc"];
    ...
    {
        $emailMd5s = ["d7de59e957cca29439d59c1c2515106e", "47d3ede936cec4f45fdf71a4a278d8de", "255a059e873eb3f1945a78b6738cea16"];
    ...
    {
        $emailMd5s = ["a01c30a2dac7095a026dda8b31e94084", "7be1174c0274d36efb3bfa0bcbfc7568"];
    ...
    {
        $emailMd5s = ["a3205ccf969438f00109492779f3c5b4", "3c5f61a81cef69bda0efe6b341991857"];
    ...
    {
        $emailMd5s = ["831a81a6329d183c1f5dd820c44ff359", "ee2dd5ee752bc7df0abc2bbfa69fd683"];
    ...
    {
        \WHMCS\Database\Capsule::table("tbladminperms")->insert(["roleid" => 1, "permid" => 139]);
    }

    protected function addManageMarketPlacePermission()
    {
        \WHMCS\Database\Capsule::table("tbladminperms")->insert(["roleid" => 1, "permid" => 141]);
    }
    ...
    {
        $roles = \WHMCS\Database\Capsule::table("tbladminperms")->where("permid", 75)->pluck("roleid")->all();
        foreach ($roles as $roleid) {
            \WHMCS\Database\Capsule::table("tbladminperms")->insert(["roleid" => $roleid, "permid" => 140]);
        }
        return $this;
    }
    ...
    public function conditionallyCreateHtaccessFile()
    {
        ...
        try {
            ...
            if ($rewriteFile->isExclusivelyWhmcs() || $rewriteFile->isEmpty()) {
                ...
                $request = (new \WHMCS\Http\Message\ServerRequest())->withQueryParams(["setting" => $configuration::SETTING_AUTO_MANAGE, "state" => true]);
                $configuration->updateUriManagementSetting($request);
            } else {
                $request = (new \WHMCS\Http\Message\ServerRequest())->withQueryParams(["setting" => $configuration::SETTING_AUTO_MANAGE, "state" => false]);
                $configuration->updateUriManagementSetting($request);
            }
        } catch (\Exception $e) {
    ...
    public function detectAndSetUriPathMode()
    {
        ...
        try {
            ...
            if (!empty($body["data"]["mode"])) {
                ...
                $request = (new \WHMCS\Http\Message\ServerRequest())->withQueryParams(["mode" => $mode]);
                $configuration->setEnvironmentMode($request);
            } else {
                $request = (new \WHMCS\Http\Message\ServerRequest())->withQueryParams(["mode" => \WHMCS\Route\UriPath::MODE_BASIC]);
                $configuration->setEnvironmentMode($request);
            }
        } catch (\Exception $e) {
