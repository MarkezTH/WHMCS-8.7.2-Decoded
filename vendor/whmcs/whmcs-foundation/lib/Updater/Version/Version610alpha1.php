...
{
    protected $updateActions = ["migrateMaxMindIgnoreCity", "moveAttachmentsProjectsFiles", "detectCronRunForHealthAndUpdates"];
    ...
    protected function migrateMaxMindIgnoreCity()
    {
        ...
        if (0 < $maxmindFraudSetting) {
            \WHMCS\Database\Capsule::table("tblfraud")->where("fraud", "maxmind")->where("setting", "Do Not Include City")->update(["setting" => "Do Not Validate Address Information"]);
        }
        return $this;
    }
    ...
    {
        $cronJobCompletedWithinLast24Hours = \WHMCS\Database\Capsule::table("tblactivitylog")->where("description", "Cron Job: Completed")->whereBetween("date", [\WHMCS\Carbon::now()->subDay()->format("Y-m-d H:i:s"), \WHMCS\Carbon::now()->format("Y-m-d H:i:s")])->count();
        if (0 < $cronJobCompletedWithinLast24Hours) {
            \WHMCS\Database\Capsule::table("tbltransientdata")->insert(["name" => "cronComplete", "data" => true, "expires" => \WHMCS\Carbon::now()->addDay()->timestamp]);
        }
        return $this;
    }
}
