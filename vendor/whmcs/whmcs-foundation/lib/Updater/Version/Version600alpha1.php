...
{
    protected $updateActions = ["removeDuplicateSettings", "convertMailTemplateBooleanColumns", "convertClientUnixTimestampColumns", "convertClientBooleanColumns", "convertDomainBooleanColumns", "convertProductBooleanColumns", "convertProductGroupBooleanColumns", "convertDownloadBooleanColumns", "convertDownloadCategoryBooleanColumns", "migrateProductDownloadIdsToItsTable", "migrateProductUpgradeIdsToItsTable", "convertServiceBooleanColumns", "convertAnnouncementBooleanColumns", "updateAdminUserForAutoReleaseModule", "createServiceUnsuspendedEmailTemplate", "addManualUpgradeRequiredEmailTemplate", "convertNoMD5Passwords", "migrateDiscontinuedOrderFormTemplates", "migrateDiscontinuedAdminOriginalTemplate", "convertContactUnixTimestampColumns"];
    ...
    {
        $distinctSettingNames = \Illuminate\Database\Capsule\Manager::table("tblconfiguration")->select("setting", "value")->distinct("setting")->get()->all();
        foreach ($distinctSettingNames as $distinctSetting) {
            $settings = \Illuminate\Database\Capsule\Manager::table("tblconfiguration")->where("setting", "=", $distinctSetting->setting)->get()->all();
            $i = 0;
            while ($i < count($settings) - 1) {
                ...
                if (\Illuminate\Database\Capsule\Manager::table("tblconfiguration")->where("setting", "=", $distinctSetting->setting)->count() == 0) {
                    \Illuminate\Database\Capsule\Manager::table("tblconfiguration")->insert(["setting" => $distinctSetting->setting, "value" => $settings[$i]->value, "created_at" => $settings[$i]->created_at, "updated_at" => $settings[$i]->updated_at]);
                } else {
                    $i++;
                }
            }
    ...
    {
        $columns = ["disabled", "custom", "plaintext"];
    ...
    {
        $columns = ["taxexempt", "latefeeoveride", "overideduenotices", "separateinvoices", "disableautocc"];
        foreach ($columns as $column) {
    ...
    {
        $columns = ["dnsmanagement", "emailforwarding", "idprotection", "donotrenew", "synced"];
        foreach ($columns as $column) {
    ...
    {
        $columns = ["hidden", "showdomainoptions", "stockcontrol", "proratabilling", "configoptionsupgrade", "tax", "affiliateonetime", "retired"];
        foreach ($columns as $column) {
    ...
    {
        $columns = ["clientsonly", "hidden", "productdownload"];
        foreach ($columns as $column) {
            ...
            foreach ($productsWithDownloads as $product) {
                ...
                if (is_array($downloads)) {
                    foreach ($downloads as $downloadId) {
                        \Illuminate\Database\Capsule\Manager::table("tblproduct_downloads")->insert(["product_id" => $product->id, "download_id" => $downloadId]);
                    }
                }
            }
            ...
            foreach ($productsWithUpgrades as $product) {
                ...
                if (is_array($upgrades)) {
                    foreach ($upgrades as $upgradeProductId) {
                        \Illuminate\Database\Capsule\Manager::table("tblproduct_upgrade_products")->insert(["product_id" => $product->id, "upgrade_product_id" => $upgradeProductId]);
                    }
                }
            }
            \Illuminate\Database\Capsule\Manager::schema()->table($productModel->getTable(), function ($table) {
    ...
    {
        $admin = \WHMCS\User\Admin::where("disabled", "=", false)->first(["id", "username", "firstname", "lastname"]);
        $adminToSave = $admin->id . "|" . $admin->firstname . " " . $admin->lastname . " (" . $admin->username . ")";
        $products = \Illuminate\Database\Capsule\Manager::table("tblproducts")->where("servertype", "=", "autorelease")->get()->all();
        foreach ($products as $product) {
            \Illuminate\Database\Capsule\Manager::table("tblproducts")->where("id", "=", $product->id)->update(["configoption7" => $adminToSave]);
        }
        return $this;
    }
    ...
    protected function convertNoMD5Passwords()
    {
        ...
        if (!empty($nomd5)) {
            ...
            $contacts = \Illuminate\Database\Capsule\Manager::table("tblcontacts")->get()->all();
            foreach ($contacts as $contact) {
                $password = generateClientPW(decrypt($contact->password));
                \Illuminate\Database\Capsule\Manager::table("tblcontacts")->where("id", "=", $contact->id)->update(["password" => $password]);
            }
            try {
    ...
    {
        $discontinuedTemplates = ["ajaxcart" => "cart", "web20cart" => "boxes"];
        foreach ($discontinuedTemplates as $discontinuedTemplate => $templateToMigrateTo) {
            \WHMCS\Product\Group::where("orderfrmtpl", "=", $discontinuedTemplate)->update(["orderfrmtpl" => $templateToMigrateTo]);
