...
{
    protected $updateActions = ["updateProductIdsInMarketConnectServices", "renameSymantecProductsAndDisable2YearCycles", "renameSymantecAddonsAndDisable2YearCycles", "updateSellingPriceOfDigiCertCertificates"];

    protected function updateProductIdsInMarketConnectServices()
    {
        $mcServices = \WHMCS\Database\Capsule::table("tblmarketconnect_services")->where("name", "symantec")->get()->all();
        foreach ($mcServices as $mcService) {
            $id = $mcService->id;
            $productIds = str_replace("symantec_", "digicert_", $mcService->product_ids);
            \WHMCS\Database\Capsule::table("tblmarketconnect_services")->where("id", $id)->update(["product_ids" => $productIds]);
        }
        return $this;
    }
    ...
    {
        $products = \WHMCS\Product\Product::marketConnect()->where("configoption1", "like", "symantec\\_%")->get()->all();
        $productIds = [];
        ...
        if (0 < count($productIds)) {
            \WHMCS\Database\Capsule::table("tblpricing")->where("type", "product")->whereIn("relid", $productIds)->update(["bsetupfee" => 0, "tsetupfee" => 0, "biennially" => -1, "triennially" => -1]);
        }
        return $this;
    }
    ...
    protected function renameSymantecAddonsAndDisable2YearCycles()
    {
        ...
        if (0 < count($addonIds)) {
            \WHMCS\Database\Capsule::table("tblpricing")->where("type", "addon")->whereIn("relid", $addonIds)->update(["bsetupfee" => 0, "tsetupfee" => 0, "biennially" => -1, "triennially" => -1]);
        }
        return $this;
    }
            ...
            switch ($product->moduleConfigOption1) {
                ...
                default:
                    foreach ($allCurrencies as $currency) {
                        \WHMCS\Database\Capsule::table("tblpricing")->where("type", "product")->where("relid", $product->id)->where("currency", $currency->id)->update(["annually" => convertCurrency($newCost, NULL, $currency->id, $usdCurrency->rate)]);
                    }
            }
        }
        ...
        foreach ($configurations as $configuration) {
            if ($configuration->productAddon && $configuration->productAddon->module == "marketconnect") {
                ...
                switch ($configuration->value) {
                    ...
                    default:
                        foreach ($allCurrencies as $currency) {
                            \WHMCS\Database\Capsule::table("tblpricing")->where("type", "addon")->where("relid", $configuration->entityId)->where("currency", $currency->id)->update(["annually" => convertCurrency($newCost, NULL, $currency->id, $usdCurrency->rate)]);
                        }
                }
            }
        }
