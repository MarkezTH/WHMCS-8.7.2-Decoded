...
{
    protected $updateActions = ["ensureOfflineCCTypeIsReplaced", "updateCreditCardPaymentConfirmation"];

    protected function ensureOfflineCCTypeIsReplaced()
    {
        \WHMCS\Database\Capsule::table("tblpaymentgateways")->where("setting", "type")->where("value", "OfflineCC")->update(["value" => \WHMCS\Module\Gateway::GATEWAY_CREDIT_CARD]);
        return $this;
    }

    protected function updateCreditCardPaymentConfirmation()
    {
        $md5Values = ["68c28a219187cd1131c06dbc35f491d9"];
