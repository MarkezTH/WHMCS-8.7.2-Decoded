...
class Version730alpha1 extends IncrementalVersion
{
    ...
    {
        \WHMCS\Database\Capsule::table("tbladminperms")->insert(["roleid" => 1, "permid" => 143]);
    }

    protected function rewordUnchangedClientSignUpEmail()
    {
        $md5Hashes = ["b3d53c45f3981f9c7d4d053455d46c58", "941c87834ee67e1b79160611538f9fc6", "2e2ee71771894abae85baed9c65286c7", "5f935b27378e06e052ce06405f99e806", "77a990c043a7f1dad14168bda8630373", "4f19b67d8c47f8bbc3462248b9fbcde5", "8de6eb28b01cd20869a6bbacad1c2e13", "03ac3993c49f759cfddcab7f14798ba7"];
    ...
    protected function syncApplicationLinksOnUpgrade()
    {
        ...
        $servers = \WHMCS\Database\Capsule::table("tblapplinks")->where("tblapplinks.is_enabled", "=", 1)->select("tblapplinks.module_name")->get()->all();
        $queue = new \WHMCS\Scheduling\Jobs\Queue();
        foreach ($servers as $server) {
            $serverIDs = \WHMCS\Database\Capsule::table("tblservers")->where("name", "=", $server->module_name)->select("id")->get()->all();
            foreach ($serverIDs as $serverID) {
                $queue->add($server->module_name . ".scope.link.clone", "WHMCS\\ApplicationLink\\Provision", "cloneScopeLink", [$serverID->id, $oldScopeName, $newScopeName], 0, false);
            }
        }
    }
    ...
    protected function cloneOauthScopeShoppingCartAddonsPivots()
    {
        ...
        if ($oldScope) {
            $data = [];
            $oldScopedClients = \WHMCS\Database\Capsule::table("tbloauthserver_client_scopes")->where("scope_id", $oldScope)->pluck("client_id")->all();
            $newScopedClients = \WHMCS\Database\Capsule::table("tbloauthserver_client_scopes")->where("scope_id", $newScope)->pluck("client_id")->all();
            $needScope = array_diff($oldScopedClients, $newScopedClients);
            foreach ($needScope as $clientId) {
                $data[] = ["client_id" => $clientId, "scope_id" => $newScope];
            }
            if ($data) {
