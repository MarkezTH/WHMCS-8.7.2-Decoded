...
class Version5312release1 extends IncrementalVersion
{
    ...
    protected function runUpdateCode()
    {
        ...
        $query = "INSERT INTO `tblemailtemplates`(`id`, `type`, `name`, `subject`, `message`, `attachments`, `fromname`,  `fromemail`, `disabled`, `custom`, `language`, `copyto`, `plaintext`) VALUES('', 'support', 'Closed Ticket Bounce Message', '" . $subject . "', '" . $message . "', '', '', '', '', '', '', '', 0);";
        mysql_query($query);
        ...
        if (!$templateExists) {
            ...
            $query = "INSERT INTO `tblemailtemplates`(`id`, `type`, `name`, `subject`, `message`, `attachments`, `fromname`,  `fromemail`, `disabled`, `custom`, `language`, `copyto`, `plaintext`) VALUES('', 'domain', 'Expired Domain Notice', '" . $subject . "', '" . $message . "', '', '', '', '', '', '', '', 0);";
            mysql_query($query);
        }
        $tableCreate = "CREATE TABLE IF NOT EXISTS `tbldomainreminders` (  `id` int(10) NOT NULL AUTO_INCREMENT,  `domain_id` int(10) NOT NULL,  `date` date NOT NULL,  `recipients` text COLLATE utf8_unicode_ci NOT NULL,  `type` tinyint(4) NOT NULL,  `days_before_expiry` tinyint(4) NOT NULL,  PRIMARY KEY (`id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8  COLLATE=utf8_unicode_ci AUTO_INCREMENT=1;";
