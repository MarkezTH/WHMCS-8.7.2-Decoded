...
class Provider
{
    protected static $providerNames = ["BasicWhois" => "\\WHMCS\\Domains\\DomainLookup\\Provider\\BasicWhois", "WhmcsWhois" => "\\WHMCS\\Domains\\DomainLookup\\Provider\\WhmcsWhois", "WhmcsDomains" => "WHMCS\\Domains\\DomainLookup\\Provider\\WhmcsDomains", "Registrar" => "\\WHMCS\\Domains\\DomainLookup\\Provider\\Registrar"];
    ...
    public static function getAvailableRegistrarProviders()
    {
        ...
        foreach ($registrars as $registrar) {
            ...
            if ($registrarModules->functionExists("CheckAvailability") || $registrarModules->functionExists("GetDomainSuggestions")) {
                $returnedRegistrars[$registrar] = ["checks" => $registrarModules->functionExists("CheckAvailability"), "suggestions" => $registrarModules->functionExists("GetDomainSuggestions"), "logo" => $registrarModules->getLogoFilename(), "name" => $registrarModules->getDisplayName(), "suggestionSettings" => $registrarModules->functionExists("DomainSuggestionOptions") ? $registrarModules->call("DomainSuggestionOptions") : []];
            }
        }
