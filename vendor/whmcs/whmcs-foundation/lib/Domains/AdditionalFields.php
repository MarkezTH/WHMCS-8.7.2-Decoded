...
class AdditionalFields
{
    ...
    protected function loadFieldsData()
    {
        ...
        if ($domainRegistrar) {
            ...
            if ($registrar->load($domainRegistrar)) {
                $result = $registrar->call("AdditionalDomainFields", ["tld" => substr($this->getTLD(), 1), "idprotection" => $idProtection, "type" => $this->domainType, "fields" => $fields]);
    ...
    {
        $placeholders = ["Countries", "CountryCodeMap"];
    ...
    protected function genFieldHTML($name, $type, $size, $options, $defaultval, $required)
    {
        ...
        if ($type == "text") {
            ...
            if ($required && defined("ADMINAREA")) {
                $input .= " *";
            }
        } else {
            if ($type == "dropdown") {
                $input = $frm->dropdown($name, $fieldoptions, $defaultval, "", "", "", "", "", "form-control w-100");
            } else {
                if ($type == "tickbox") {
    ...
    {
        $values = [];
        $result = select_query("tbldomainsadditionalfields", "name,value", ["domainid" => $domainID]);
    ...
    public function saveToDatabase($domainID, $creating = true)
    {
        ...
        foreach ($this->getFields() as $fieldKey => $values) {
            $name = $this->getConfigValue($fieldKey, "Name");
            $value = $this->getFieldValue($fieldKey);
            $additionalField = \WHMCS\Domain\AdditionalField::firstOrNew(["domainid" => $domainID, "name" => $name]);
            ...
            if ($additionalField->value != $value) {
                $changes[] = $name . " changed from '" . $additionalField->value . "' to '" . $value . "'";
    ...
    protected function processFieldOverrides(&$fields, $fieldMap, $overrideData)
    {
