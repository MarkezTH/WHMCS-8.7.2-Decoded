...
class Rss
{
    public function toXml(\Psr\Http\Message\ServerRequestInterface $request)
    {
        ...
        $response = new \Laminas\Diactoros\Response\TextResponse($xml, 200);
    ...
    protected function getXmlItems()
    {
        ...
        $result = select_query("tblannouncements", "*", ["published" => "1"], "date", "DESC");
        while ($data = mysql_fetch_array($result)) {
            $id = $data["id"];
            $date = $data["date"];
            $title = $data["title"];
            $announcement = $data["announcement"];
            $result2 = select_query("tblannouncements", "", ["parentid" => $id, "language" => $language]);
