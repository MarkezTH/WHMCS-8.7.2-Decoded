...
class Form
{
    ...
    public function dropdown($name, $values = [], $selected = "", $onchange = "", $anyopt = "", $noneopt = "", $size = "1", $id = "", $cssClass = "form-control select-inline", $dataAttributes = [])
    {
        ...
        if (count($dataAttributes)) {
            foreach ($dataAttributes as $dataName => $dataValue) {
                $code .= " data-" . $dataName . "=\"" . $dataValue . "\"";
            }
        }
        $code .= ">";
        if ($anyopt) {
            $code .= "<option value=\"0\">" . \AdminLang::trans("global.any") . "</option>";
        }
        if ($noneopt) {
            $code .= "<option value=\"0\">" . \AdminLang::trans("global.none") . "</option>";
        }
        if (is_array($values)) {
    ...
    public function dropdownWithOptGroups($name, $values = [], $selected = "", $onchange = "", $anyopt = "", $noneopt = "", $size = "1", $id = "", $cssClass = "form-control select-inline", $style = "")
    {
        $code = "<select name=\"" . $name . "\"";
        if (1 < $size) {
            $code .= " size=\"" . $size . "\"";
        ...
        if (is_array($values)) {
            foreach ($values as $groupName => $groupData) {
                if ($groupData) {
                    $code .= "<optgroup label=\"" . \AdminLang::trans($groupName) . "\">";
    ...
    public function radio($name, $values = [], $selected = "", $spacer = "<br />")
    {
        $code = "";
        foreach ($values as $k => $v) {
    ...
    {
        $code = "<p align=\"center\">" . $this->submit(\AdminLang::trans("global.savechanges"), "btn btn-primary") . " " . $this->reset(\AdminLang::trans("global.cancelchanges")) . "</p>";
        return $code;
    }
