...
class Application extends Init
{
    ...
    protected $dangerousVariables = ["_GET", "_POST", "_REQUEST", "_SERVER", "_COOKIE", "_FILES", "_ENV", "GLOBALS"];
    protected $phpSelf = NULL;
    const FILES_VERSION = "8.7.2-release.1";
    ...
    public function __construct(Config\AbstractConfig $config, Database $database = NULL)
    {
        ...
        $this->setRemoteIp(Utility\Environment\CurrentRequest::getIP());
        ...
        $this->clientTemplate = View\Template\Theme::factory(Config\Setting::getValue("Template"), Session::get("Template"), $this->get_req_var("systpl"));
        $carttpl = $this->isInRequest("carttpl") ? $this->get_req_var("carttpl") : NULL;
        View\Template\CompatUtil::initializeRuntimeOrderForm($this->clientTemplate, $carttpl);
    ...
    public function redirect($path = NULL, $vars = [], $prefix = "")
    {
        $url = $this->getRedirectUrl($path, $vars, $prefix);
        $this->fqRedirect($url);
    }

    public function fqRedirect($url)
    {
        $this->doRedirect($url);
    ...
    public function getRedirectUrl($path, $vars = [], $prefix = "")
    {
        if (!$path) {
    ...
    public function redirectToRoutePath($route, $routeVariables = [], $queryParameters = [])
    {
        ...
        if (isset($_SERVER["REQUEST_METHOD"])) {
            switch ($_SERVER["REQUEST_METHOD"]) {
                ...
                default:
                    ...
                    throw new Exception\Fatal($_SERVER["REQUEST_METHOD"] . " Request Method Not Allowed");
            }
        }
    ...
    {
        $excludedPages = ["banned.php", "includes/api.php"];
    ...
    {
        if ($this->isApiCall() || $this->isLocalApiRequest()) {
            return true;
        }
        if (isset($_SERVER["REQUEST_URI"]) && 0 < strlen($_SERVER["REQUEST_URI"])) {
            $path = $this->getRequestPath($_SERVER["REQUEST_URI"], $_SERVER["SCRIPT_NAME"]);
            if ((new Route\Middleware\RoutableApiRequestUri())->getApiVersionFromPath($path)) {
                return true;
            }
        }
        return false;
    ...
    protected function getRequestPath($uri, $scriptName)
    {
        $path = "";
        if (preg_match("/(rp=|index.php\\/)([^&]+)/", $uri, $matches)) {
            $path = $matches[count($matches) - 1];
        }
        return scoalesce($path, Utility\Environment\WebHelper::getRelativePath($uri, $scriptName));
    }
    ...
    public function isLocalApiRequest()
    {
        return \DI::make("runtimeStorage")->runningViaLocalApi === true;
    }
    ...
    {
        $appConf = $this->getApplicationConfig();
        if ($appConf->automationStatus && isset($appConf->automationStatus["InstallationTimestamp"])) {
            $installationTimestamp = $appConf->automationStatus["InstallationTimestamp"];
        } else {
            $installationTimestamp = Config\Setting::getValue("InstallationTimestamp");
        }
        if (is_numeric($installationTimestamp)) {
            return time() - 172800 < $installationTimestamp;
        }
        return false;
    }

    public function isRecentlyUpgraded()
    {
        if ($this->isNewInstallation()) {
            return true;
        }
        $upgradeTimestamp = Config\Setting::getValue("LastUpgradeTimestamp");
        if (is_numeric($upgradeTimestamp)) {
            return time() - 86400 < $upgradeTimestamp;
        }
        return false;
    }
