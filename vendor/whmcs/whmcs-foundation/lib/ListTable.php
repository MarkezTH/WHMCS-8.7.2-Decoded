...
class ListTable
{
    ...
    private $tablebg = "tablebg";
    private $hideInactiveTogglePages = ["addons", "clients", "domains", "services"];
    private $aInt = NULL;
    ...
    {
        $this->columns[] = $column;
        $orderByVals = $this->getPageObj()->getValidOrderByValues();
        if (1 < count($column)) {
            $orderByVals[] = $column[0];
        }
        $this->getPageObj()->setValidOrderByValues($orderByVals);
        return $this;
    }
    ...
    public function outputTableHeader()
    {
        ...
        $showing = "";
        if ($numResults) {
            $showing = ", " . \AdminLang::trans("global.showing", [":start" => $this->getPageObj()->getStartingNumber(), ":end" => $this->getPageObj()->getEndingNumber()]);
        }
        $content = "<form id=\"frmRecordsFound\" method=\"post\" action=\"" . $basePath . "filter=1\">\n    <div class=\"row\">\n        <div class=\"col-md-6 col-sm-12\">\n            " . $numResults . " " . \AdminLang::trans("global.recordsfound") . $showing . "\n        </div>\n        <div class=\"col-md-6 col-sm-12 text-right\">" . \AdminLang::trans("global.jumppage") . ":\n            <div class=\"right-margin-5 inline\">" . $hiddenOutput["right"] . "</div>";
        $content .= $this->getPageDropdown();
        $content .= "</div>\n    </div>\n</form>\n";
        $this->addOutput($content);
    }

    public function setTableBgClass($self, $class)
    {
        $this->tablebg = $class;
        return $this;
    }

    public function getTableBgClass()
    {
        return $this->tablebg;
    }
    ...
    public function outputTable()
    {
        ...
        if (count($rows)) {
                ...
                if ($vals[0] == "dividingline") {
                    ...
                    if ($vals[0] == "expandline") {
                        ...
                        foreach ($vals as $val) {
                            $tdAttributes = [];
                            if (is_array($val)) {
                                if (isset($val["tdAttributes"]) && is_array($val["tdAttributes"])) {
                                    $tdAttributes = $val["tdAttributes"];
                                }
                                $val = isset($val[0]) ? $val[0] : "";
                            }
                            $content .= "<td";
                            foreach ($tdAttributes as $tdKey => $tdValue) {
                                $content .= " " . $tdKey . "=\"" . $tdValue . "\"";
                            }
                            $content .= ">" . $val . "</td>";
                        }
                        $content .= "</tr>";
            ...
            if (0 < $this->getPageObj()->getHiddenCount()) {
                $message = \AdminLang::trans("global.hiddenRecordsFound", [":numHidden" => $this->getPageObj()->getHiddenCount()]);
    ...
    public function outputTablePagination()
    {
        ...
        $pageList = array_unique([1, 2, 3, $thisPage - 1, $thisPage, $thisPage + 1, $totalPages - 2, $totalPages - 1, $totalPages, $halfPages], SORT_NUMERIC);
    ...
    protected function getPageDropdown()
    {
        ...
        $thisPage = $this->getPageObj()->getPage();
        $totalPages = $this->getPageObj()->getTotalPages();
        $start = max(1, $thisPage - $maxPadActive);
        $end = min($totalPages, $thisPage + $maxPadActive);
        ...
        foreach ($pageList as $key => $page) {
            ...
            $next = $pageList[++$key] ?? false;
            if ($next && $next !== $page + 1) {
                $dropdownEntries .= "<li class=\"divider\"></li>";
            }
        }
    ...
    {
        $active = $this->getPageObj()->getPage() == $pageNumber ? " active" : "";
        return "<li class=\"text-center" . $active . "\">\n    <a href=\"#\" class=\"dropdown-page-selector\" data-page=\"" . $pageNumber . "\">\n        " . $pageNumber . "\n    </a>\n</li>";
    }
