...
class Client
{
    ...
    public function hasProductTypes($types)
    {
    ...
    public function getServices($serviceName = "")
    {
        ...
        foreach (array_keys(\WHMCS\MarketConnect\MarketConnect::getPromotionServices()) as $service) {
        ...
        $services = \WHMCS\Service\Service::with(["product", "addons" => function ($query) {
        ...
        foreach ($services as $serviceModel) {
            if ($serviceModel->product()->first()->module == "marketconnect") {
                ...
                $accounts[$type[0]][] = ["type" => "service", "id" => $serviceModel->id, "domain" => $serviceModel->domain];
            }
            foreach ($serviceModel->addons as $addon) {
                foreach ($addon->productAddon->moduleConfiguration as $moduleConfiguration) {
                    ...
                    $accounts[$type[0]][] = ["type" => "addon", "id" => $addon->id, "domain" => $serviceModel->domain];
                }
            }
        }
        return isset($accounts[$serviceName]) ? $accounts[$serviceName] : [];
    }
}
