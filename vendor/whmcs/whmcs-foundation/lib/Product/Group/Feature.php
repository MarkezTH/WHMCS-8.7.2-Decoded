...
class Feature extends \WHMCS\Model\AbstractModel
{
    ...
    public static function boot()
    {
        ...
        Feature::saved(function (Feature $feature) {
            if (\WHMCS\Config\Setting::getValue("EnableTranslations")) {
                $translation = \WHMCS\Language\DynamicTranslation::firstOrNew(["related_type" => "product_group_feature.{id}.feature", "related_id" => $feature->id, "language" => \WHMCS\Config\Setting::getValue("Language"), "input_type" => "text"]);
    ...
    {
        return $this->hasMany("WHMCS\\Language\\DynamicTranslation", "related_id")->where("related_type", "=", "product_group_features.{id}.feature")->select(["language", "translation"]);
    }
    ...
    public function getFeatureAttribute($feature)
    {
        $translatedFeature = "";
        if (\WHMCS\Config\Setting::getValue("EnableTranslations")) {
            $translatedFeature = \Lang::trans("product_group_features." . $this->id . ".feature", [], "dynamicMessages");
        }
