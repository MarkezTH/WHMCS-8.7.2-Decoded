...
class ConfigOptionSelection extends \WHMCS\Model\AbstractModel
{
    ...
    protected $casts = ["configid" => "integer", "optionname" => "string", "sortorder" => "integer", "hidden" => "boolean"];
    ...
    {
        $model = \WHMCS\Billing\Pricing::firstOrNew(["relid" => $this->id, "currency" => $currency->id, "type" => "configoptions"]);
        ...
        foreach ($expectedCycles as $cycleKey) {
            ...
            $pricing[$cycleKey] = new Pricing\Price(["cycle" => $cycleKey, "price" => $recurringPrice, "setupfee" => $setupFee]);
        }
        return $pricing;
    }
        ...
        if (is_null($this->hasLinearCyclePricing)) {
            ...
            foreach ($models as $model) {
                ...
                foreach ($expectedCycles as $cycle) {
                    ...
                    if ($cycle != "monthly") {
                        $price = new Pricing\Price(["cycle" => $cycle]);
                        if ($price->isYearly()) {
                        ...
                        if (0 < $maxDiffRecurring || 0 < $maxDiffSetup) {
                            $hasLinear = false;
    ...
    public function getPricingByCycle()
    {
        ...
        $pricing = array_fill_keys($expectedCycles, ["setup" => $currencyFill, "recurring" => $currencyFill]);
        foreach ($currencies as $currency) {
            $model = (new \WHMCS\Billing\Pricing())->firstOrNew(["relid" => $this->id, "type" => "configoptions", "currency" => $currency->id]);
            foreach ($expectedCycles as $cycle) {
                $setupField = substr($cycle, 0, 1) . "setupfee";
