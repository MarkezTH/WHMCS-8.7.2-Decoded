...
class Admin
{
    ...
    public $clientSearchSubmitUrl = NULL;
    protected $tabPrefix = "";
    public $smarty = "";
    ...
    protected $contentAreaClasses = ["contentarea"];
    public $isSetupPage = false;
    protected $popupwincount = NULL;
    ...
    public function __construct($reqpermission, $releaseSession = true)
    {
        ...
        if ($this->loginRequired && isset($_SESSION["adminid"])) {
            ...
            $twofa->setUser(User\Admin::getAuthenticatedUser());
        ...
        try {
            if (\AdminLang::getName() != $this->language) {
                ...
                $adminLang = \DI::make("adminlang", [$this->language]);
    ...
    {
        if ($this->isSetupPage || $this->sidebar == "config") {
            (new VisitTracking("setup", 10))->log($this->title);
        }
        ...
        if (count($this->chartFunctions)) {
            ...
            $this->extrajscode[] = "function redrawCharts() {\n    " . $chartredrawjs . "\n}";
            $this->extrajscode[] = "\$(window).resize(function() {\n    if (this.resizeTime) clearTimeout(this.resizeTime); //Prevent multiple redraws in a short time\n    this.resizeTime = setTimeout(function() {\n        \$(this).trigger('resizeEnd');\n    }, 500);\n});\n\$(window).on('resizeEnd', function() {\n    redrawCharts();\n});";
        }
    ...
    public function populateStandardAdminSmartyVariables()
    {
        ...
        $minSidebar = false;
        if (Cookie::get("MinSidebar")) {
            $minSidebar = true;
            $this->addContentAreaClass("sidebar-minimized");
        }
        $this->assign("minsidebar", $minSidebar);
        $this->assign("contentAreaClasses", $this->getContentAreaClassesForOutput());
        $this->assign("pageicon", $this->icon);
        if (!isset($this->templatevars["ticketfilterdata"])) {
            $this->assign("ticketfilterdata", ["view" => "", "deptid" => "", "subject" => "", "email" => ""]);
        }
        ...
        $this->assign("isUpdateAvailable", \App::isUpdateAvailable());
        $this->assign("showUpdateAvailable", \App::isUpdateAvailable() && $this->hasPermission("Update WHMCS"));
        $this->assign("isNewInstallation", \App::isNewInstallation());
        $this->assign("isRecentlyUpgraded", \App::isRecentlyUpgraded());
        $this->assign("isCronError", (new Cron\Status())->hasError());
        $this->assign("isCronWarning", (new Cron\Status())->hasWarning());
        $this->assign("versionHash", View\Helper::getAssetVersionHash());
        $this->assign("hasSetupMenuAccess", User\Admin\Permission::hasPermissionInGroup(User\Admin\Permission::PERMISSION_GROUP_SETUP));
        $this->assign("dateTimeNowFormatted", Carbon::now()->toAdminDateTimeFormat());
        $assetHelper = \DI::make("asset");
        $this->assign("WEB_ROOT", $assetHelper->getWebRoot());
        $this->assign("ADMIN_WEB_ROOT", Utility\Environment\WebHelper::getBaseUrl() . Admin\AdminServiceProvider::getAdminRouteBase());
        ...
        $this->assign("datepickerformat", str_replace(["DD", "MM", "YYYY"], ["dd", "mm", "yy"], $whmcs->get_config("DateFormat")));
        if (Session::get("adminid")) {
            ...
            $this->assign("anyGatewaysActivated", (bool) count(Module\GatewaySetting::getActiveGatewayFriendlyNames()));
            $this->assign("addon_modules", array_key_exists($adminRoleID, $addonModulesPermissions) ? $addonModulesPermissions[$adminRoleID] : []);
            $this->assign("adminLanguage", $admin->language);
        }
        $this->assign("intelligentSearch", ["autoSearchEnabled" => Search\IntelligentSearchAutoSearch::isEnabled()]);
        ...
        foreach ($locales as $locale) {
            if ($locale["language"] == \AdminLang::getName()) {
                ...
                if ($this->sidebar == "support") {
                    $params = ["includeCountsByStatus" => !$disableAdminTicketPageCounts];
                    ...
                    $departments = [];
                    $departmentsData = Support\Department::whereIn("id", $ticketStats["filteredDepartments"])->orderBy("order")->get(["id", "name"]);
    ...
    public function output($jsonEncodingOptions = NULL)
    {
        if ($this->isJsonResponseType()) {
            ...
            if ($this->isModalResponseType()) {
                ...
                return new Http\Message\JsonResponse(["body" => $jQueryCode . $this->content]);
            }
            if ($this->isJsonMessageResponseType()) {
                ...
                return new Http\Message\JsonResponse($data, 200, [], $jsonEncodingOptions);
            }
            ob_start();
            $whmcs = Application::getInstance();
            $hookvars = $this->templatevars;
    ...
    public function displayPopUp()
    {
        ...
        $view->setJquery([$this->jquerycode]);
        (new \Laminas\HttpHandlerRunner\Emitter\SapiEmitter())->emit($view);
        return $view;
    }
    ...
    public function sortableTableInit($defaultsort, $defaultorder = "ASC")
    {
        ...
        if ($defaultsort == "nopagination") {
            ...
            $xorderby = $sortdata[$sortpage . "orderby"] ?? NULL;
            $xorder = $sortdata[$sortpage . "order"] ?? NULL;
            if (!$xorderby) {
    ...
    public function sortableTable($columns, $tabledata, $formurl = "", $formbuttons = "", $topbuttons = "")
    {
        ...
        if ($this->tablePagination) {
            foreach ($_REQUEST as $key => $value) {
                if (!in_array($key, ["orderby", "page", "PHPSESSID", "token"]) && $value) {
                    if (is_array($value)) {
    ...
    protected function getClientsProfileHeader($clientId = 0)
    {
        $uid = NULL;
        if (isset($GLOBALS["userid"])) {
            $uid = (int) $GLOBALS["userid"];
        }
        $uid = ecoalesce($clientId, $uid);
        $tabarray = [];
        ...
        $tabarray["clients-users"] = ["label" => \AdminLang::trans("user.userTab"), "routePath" => routePath("admin-client-users", $uid)];
        ...
        $tabarray["clients-tickets"] = ["label" => \AdminLang::trans("clientsummary.tickets"), "routePath" => routePath("admin-client-tickets", $uid)];
        $tabarray["clientsemails"] = \AdminLang::trans("clientsummary.emails");
        $tabarray["clientsnotes"] = \AdminLang::trans("clientsummary.notes") . " (" . get_query_val("tblnotes", "COUNT(id)", ["userid" => $uid]) . ")";
        $tabarray["clientslog"] = \AdminLang::trans("clientsummary.log");
        $content = "<form action=\"" . $this->getClientSearchSubmitLocation() . "\" method=\"POST\">\n    <div class=\"row client-dropdown-container\">\n        <div class=\"col-md-5 col-sm-9\">\n            " . $this->clientsDropDown($uid) . "\n            <input type=\"submit\" id=\"goButton\" value=\"Go\" class=\"btn btn-primary hidden\" />\n        </div>\n    </div>\n</form>";
        $markup = new View\Markup\Markup();
        $clientnotes = [];
        $result = select_query("tblnotes", "tblnotes.*,(SELECT CONCAT(firstname,' ',lastname) FROM tbladmins WHERE tbladmins.id=tblnotes.adminid) AS adminuser", ["userid" => $uid, "sticky" => "1"], "modified", "DESC");
    ...
    public function cyclesDropDown($billingcycle, $any = "", $freeop = "", $name = "billingcycle", $onchange = "", $id = "")
    {
        ...
        if (in_array(strtolower($billingcycle), ["one time", "onetime"])) {
            $code .= " selected";
        }
    ...
    public function jqueryDialog($name, $title, $message, $buttons = [], $height = "", $width = "", $alerttype = "alert")
    {
    ...
    {
        if (!is_array($userid)) {
            $client = User\Client::find($userid);
            if ($client) {
                $clientFormatter = $client->formatter();
                if ($newWindow) {
                    $clientFormatter->openWindow();
                }
                return $clientFormatter->markup();
            }
        }
        $contactid = 0;
        if (is_array($userid)) {
        ...
        if (!$firstname && !$lastname && !$companyname) {
            if (isset($ClientOutputData[$userid])) {
                ...
                $result = select_query("tblclients", "firstname,lastname,companyname,groupid", ["id" => $userid]);
            ...
            if ($contactid) {
                if (isset($ContactOutputData[$contactid])) {
                    ...
                    $contactdata = get_query_vals("tblcontacts", "firstname,lastname", ["id" => $contactid, "userid" => $userid]);
        ...
        Config\Setting::getValue("ClientDisplayFormat");
    ...
    {
        $translated = \AdminLang::trans($section . "." . $var);
        if ($escape) {
            return addslashes($translated);
        }
        if ($translated == $section . "." . $var) {
            if (defined("DEVMODE")) {
    ...
    public function valUserID($tempuid)
    {
        ...
        if (!$userid) {
            $this->gracefulExit(\AdminLang::trans("clients.invalidclient"));
        }
    ...
    {
        $plugins = ["autosave", "print", "preview", "searchreplace", "autolink", "directionality", "visualblocks", "visualchars", "fullscreen", "image", "link", "media", "template", "code", "codesample", "table", "charmap", "hr", "pagebreak", "nonbreaking", "anchor", "insertdatetime", "advlist", "lists", "textcolor", "wordcount", "contextmenu", "colorpicker", "textpattern", "help"];
        switch ($location) {
            case "emailTemplates":
            case "sendMessage":
    ...
    public function beginAdminTabs($tabs = [], $defaultTabOpen = false, $tabPrefix = "")
    {
        ...
        foreach ($tabs as $i => $label) {
            ...
            $code .= "<li" . $isTabActive . ">" . "<a class=\"tab-top\" href=\"#tab" . $tabPrefix . $tabIDNum . "\" role=\"tab\" data-toggle=\"tab\"" . " id=\"tabLink" . $tabPrefix . $tabIDNum . "\" data-tab-id=\"" . $tabIDNum . "\">" . $label . "</a></li>" . PHP_EOL;
        }
        $isActive = "";
        if ($selectedTab == 1) {
            $isActive = " active";
    ...
    {
        $modalOutput = View\Helper::confirmationModal($name, $question, $url);
        $this->extrajscode[] = "function " . $name . "(id) {\n    \$('#" . $name . "').find('.id-target').val(id).end().modal('show');\n}";
        return $modalOutput;
    }

    public function clientSearchDropdown($name, $selectedOption = "", $options = [], $placeHolder = "", $valueAttribute = "id", $tabOrder = 0)
    {
    ...
    public function performClientSearch($searchTerm)
    {
        ...
        foreach ($matchingClients->get() as $client) {
            $searchResults[] = ["id" => $client->id, "name" => Input\Sanitize::decode($client->firstname . " " . $client->lastname), "companyname" => Input\Sanitize::decode($client->companyname), "email" => Input\Sanitize::decode($client->email)];
        }
    ...
    public function flash($title, $message, $status = "success")
    {
        ...
        $flashMessages[\App::getPhpSelf()] = ["title" => $title, "message" => $message, "status" => $status];
    ...
    public function formatImportantClientNotes($notes)
    {
    ...
    public function getTranslationLink($type, $id, $customFieldType = "")
    {
        ...
        $linkId = "translate" . str_replace(" ", "", ucwords(str_replace([".", "_"], " ", $type)));
        ...
        if ($customFieldType && in_array($customFieldType, ["addon", "client", "product", "support"])) {
    ...
    {
        $previousPost = !empty($_POST) ? $_POST : [];
        foreach (["confirmpw", "token", "authconfirm"] as $key) {
        ...
        if (!$this->hasAuthConfirmation()) {
            ...
            if ($this->isJsonResponseType()) {
                $this->setBodyContent(["error" => \AdminLang::trans("login.auth.passwordConfirmationRequired") . " " . \AdminLang::trans("global.reloadTryAgain")]);
            } else {
    ...
    {
        $data = self::getFeatureHighlightsByAdmin();
        if (!is_array($data)) {
            $data = [];
        }
    ...
    {
        $adminId = $this->getAdminID();
        Database\Capsule::table("tbladmins")->where("id", $adminId)->update(["template" => $templateName]);
    }

    public function setClientSearchSubmitLocation($clientSearchPostUrl)
    {
        $this->clientSearchSubmitUrl = $clientSearchPostUrl;
        return $this;
    }

    protected function getClientSearchSubmitLocation()
    {
        if (is_null($this->clientSearchSubmitUrl)) {
            $this->clientSearchSubmitUrl = routePath("admin-client-search-submit-location", \App::getCurrentFilename());
        }
        return $this->clientSearchSubmitUrl;
    }

    public function addContentAreaClass($className)
    {
        $this->contentAreaClasses[] = $className;
        return $this;
    }

    public function getContentAreaClassesForOutput()
    {
        return implode(" ", $this->contentAreaClasses);
    }

    protected static function getFeatureHighlightsByAdmin()
    {
        $data = NULL;
        $setting = Config\Setting::getValue("FeatureHighlightsByAdmin");
