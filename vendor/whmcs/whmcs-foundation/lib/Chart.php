...
class Chart
{
    ...
    public function drawChart($type, $data, $args = [], $height = "300px", $width = "100%")
    {
        global $aInt;
        ...
        if ($datafunc && isset($_POST["chartdata"]) && $_POST["chartdata"] == $datafunc) {
            ...
            if ($this->chartcount == 0) {
                if (is_string($aInt->headOutput)) {
                    $aInt->headOutput .= "<script src=\"https://www.gstatic.com/charts/loader.js\"></script>";
                } else {
                    if (is_array($aInt->headOutput)) {
                        $aInt->addHeadOutput("<script src=\"https://www.gstatic.com/charts/loader.js\"></script>");
                    }
                }
            }
            $this->chartcount++;
            ...
            $output = "\n            <script type=\"text/javascript\">\n            google.charts.load(\"current\", {packages: [\"" . ($type == "Geo" ? "geochart" : "corechart") . "\"]});\n            google.charts.setOnLoadCallback(" . $chartName . ");\n            function " . $chartName . "() {";
            if ($datafunc) {
            ...
            $heightNumeric = intval(preg_replace("/[^\\d]+/", "", $height));
            $output .= "\n        var data = new google.visualization.DataTable(jsonData);\n        var options = { " . implode(",", $options) . " };\n        var chart = new google.visualization." . $type . "Chart(document.getElementById(\"chartcont" . $chartUniqueId . "\"));\n        chart.draw(data,options);\n        }\n        </script>\n        <div id=\"chartcont" . $chartUniqueId . "\" style=\"width:" . $width . ";height:" . $height . ";\"><div style=\"padding-top:" . round($heightNumeric / 2 - 10, 0) . "px;text-align:center;\"><img src=\"images/loading.gif\" /> Loading...</div></div>\n        ";
