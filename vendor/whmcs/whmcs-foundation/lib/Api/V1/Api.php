...
class Api
{
    ...
    public function setAdminUser($user)
    {
        $adminId = 0;
        if (is_numeric($user)) {
            $admin = \Illuminate\Database\Capsule\Manager::table("tbladmins")->find($user, ["id"]);
            if (!is_null($admin)) {
                $adminId = $admin->id;
            }
        } else {
            $admin = \Illuminate\Database\Capsule\Manager::table("tbladmins")->where("username", "=", $user)->first(["id"]);
            if (!is_null($admin)) {
                $adminId = $admin->id;
            }
        }
    ...
    {
        $apiLicense = new \WHMCS\Api\Licensing\ApiLicense();
        if ($action) {
            $this->setAction($action);
        }
        if (!$apiLicense->isValidForAction((string) $this->getAction())) {
            $this->response = ["result" => "error", "message" => $apiLicense->getErrorMessage()];
            return $this;
        }
