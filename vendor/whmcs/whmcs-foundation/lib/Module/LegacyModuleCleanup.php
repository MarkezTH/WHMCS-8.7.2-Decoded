...
class LegacyModuleCleanup
{
    ...
    private function isModuleUsed($moduleName, $moduleType)
    {
        ...
        switch ($moduleType) {
            ...
            case AbstractModule::TYPE_GATEWAY:
                ...
                break;
            case AbstractModule::TYPE_MAIL:
                $mailConfig = Mail::getStoredConfiguration();
                if (!empty($mailConfig["module"]) && $mailConfig["module"] === $moduleName) {
                    return true;
                }
                return false;
                break;
    ...
    public function removeModule($moduleName, $moduleType)
    {
        ...
        foreach ($moduleAssets as $asset) {
            if (is_dir($asset)) {
                \WHMCS\Utility\File::recursiveDelete($asset, [], true);
            } else {
    ...
    public function removeModulesIfInstalledAndUnused($modules)
    {
        $modulesInUse = [];
        $modulesUnusedAndDeleted = [];
        $modulesFailedDeletion = [];
        ...
        if (!empty($adminNotificationMessageLines)) {
            ...
            $queue->add(self::JOB_UPDATER_ADMIN_NOTIFICATION, "WHMCS\\Mail\\Job\\AdminNotification", "send", [$to, $subject, $message], 0, true);
        }
    }
}
