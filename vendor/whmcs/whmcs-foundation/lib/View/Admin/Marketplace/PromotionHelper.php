...
class PromotionHelper
{
    ...
    public function fetchPromoContent($partner, $params = [])
    {
        ...
        $params = $params + ["params" => ["licenseKey" => $whmcs->get_license_key(), "registeredTo" => $licensing->getKeyData("registeredname"), "version" => $whmcs->getVersion()->getCanonical(), "ssl" => $whmcs->in_ssl()]];
    ...
    protected function generateResponse($response)
    {
        if ($response) {
            ...
            return ["title" => isset($response["outputTitle"]) && $response["outputTitle"] ? $response["outputTitle"] : "", "tagline" => isset($response["outputTagline"]) && $response["outputTagline"] ? $response["outputTagline"] : "", "content" => isset($response["outputContent"]) ? $response["outputContent"] : ""];
        }
        return [];
    }
    ...
    protected function activateRegistrar($moduleName, $parameters)
    {
        ...
        foreach ($parameters as $key => $value) {
            ...
            \Illuminate\Database\Capsule\Manager::table("tblregistrars")->insert(["registrar" => $moduleName, "setting" => $key, "value" => encrypt($value)]);
        }
        return true;
    }
    ...
    {
        if (\WHMCS\Module\GatewaySetting::gateway($moduleName)->exists()) {
            return false;
        }
        foreach ($parameters as $key => $value) {
            $key = \App::sanitize("a-z", $key);
            \WHMCS\Module\GatewaySetting::setValue($moduleName, $key, $value);
        }
        return true;
    }
}
